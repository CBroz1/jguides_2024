...
from jguides_2024.metadata.jguidera_brain_region import BrainRegionCohort
...
from jguides_2024.metadata.jguidera_brain_region import BrainRegionColor
...
time_bin_width = 0.1  # used to order units by peak path firing rate
min_epoch_mean_firing_rate = 0.1  # minimum epoch average firing rate threshold
# Trial
path_name = MazePathWell().get_rewarded_path_names(nwb_file_name, epoch)[
    2
]  # plot a trial along this path
well_name = "_".join(path_name.split("_")[-2:])
path_trial_num = 5
# Bars to denote task period
use_cmaps = False
task_period_colors_map = {
    "path traversal": "lightsteelblue",
    "delay": "lightgray",
    "reward": "wheat",
}
# Raster
...
# Get plot x ranges
trials_map = (DioWellDDTrials & key).trial_intervals(
    trial_feature_map={"path_names": [path_name]}, as_dict=True
)
...
ax_map = {
    k: [
        fig.add_subplot(gs[row_idx, column_idx])
        for column_idx in np.arange(0, num_columns)
    ]
...
for ax, brain_region in zip(ax_map["raster"], brain_regions):
    ...
    sort_group_unit_ids_map = (BrainRegionUnits & key).fetch1(
        "sort_group_unit_ids_map"
    )
    spike_times_df = (
        EpochSpikeTimesRelabel.RelabelEntries().spike_times_across_sort_groups(
            key, sort_group_unit_ids_map
        )
    )
    spike_times_list = spike_times_df.loc[
        [x for x in unit_order if x in spike_times_df.index]
    ].epoch_spike_times.values
    # Convert spike times to time in trial
    spike_times_list = [x - plot_x_range[0] for x in spike_times_list]
    plot_spike_times(
        spike_times_list,
        ax=ax,
        colors=color_map[brain_region],
        xlims=global_xlims,
        alpha=alpha,
...
wa_trial_interval = (DioWellArrivalTrials & key).trial_intervals(as_dict=True)[
    trial_num
]
boundaries = (
    np.asarray([plot_x_range[0]] + list(wa_trial_interval) + [plot_x_range[1]])
    - plot_x_range[0]
)  # time relative to interval start
for ax in ax_map["task_progression"]:
    for (task_period, color), (x_start, width) in zip(
        task_period_colors_map.items(), zip(boundaries, np.diff(boundaries))
    ):
        ax.add_patch(
            Rectangle((x_start, 0), width, rectangle_height, color=color)
        )
    # Remove axis
    ax.axis("off")
    # Set x lims
...
for ax in ax_map["raster"]:
    span_data = expand_interval(ax.get_ylim(), expand_factor=-0.02)
    for boundary in boundaries:
        plot_spanning_line(
            ax=ax,
            span_axis="y",
            span_data=span_data,
            constant_val=boundary,
            zorder=0,
            color="gray",
            linewidth=3,
            alpha=0.3,
...
for idx, (ax, brain_region) in enumerate(
    zip(ax_map["task_progression"], brain_regions)
):
    xlabel, ylabel = "", ""
    if idx == 0:
        xlabel = "Time (s)"
