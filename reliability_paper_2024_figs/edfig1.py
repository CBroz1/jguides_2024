...
num_epochs_per_day = [8, 8, 6]

key = dict()
cum_x_vals = []
xticklabels = []
for subject_id_idx, (subject_id, nwb_file_names) in enumerate(
    nwb_file_names_map.items()
):
    ...
    for nwb_file_name, num_epochs in zip(nwb_file_names, num_epochs_per_day):
        ...
        epochs = EpochsDescriptions().get_epochs(
            nwb_file_name, "valid_single_contingency_runs"
        )
        ...
        performance = [
            (
                AlternationTaskPerformanceStatistics
                & {**key, **{"epoch": epoch}}
            ).fetch1("percent_correct")
            for epoch in epochs
        ]

        # Define x values
        # ...Get all run epochs
        all_run_epochs = (RunEpoch() & {"nwb_file_name": nwb_file_name}).fetch(
            "epoch"
        )
        ...
        all_run_epochs = [all_run_epochs[0]] + [
            epoch
            for epoch, diff in zip(all_run_epochs[1:], np.diff(all_run_epochs))
            if diff > 1
        ]
        ...
        x_vals = (
            np.asarray(
                [
                    unpack_single_element(np.where(all_run_epochs == epoch)[0])
                    for epoch in epochs
                ]
            )
            + x1
        ...
        ax.plot(
            x_vals,
            performance,
            get_plot_marker(nwb_file_name=nwb_file_name),
            color="black",
            markersize=markersize,
            alpha=marker_alpha,
        )
        # ...Lines between contiguous sessions
        for x_val_1, x_val_2, p1, p2 in zip(
            x_vals[:-1], x_vals[1:], performance[:-1], performance[1:]
        ):
        ...
        if subject_id_idx == 0:
            ...
            xticklabels += [x1_label + 1] + [""] * (
                num_epochs - 1
            )  # add one so not zero indexed to x tick labels
