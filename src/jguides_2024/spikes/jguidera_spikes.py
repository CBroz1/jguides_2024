...
class EpochSpikeTimes(ComputedBase):
    ...
    def make(self, key):
        ...
        if len(curated_spike_sorting_entry) != 1:
            raise Exception(
                f"Should have found exactly one entry in CuratedSpikeSorting for key but found "
                f"{len(curated_spike_sorting_entry)}"
            )
        curated_spike_sorting_entry = curated_spike_sorting_entry[
            0
        ]  # take single entry in table
        ...
        if "units" in curated_spike_sorting_entry:
            ...
            if all(
                np.logical_and(
                    t_range[0] + margin_tolerance + premaze_duration
                    >= sort_interval[0],
                    t_range[1] - margin_tolerance <= sort_interval[1],
                )
                for t_range in epoch_valid_times
            ):  # if all epoch valid times within sort interval
                epoch_spikes_dict = {
                    unit_id: spike_times[
                        np.asarray(
                            [
                                np.logical_and(
                                    spike_times > t_range[0],
                                    spike_times < t_range[1],
                                )
                                for t_range in epoch_valid_times
                            ]
                        ).sum(axis=0)
                    ...
                    for unit_id, spike_times in curated_spike_sorting_entry[
                        "units"
                    ]["spike_times"].items()
                }
                epoch_spikes_df = pd.DataFrame.from_dict(
                    {
                        "unit_id": list(epoch_spikes_dict.keys()),
            ...
            *[
                (
                    ...
                    event_times_in_intervals(
                        event_times=epoch_spike_times.values[0],
    ...
    def cleanup(self, safemode=True):
        ...
        invalid_curation_ids = set(self.fetch("curation_id")) - set(
            valid_curation_ids
        )
        for invalid_curation_id in invalid_curation_ids:
            ...
            (
                ...
                & {"curation_id": invalid_curation_id}
            ).delete()
    ...
    def populate_(
        ...
        if "curation_id" in key_:
            if key["curation_id"] != target_curation_id:
                raise Exception(
                    f"curation_id must be {target_curation_id} when running populate_ on EpochSpikeTimes, "
...
class EpochSpikeTimesRelabelParams(SecKeyParamsBase):
    ...
    def insert_defaults(self, **kwargs):
        ...
        key_filter = add_param_defaults(
            key_filter, target_keys=["sorter"], add_nonexistent_keys=True
        )
        ...
        pos_table_entries = [
            k for k in table_entries if "pos" in k["sort_interval_name"]
        ]
        # Get starting interval list name for these sort intervals
        starting_interval_list_names = [
            x["sort_interval_name"].split("valid times")[0] + "valid times"
            for x in pos_table_entries
        ]
        ...
        starting_interval_list_names_map = {
            (
                interval_list_name,
                nwb_file_name,
            ): EpochIntervalListName().get_epoch(
                nwb_file_name, interval_list_name
            )
            for interval_list_name, nwb_file_name in set(
                list(zip(starting_interval_list_names, pos_nwb_file_names))
            )
        }
        ...
        pos_epochs = [
            starting_interval_list_names_map[
                (interval_list_name, nwb_file_name)
            ]
            for interval_list_name, nwb_file_name in zip(
        ...
        non_pos_table_entries = [
            k for k in table_entries if "pos" not in k["sort_interval_name"]
        ]
            ...
            key.update(
                {
                    "curation_name": get_curation_name(
                        key["sort_interval_name"], key["curation_id"]
                    )
                }
            )
            # Insert into table
            self.insert1(
                {k: key[k] for k in get_table_key_names(self)},
...
def convert_df_index_unit_name(unit_id_df, sort_group_id, unit_ids=None):
    ...
    sort_group_unit_ids = [
        get_unit_name(sort_group_id, unit_id) for unit_id in df_subset.index
    ]
...
class EpochSpikeTimesRelabel(ComputedBase):
    ...
    class RelabelEntries(PartBase):
        ...
        def spike_times_across_sort_groups(self, key, sort_group_unit_ids_map):
            return pd.concat(
                [
                    convert_df_index_unit_name(
                        (
                            self & {**key, **{"sort_group_id": sort_group_id}}
                        ).fetch1_dataframe(),
                        sort_group_id,
                        unit_ids,
                    )
    ...
    def make(self, key):
        ...
        if len((EpochSpikeTimes & relabel_params)) == 0:
            print(
                f"No entry in EpochSpikeTimes for {relabel_params}; cannot populate EpochSpikeTimesRelabel"
            )
            return
    ...
    def delete_(self, key, safemode=True):
        ...
        for curation_name in get_table_curation_names_for_key(self, key):
            ...
            delete_(
                self,
                [EpochMeanFiringRate, ResEpochSpikeCountsSel],
                key,
                safemode,
            )
...
class EpochMeanFiringRate(ComputedBase):
    ...
    def make(self, key):
        ...
        mean_firing_rate_list = [
            calculate_average_event_rate(
                event_times=epoch_spike_times,
                valid_time_intervals=epoch_interval_list,
            )
            for epoch_spike_times in epoch_spike_times_df["epoch_spike_times"]
        ]
        mean_firing_rate_df = pd.DataFrame.from_dict(
            {
                "unit_id": epoch_spike_times_df.index,
