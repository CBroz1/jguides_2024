...
from jguides_2024.metadata.jguidera_histology import LivermoreD2, ValidShank
...
from jguides_2024.spike_sorting_curation.spikesorting_helpers import (
    get_peak_ch_map,
)
...
from jguides_2024.utils.dict_helpers import (
    dict_comprehension,
    dict_comprehension_repeated_keys,
)
...
class EpsUnitsSel(SelBase):
    ...
    def _get_potential_keys(self, key_filter=None, verbose=True):
        ...
        brain_region_sort_group_id_map = (
            BrainRegionSortGroup().get_brain_region_sort_group_id_map()
        )
        ...
        for idx, key in enumerate(keys):
            ...
            brain_region_sort_group_id_map_subset = df_filter_columns(
                brain_region_sort_group_id_map, df_key
            )
            ...
            brain_region = unpack_single_element(
                brain_region_sort_group_id_map_subset["brain_region"].values
            )

            # Continue if brain region not in brain region cohort we want
            if brain_region not in (
                BrainRegionCohort()
                & {
                    "nwb_file_name": nwb_file_name,
            ...
            if len(table_subset) == 0 and verbose:
                print(
                    f"no entry in CurationSet for curation_key {curation_key}"
                )
            ...
            if len(table_subset) > 0:
                ...
                for epoch in epochs:
                    ...
                    eps_description_key = {
                        "nwb_file_name": nwb_file_name,
                        "epochs_id": get_epochs_id([epoch]),
                    }

                    eps_description_table_subset = (
                        EpochsDescription & eps_description_key
                    )
                    ...
                    epochs_description = eps_description_table_subset.fetch1(
                        "epochs_description"
                    )
                    ...
                    curation_epochs = (
                        DefineSortInterval.get_epochs_for_curation_name(
                            nwb_file_name, curation_name
                        )
                    )

                    epoch_mean_fr_populated *= (
                        len(
                            EpochMeanFiringRate
                            & {
                                **key,
                ...
                if epoch_mean_fr_populated and valid_epochs:
                    ...
                    if verbose:
                        print(
                            f"Need both EpochMeanFiringRate populated for this key and all valid epochs."
...
class EpsUnits(ComputedBase):
    ...
    def make(self, key):
        min_epoch_mean_firing_rate = (EpsUnitsParams & key).fetch1(
        ...
        for epoch in epochs:
            ...
            if len(EpochMeanFiringRate & epoch_key) == 0:
                print(
                    f"Could not populate EpsUnits for key {key} because no corresponding entry in "
            ...
            if len(df) > 0:  # if units for this epoch, update unit ids
                unit_ids_epoch = set(
                    df[
                        df["mean_firing_rate"] >= min_epoch_mean_firing_rate
                    ].index
                )
        ...
        for epoch in epochs:
            ...
            insert1_print(
                self.Upstream,
                {**key, **(EpochMeanFiringRate & key).fetch1("KEY")},
            )
    ...
    def get_sort_group_unit_ids_map(
        self, sort_group_ids=None, single_unit=True
    ):
        # Return map from sort group id to unit ids
        # First, check that same value at primary keys other than sort group id
        [
            check_single_element(x)
            for x in self.fetch(
                *[x for x in self.primary_key if x != "sort_group_id"]
            )
        ]
        # Make map
        sort_group_unit_ids_map = dict_comprehension(
            *self.fetch("sort_group_id", "unit_ids")
        )
        ...
        if sort_group_ids is not None:
            ...
            check_membership(
                ...
                "passed sort group ids",
                "available sort group ids",
            )
            sort_group_unit_ids_map = {
                k: sort_group_unit_ids_map[k] for k in sort_group_ids
            }
        ...
        if single_unit:
            nwb_file_name, curation_name = [
                ...
                for x in list(
                    map(
                        check_return_single_element,
                        self.fetch("nwb_file_name", "curation_name"),
                    )
                )
            ]
            sort_group_unit_ids_map = {
                sort_group_id: [
                    ...
                    if get_unit_label(
                        nwb_file_name, sort_group_id, unit_id, curation_name
                    )
                    == "accept"
                ]
    ...
    def unit_names_to_sort_group_unit_ids_map(unit_names, sort_unit_ids=True):
        ...
        return dict_comprehension_repeated_keys(
            sort_group_ids, unit_ids, sort_unit_ids
        )

    def rand_subset_units(
        self,
        unit_subset_size,
        single_unit=True,
        sort_group_ids=None,
        ...
        rand_unit_names = random_sample(
            unit_names, unit_subset_size, replace, tolerate_error=tolerate_error
        )
        ...
        if as_dict:
            return self.unit_names_to_sort_group_unit_ids_map(
                rand_unit_names, sort_unit_ids
            )
...
class BrainRegionUnitsParams(SecKeyParamsBase):
    ...
    def insert_epochs(
        ...
        secondary_key_subset_map = {
            k: key[k] for k in self._param_name_secondary_key_columns()
        }
        key.update(
            {
                "brain_region_units_param_name": self._make_param_name(
                    secondary_key_subset_map
                )
            }
        )
        ...
        if key["unit_subset_type"] == "all":
            if not all(
                [
                    x is None
                    for x in [
                        key["unit_subset_size"],
                        key["unit_subset_iteration"],
                    ]
                ]
            ):
                raise Exception(
    ...
    def _combination_param_sets():
        ...
        return [
            ...
            [True, "rand_target_region", 50, unit_subset_num]
            for unit_subset_num in np.arange(0, 10)
        ]
    ...
    def _default_params(self):
        ...
        primary_min_epoch_mean_frs = [0.1]
        primary_eps_units_param_names = [
            EpsUnitsParams().lookup_param_name([x])
            for x in primary_min_epoch_mean_frs
        ]
        ...
        glm_params = [
            [EpsUnitsParams().lookup_param_name([0]), epochs_description]
            + glm_combination_param_set
            for epochs_description in epochs_descriptions
        ]

        # Return parameters
        return [
            [eps_units_param_name, epochs_description] + combination_param_set
            for eps_units_param_name in primary_eps_units_param_names
            for epochs_description in epochs_descriptions
            for combination_param_set in combination_param_sets
        ] + glm_params

    def _param_name_secondary_key_columns(self):
        return [
            x for x in get_table_secondary_key_names(self) if x != "unit_subset"
        ]

    def _get_eps_description_key(
        self, nwb_file_name, brain_region_units_param_name=None
    ):
        if brain_region_units_param_name is None:
            brain_region_units_param_name = self.fetch1(
        ...
        key.update(
            {"epochs_description": (self & key).fetch1("epochs_description")}
        )
        return key
    ...
    def get_epochs(self, nwb_file_name, brain_region_units_param_name=None):
        return (
            EpochsDescription
            & self._get_eps_description_key(
                nwb_file_name, brain_region_units_param_name
            )
        ).fetch1("epochs")
    ...
    def _make_param_name(
        ...
        check_set_equality(
            ...
            self._param_name_secondary_key_columns(),
            "passed secondary key subset keys",
            "table secondary key names",
        )
        # ...Check unit subset type valid
        self._check_unit_subset_type(
            secondary_key_subset_map["unit_subset_type"]
        )
        ...
        if secondary_key_subset_map["unit_subset_type"] not in [
            "rand",
            "rand_target_region",
        ]:  # not random subset
            non_unit_subset_names = [
                x
                for x in self._param_name_secondary_key_columns()
                if x not in ["unit_subset_size", "unit_subset_iteration"]
            ]
            return "_".join(
                [secondary_key_subset_map[k] for k in non_unit_subset_names]
            )
        # ...if random subset, string together secondary key values except for unit subset which provides no additional
        # information
        secondary_key_subset_map = {
            k: secondary_key_subset_map[k]
            for k in self._param_name_secondary_key_columns()
        }
        return super()._make_param_name(
    ...
    def lookup_epochs_param_name(
        ...
        if insert_upstream:
            if all(
                [
                    epoch
                    in (RunEpoch & {"nwb_file_name": nwb_file_name}).fetch(
                        "epoch"
                    )
                    for epoch in epochs
                ]
            ):
                EpochsDescription().insert_runs(
                    nwb_file_name, epochs
                )  # insert runs
            else:
                raise Exception(
    ...
    def lookup_single_epoch_param_name(
        ...
        # Return param name
        return self.lookup_epochs_param_name(
            nwb_file_name,
            [epoch],
            min_epoch_mean_firing_rate,
            unit_subset_type,
...
class BrainRegionUnitsSel(SelBase):
    ...
    def get_eps_units_key(key):
        ...
        eps_units_param_name = (BrainRegionUnitsParams & key).fetch1(
            "eps_units_param_name"
        )
    ...
    def check_sort_group_ids_present(self, key, tolerate_error=True):
        ...
        eps_units_sort_group_ids = (
            EpsUnits & self.get_eps_units_key(key)
        ).fetch("sort_group_id")
        ...
        return check_membership(
            sort_group_ids,
            eps_units_sort_group_ids,
            "sort group ids in BrainRegionSortGroup",
            "sort group ids in EpsUnits",
            tolerate_error,
        )
    ...
    def _get_potential_keys(self, key_filter=None, verbose=True):
        ...
        keys = []
        for k1 in ((JguideraNwbfile * BrainRegionSortGroup) & key_filter).fetch(
            "KEY"
        ):
            ...
            for k2 in (EpochsDescription & k1).fetch("KEY"):
                ...
                brup_keys = [
                    k
                    for k in brup_keys
                    if (BrainRegionUnitsParams & k).fetch1("unit_subset_type")
                ...
                for k3 in brup_keys:
                    ...
                    curation_names = set(
                        (EpsUnits & self.get_eps_units_key(k3)).fetch(
                            "curation_name"
                        )
                    )
                    ...
                    single_epochs_descriptions = [
                        EpochsDescription().get_single_run_description(
                            k3["nwb_file_name"], epoch
                        )
                        for epoch in epochs
                    ]

                    valid_curation_names = [
                        table_subset.get_curation_name(
                            k3["brain_region"],
                            epochs_description,
...
class BrainRegionUnits(ComputedBase):
    ...
    def make(self, key):
        ...
        selection_table().check_sort_group_ids_present(
            key, tolerate_error=False
        )

        # Define quantities for taking unit subset as indicated
        unit_subset_type = (BrainRegionUnitsParams & key).fetch1(
            "unit_subset_type"
        )
        ...
        unit_subset_size = (BrainRegionUnitsParams & key).fetch1(
            "unit_subset_size"
        )

        # Get electrode groups in this brain region
        targeted_location = get_targeted_location_from_brain_region(
            key["brain_region"]
        )
        nwb_file_name = key["nwb_file_name"]
        ...
        if len(table_subset) == 0:
            raise Exception(
                f"ElectrodeGroupTargetedLocation has no entries for {nwb_file_name}, {targeted_location}"
            )
        ...
        # ...All units
        sort_group_unit_ids_map = (
            EpsUnits & eps_units_key
        ).get_sort_group_unit_ids_map(
            sort_group_ids, single_unit
        )  # default
        ...
        if unit_subset_type == "rand":
            ...
            if sort_group_unit_ids_map is None:
                ...
                if (
                    unit_subset_type in ["target_region", "rand_target_region"]
                    and key["brain_region"] != "CA1_targeted"
                ):
        ...
        # 2) within target region based on histology
            ...
            check_membership(
                [key["brain_region"]],
                ["mPFC_targeted", "OFC_targeted", "CA1_targeted"],
            )
            ...
            if (
                subject_id in ["J16", "mango", "june", "fig", "peanut"]
                and key["brain_region"] == "CA1_targeted"
            ):
                pass
            ...
            # could be in different areas)
                ...
                for electrode_group_name in electrode_group_names:
                    ...
                    valid_chs += list(
                        electrode_ids[
                            LivermoreD2().get_valid_idxs(
                                below_dorsal_limit_lens,
                                below_ventral_limit_lens,
                            )
                        ]
                    )
                ...
                for sort_group_id, unit_ids in sort_group_unit_ids_map.items():
                    ...
                    peak_ch_map = get_peak_ch_map(
                        key["nwb_file_name"], sort_group_id, sort_interval_name
                    )
                    valid_unit_ids = [
                        unit_id
                        for unit_id, peak_ch in peak_ch_map.items()
                        if peak_ch in valid_chs
                    ]
                    sort_group_unit_ids_map[sort_group_id] = [
                        x for x in unit_ids if x in valid_unit_ids
                    ]
            ...
            if unit_subset_type == "rand_target_region":
                ...
                # Get random sample
                unit_names_sample = random_sample(
                    unit_names,
                    unit_subset_size,
                    replace=False,
                    tolerate_error=True,
                ...
                sort_group_unit_ids_map = (
                    EpsUnits.unit_names_to_sort_group_unit_ids_map(
                        unit_names_sample
                    )
                )
            ...
            raise Exception(
                f"unit_subset_type {unit_subset_type} not accounted for in code"
            )

        # Insert into main table
        insert1_print(
            self,
            {**key, **{"sort_group_unit_ids_map": sort_group_unit_ids_map}},
        )
    ...
    def get_unit_name_df(
        ...
        if brain_regions is None:
            ...
            brain_regions = [
                x for x in brain_regions if x in valid_brain_regions
            ]
        ...
        for brain_region in brain_regions:
            ...
            curation_name = check_return_single_element(
                df_filter_columns(
                    curation_names_df, df_key
                ).curation_name.values
            ).single_element
            ...
            ).fetch1("sort_group_unit_ids_map")
    ...
    def populate_(self, **kwargs):
        EpochSpikeTimesRelabel().populate_(
            **kwargs
        )  # use populate_ to populate epoch spike times relabel params table
        super().populate_(**kwargs)
...
class BrainRegionUnitsCohortType(SecKeyParamsBase):
    ...
    def insert_defaults(self, **kwargs):
        ...
        for key in keys:
            key.update(
                {"brain_region_units_cohort_type": self._make_param_name(key)}
            )
    ...
    def _unit_subset_iterations_map(cls):
        ...
        for unit_subset_iteration in np.arange(0, 10):
            unit_subset_iterations_map[
                cls._make_unit_subset_iterations_map_key(
                    [unit_subset_iteration]
                )
        ...
        unit_subset_iterations_map[
            cls._make_unit_subset_iterations_map_key(unit_subset_iterations)
    ...
    def _get_unit_subset_iterations_shorthand(cls, unit_subset_iterations):
        return cls._unit_subset_iterations_map()[
            cls._make_unit_subset_iterations_map_key(unit_subset_iterations)
        ]
    ...
    def _make_param_name(self, secondary_key_subset_map):
        ...
        check_membership(
            secondary_key_subset_map.keys(),
            get_table_secondary_key_names(self),
            "passed secondary keys",
            f"{get_table_name(self)} secondary keys",
        )
        # Enforce secondary key order in table definition
        secondary_key_subset_map = {
            k: secondary_key_subset_map[k]
            for k in get_table_secondary_key_names(self)
        ...
        secondary_key_subset_map["unit_subset_iterations"] = (
            self._get_unit_subset_iterations_shorthand(
                secondary_key_subset_map["unit_subset_iterations"]
            )
        )
        ...
        param_name_vals = [
            x for x in secondary_key_subset_map.values() if x != ""
        ]

        # String together values
        return make_param_name(
            param_name_vals, separating_character, tolerate_non_unique
        )
