...
from src.jguides_2024.utils.make_rate_map import (
    make_1D_rate_map_measurement_bouts,
)
    ...
    def make(self, key):
        ...
        # Otherwise get df with rate map
            ...
            unit_ids, event_rates = unzip_as_list(
                [
                    (
                        unit_id,
                        make_1D_rate_map_measurement_bouts(
                            event_times, ppt_bouts, ppt_bouts_t, ppt_bin_edges
                        )[0],
                    )
                    for unit_id, event_times in epoch_spike_times_df[
                        "epoch_spike_times"
                    ].items()
                ]
            )
            frmap_ppt_df = pd.DataFrame.from_dict(
                {
                    "unit_id": unit_ids,
                    "rate_map": event_rates,
                    "ppt_bin_centers": [vector_midpoints(ppt_bin_edges)]
                    * len(unit_ids),
                    "ppt_bin_edges": [ppt_bin_edges] * len(unit_ids),
                    "num_trials": [len(ppt_bouts)] * len(unit_ids),
                }
            )

        # Insert into table
        insert_analysis_table_entry(
            self, [frmap_ppt_df], key
        )  # insert into table
...
def _rate_maps_both_finite(fr_df_1, fr_df_2, unit_id):
    fr_1 = _unit_data(
        fr_df_1, unit_id, "smoothed_rate_map"
    )  # first epoch rate map
    fr_2 = _unit_data(
        fr_df_2, unit_id, "smoothed_rate_map"
    )  # second epoch rate map
    valid_bool = vectors_finite_idxs(
        [fr_1, fr_2]
    )  # indices where rate maps both finite
...
def _calculate_corr(fr_df_1, fr_df_2, unit_id):
    ...
    return tuple(
        [len(fr_1_valid)] + list(sp.stats.pearsonr(fr_1_valid, fr_2_valid))
    )
...
def _get_fr_df_unit_ids_two_entries(key, fr_df, original_key_names):
    fr_df_1 = (
        ...
        & {
            **key,
            **{
                original_key_name: key[f"{original_key_name}_1"]
                for original_key_name in original_key_names
            },
        }
    ).fetch1_dataframe()
    fr_df_2 = (
        ...
        & {
            **key,
            **{
                original_key_name: key[f"{original_key_name}_2"]
                for original_key_name in original_key_names
            },
        }
    ).fetch1_dataframe()
    unit_ids = list(
        set(fr_df_2.index).intersection(fr_df_1.index)
    )  # units present in both epochs
    for unit_id in unit_ids:  # check same bin edges across epochs
        if not all(
            _unit_data(
                fr_df_1, unit_id, "ppt_bin_edges"
...
def _similarity_df(
    ...
    similarity_dict = {
        "unit_id": unit_ids,
        similarity_measure_name: np.asarray(similarity_measures),
        "number_samples": np.asarray(num_samples),
        f"{original_attribute_name}_1": [key[f"{original_attribute_name}_1"]]
        * len(unit_ids),
        f"{original_attribute_name}_2": [key[f"{original_attribute_name}_2"]]
        * len(unit_ids),
    }
...
class CorrFrmapPptSm(ComputedBase):
    ...
    def make(self, key):
        fr_df_1, fr_df_2, unit_ids = _get_fr_df_unit_ids_two_entries(
            key, FrmapPptSm, ["epoch"]
        )
        num_samples, corr_coeffs, p_vals = zip(
            *[
                _calculate_corr(fr_df_1, fr_df_2, unit_id)
        ...
        _insert_similarity_table_entry(
            self, key, similarity_df, "corr_frmap_ppt_sm_object_id"
        )
...
class OverlapFrmapPptSm(ComputedBase):
    ...
    def make(self, key):
        ...
        num_samples, overlaps = zip(
            *[
                _calculate_overlap(fr_df_1, fr_df_2, unit_id)
                for unit_id in unit_ids
            ]
        )
        similarity_df = _similarity_df(
            unit_ids, overlaps, "overlap", num_samples, key, "epoch"
        )
        _insert_similarity_table_entry(
            self, key, similarity_df, "overlap_frmap_ppt_sm_object_id"
        )
...
class FrmapPupt(FrmapBase):
    ...
    def make(self, key):
        ...
        for path_name in path_names:
            ppt_df_subset = df_filter_columns(
                ppt_df, {"trials_path_name": path_name}
            )
    ...
    def make(self, key):
        ...
        for path_name in path_names:
            ...
            for ppt_bout, ppt_bout_t in zip(ppt_bouts, ppt_bouts_t):
                event_rates_list += [
                    make_1D_rate_map_measurement_bouts(
                        event_times, [ppt_bout], [ppt_bout_t], ppt_bin_edges
                    )[0]
                    for event_times in epoch_spike_times_df[
                        "epoch_spike_times"
                    ].values
                ]
            unit_ids = list(epoch_spike_times_df["epoch_spike_times"].index)
            trial_start_epoch_trial_numbers = duplicate_elements(
                ppt_df_subset.index, len(unit_ids)
            )
            ...
            dio_well_da_trials_param_name = (PptParams & key).fetch1(
            ...
            info_dict = {
                ...
                for k, v in zip(
                    ...
                    unpack_df_columns(
                        ...
                        .set_index("trial_start_epoch_trial_numbers")
                        .loc[trial_start_epoch_trial_numbers],
                        copy_column_names,
                    ),
                )
            ...
            insert_analysis_table_entry(
                self,
                nwb_objects=[st_frmap_pupt_df],
                key=key,
                nwb_object_names=[self.get_object_id_name()],
            )
...
class STFrmapPuptSm(ComputedBase):
    ...
    def make(self, key):
        ...
        smoothed_firing_rate_map_df = smooth_datajoint_table_fr(
            fr_table=STFrmapPupt,
            params_table=FrmapPptSmParams,
            key=key,
        ...
        copy_column_names = [
            column_name
            for column_name in rate_map_df.columns
            if column_name
            not in list(smoothed_firing_rate_map_df) + ["rate_map"]
        ]
        for column_name in copy_column_names:
            smoothed_firing_rate_map_df[column_name] = rate_map_df[
                column_name
            ].values
