...
class STFrmapWAD(ComputedBase):
    ...
    def make(self, key):
        ...
        single_trial_firing_rate_map_df = make_well_single_trial_table_fr_df(
            ...
            trials_params_table=DioWellADTrialsParams,
        ...
        insert_analysis_table_entry(
            self,
            [single_trial_firing_rate_map_df],
            key,
            [self.get_object_id_name()],
        )
...
class STFrmapWADSmWTSel(SelBase):
    ...
    def insert_defaults(self, **kwargs):
        ...
        for key in STFrmapWADSm().fetch("KEY"):
            for params in [(0, 1, 0.05)]:
                WarpedAxisBinsParams().insert_entry(params)
                WarpedAxisBins.populate()
                warped_axis_bins_param_name = (
                    WarpedAxisBinsParams().lookup_param_name(params)
                )
                key.update(
                    {"warped_axis_bins_param_name": warped_axis_bins_param_name}
                )
...
class STFrmapWADSmWT(ComputedBase):
    ...
    def make(self, key):
        ...
        bin_centers, bin_edges = (WarpedAxisBins & key).fetch1(
            "bin_centers", "bin_edges"
        )

        # 1) Rescale time bin edges to specified arbitrary axis
        rescaled_time_bin_edges_list = [
            rescale_1d(bin_edges, x) for x in fr_df["time_bin_edges"]
        ]
        rescaled_time_bin_centers_list = list(
            map(vector_midpoints, rescaled_time_bin_edges_list)
        )

        # 2) Sample rescaled rate map in warped axis bins
        rate_map_interp = [
            np.interp(bin_centers, rescaled_time_bin_centers, rate_map)
            for rescaled_time_bin_centers, rate_map in zip(
                rescaled_time_bin_centers_list, fr_df["smoothed_rate_map"]
            )
        ]
        ...
        # Store in table
        insert_analysis_table_entry(
            self, [fr_df], key, [self.get_object_id_name()]
        )
...
class FrmapWADSmWTParams(SecKeyParamsBase):
    ...
    def _make_param_name(self, secondary_key_subset_map):
        ...
        return super()._make_param_name(
            {
                k: order_trial_feature_names(k, v)
...
class FrmapWADSmWT(FrmapBase):
    ...
    def make(self, key):
        trial_feature_names = (FrmapWADSmWTParams & key).fetch1(
            "trial_feature_names"
        )
        fr_df = (STFrmapWADSmWT & key).fetch1_dataframe()
        new_bin_centers, new_bin_edges = (WarpedAxisBins & key).fetch1(
            "bin_centers", "bin_edges"
        )

        data_list = []
        for df_key in unique_df_column_sets(
            fr_df, trial_feature_names, as_dict=True
        ):
            ...
            ave_fr = np.nanmean(
                np.asarray(
                    [
                        np.interp(new_bin_centers, bin_centers, rate_map)
                        for bin_centers, rate_map in zip_df_columns(
                            df_subset, ["bin_centers", "smoothed_rate_map"]
                        )
                    ]
                ),
                axis=0,
            ...
            data_list.append(
                tuple(
                    list(df_key.values())
                    + [ave_fr, new_bin_centers, new_bin_edges]
                )
            )

        # Store in dataframe
        entry_names = trial_feature_names + [
            "smoothed_rate_map",
            "bin_centers",
            "bin_edges",
        ]
        ...
        # Store in table
        insert_analysis_table_entry(
            self, [ave_fr_df], key, [self.get_object_id_name()]
        )


def populate_jguidera_well_arrival_departure_firing_rate_map(
