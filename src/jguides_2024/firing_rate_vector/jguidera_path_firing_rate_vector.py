...
class PathFRVecParams(SecKeyParamsBase):
    ...
    def _default_params(self):
        return [
            [x] for x in ["none", "even_odd_trials", "correct_incorrect_trials"]
        ]
...
class PathFRVecSel(CovariateFRVecSelBase):
    ...
    def _get_potential_keys(self, key_filter=None, verbose=True):
        ...
        primary_ppt_dig_param_name = PptDigParams().lookup_param_name([0.05])
        ...
        all_features = {
            ...
            "ppt_dig_param_name": [
                PptDigParams().lookup_param_name([x]) for x in [0.05]
            ],
