...
class FRVecEmb(ComputedBase):
    ...
    def _plot_by_covariate(
        ...
        **plot_params,
        ...
        # Plot invalid points for context
        self._plot_invalid(
            embedding, invalid_bool, ax, plot_components, **plot_params_copy
        )
        ...
        plot_embedding_scatter(
            embedding.loc[valid_bool], ax, plot_components, **plot_params_copy
        )
        ...
        **plot_params,
        ...
        if exclusion_params is not None:
            check_membership(
                ...
                ["stay_trial", "leave_trial", "paths", "epoch_trial_numbers"],
                "exclusion type",
        ...
        for path_name in path_names:
            ...
            if task_period == "path":
                trial_intervals = (DioWellDATrials & key).trial_intervals(
                    trial_feature_map, as_dict=True
                )
                ...
                trial_intervals = {
                    k: v
                    for k, v in zip(
                        df_subset["trial_start_epoch_trial_numbers"],
                        trial_intervals,
                    )
                }
            ...
            if "epoch_trial_numbers" in exclusion_params["exclusion_types"]:
                trial_intervals = {
                    k: v
                    for k, v in trial_intervals.items()
                    if k not in exclusion_params["epoch_trial_numbers"]
                }
            ...
            valid_bool = np.logical_and(
                np.ndarray.flatten(dfs.epoch_vector.values) == target_epoch,
                event_times_in_intervals_bool(
                    dfs.embedding.index.values, trial_intervals_
                ),
            )
            ...
            first_crossed_junction_df = (
                Ppt & key
            ).get_first_crossed_junction_df()
            for junction_num, marker in zip([0, 1], ["x", "o"]):
                df_subset = df_filter_columns(
                    first_crossed_junction_df, {"junction_number": junction_num}
                )
                # ...restrict to valid epoch trial numbers
                df_subset = df_subset.loc[list(trial_intervals.keys())]
                idxs = [
                    np.argmin(abs(dfs.embedding.index - x))
                    for x in df_subset.time_estimate
                ]
                junction_params = {"marker": marker, "s": 80, "alpha": 1}
                self._plot_single_color(
                    dfs.embedding.iloc[idxs],
                    valid_bool[idxs],
                    ax,
                    plot_components,
                    color_map[path_name],
