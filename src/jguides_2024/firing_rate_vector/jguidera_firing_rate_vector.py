...
from jguides_2024.utils.df_helpers import df_filter_columns, unpack_single_df
...
class FRVecSel(SelBase):
    ...
    def _get_potential_keys(self, key_filter=None):
        ...
        valid_nwb_file_name_epochs_map = (
            EpochsDescriptions & key
        ).get_nwb_file_name_epochs_map()
        # ...Get starting nwb files / epochs
        (
            nwb_file_names,
            epochs_descriptions,
            epochs_list,
        ) = EpochsDescription().fetch(
        ...
        for nwb_file_name in nwb_file_names:
            if nwb_file_name not in valid_nwb_file_name_epochs_map:
                valid_nwb_file_name_epochs_map[nwb_file_name] = (
                    []
                )  # indicate no valid epochs
        ...
        print(
            "valid_nwb_file_name_epochs_map: ", valid_nwb_file_name_epochs_map
        )

        valid_bool = [
            np.logical_and(
                len(epochs) == 1,
                epochs[0] in valid_nwb_file_name_epochs_map[nwb_file_name],
            )
