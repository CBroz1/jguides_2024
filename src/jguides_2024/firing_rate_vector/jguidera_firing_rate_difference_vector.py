...
class FRDiffVec(ComputedBase):
    ...
    def make(self, key):
        ...
        fr_df, time_vector = FRVec().firing_rate_across_sort_groups(
        ...
        fr_diff_df = pd.DataFrame.from_dict(
            {unit_name: x for unit_name, x in zip(fr_df.index, fr_diff_arr)}
        )

        # Store vector with concatenated time samples of vector tip
        vector_tip_time_df = pd.DataFrame.from_dict(
            {"vector_tip_time": time_vector[diff_n:]}
        )

        # Store vector with concatenated time samples of vector tail
        vector_tail_time_df = pd.DataFrame.from_dict(
            {"vector_tail_time": time_vector[:-diff_n]}
        )

        # Insert into table
        insert_analysis_table_entry(
            self, [fr_diff_df, vector_tip_time_df, vector_tail_time_df], key
        )

    def fetch1_fr_diff_vec(self, df_index_name="vector_tail_time"):
        return pd.concat(
            (
                self.fetch1_dataframe("fr_diff_vec"),
                self.fetch1_dataframe(df_index_name),
            ),
            axis=1,
        ).set_index(df_index_name)
    ...
    def plot_results(self):
        ...
        plot_heatmap(
            fr_diff_vec_arr, scale_clim=0.1, xlabel="time", ylabel="unit"
        )
