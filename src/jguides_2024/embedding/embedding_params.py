...
class EmbeddingParams:
    ...
    def _get_embedding_params(self):
        ...
        k_hpc = {
            "curation_name": "raw data valid times no premaze no home no sleep_3"
        }
        ...
        data_list += [
            (
                {**k, **k_ctx, **{"brain_region": "mPFC_targeted"}},
                [],
                173,
                -54,
                ZoomScaleFactor(  # 158, -94
                    scale_factor,
                    scale_factor,
                    scale_factor,
                    scale_factor,
            ...
            (
                {**k, **k_ctx, **{"brain_region": "OFC_targeted"}},
                [],
                136,
                111,
                ZoomScaleFactor(
            ...
            (
                {**k, **k_hpc, **{"brain_region": "CA1_targeted"}},
                [],
                63,
                -162,
                ZoomScaleFactor(
        ...
        k_hpc = {
            "curation_name": "pos 7 valid times no premaze no home no sleep_3"
        }
        scale_factor = 0.08
        data_list += [
            (
    ...
    def _get_matching_params_entry(self, key, tolerate_no_entry=False):
        ...
        target_embedding_params_keys = check_return_single_element(
            [x.keys() for x in self.embedding_params["key"]]
        ).single_element

        df_key = {k: key[k] for k in target_embedding_params_keys if k in key}
        valid_bool = [
            check_same_values_at_shared_keys([x, df_key], tolerate_error=True)
            for x in self.embedding_params["key"].values
        ]
        ...
        if num_matching_entries == 0 and not tolerate_no_entry:
            raise Exception(
                "No matching entries in embedding params for passed key"
            )
    ...
    def apply_embedding_params(
        self, key, kwargs, ax=None, param_names=None, verbose=False
    ):
        ...
        if "zoom" in param_names:
            for axis_name in ["x", "y", "z"]:
                ...
                lim_width = x2 - x1
                delta1 = (
                    getattr(df_subset.zoom_scale_factor, f"{axis_name}1")
                    * lim_width
                )
                delta2 = (
                    getattr(df_subset.zoom_scale_factor, f"{axis_name}2")
                    * lim_width
                )
