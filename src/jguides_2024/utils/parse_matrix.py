...
def _get_setting_bool(half_setting):
    ...
    return np.prod(
        np.vstack(
            [getattr(df, k) == v for k, v in zip(characteristics, half_setting)]
        ),
        axis=0,
    )
...
def parse_matrix(
    ...
    if settings is None:
        ...
        if symmetric_metric:
            half_settings = list(
                itertools.product(
                    *[np.unique(getattr(df, x).values) for x in characteristics]
                )
            )  # cartesian product
            # Get combinations of pairs of half settings
            # We have to add to these cases where items in the pair are the same
            settings_combinations = np.asarray(
                list(itertools.combinations(half_settings, r=2))
                + [(x, x) for x in half_settings],
                dtype=object,
            )
        ...
        # Asymmetric metric
            ...
            settings = list(
                itertools.product(
                    *[
                        np.unique(getattr(df, x).values)
                        for x in characteristics * 2
                    ]
                )
            )  # cartesian product
    ...
    print(
        f"Parsing square matrix of size {np.shape(arr)} for {len(settings)} settings of characteristics..."
    )
    ...
    for idx, setting in enumerate(settings):
        if verbose:
            print_iteration_progress(
                idx, len(settings), target_num_print_statements=10
            )
        pool.apply_async(
            apply_setting_bools,
            args=(setting,),
    ...
    metric_name = format_optional_var(
        metric_name, append_underscore=True
    )  # add underscore to metric name if passed
    column_names = list(
        np.concatenate([[f"{x}_{y}" for x in characteristics] for y in [1, 2]])
    ) + [
        "row_idxs",
        "column_idxs",
        f"{metric_name}mean",
        f"{metric_name}variance",
        "num_samples",
    ]
