...
def event_times_in_intervals_bool(event_times, valid_time_intervals):
    ...
    return (
        np.sum(
            np.asarray(
                [
                    np.logical_and(event_times >= t1, event_times <= t2)
                    for t1, t2 in valid_time_intervals
                ]
            ),
            axis=0,
        )
...
def bins_in_intervals_bool(valid_intervals, bin_centers, bin_edges=None):
    ...
    return (
        np.sum(
            np.vstack(
                [
                    event_times_in_intervals_bool(x, valid_intervals)
                    for x in [bin_centers, bin_starts, bin_ends]
                ]
            ),
            axis=0,
        )
...
def bins_in_intervals(
    valid_intervals, bin_centers, bin_edges=None, verbose=False
):
    ...
    if verbose:
        ...
        ax.plot(
            valid_bin_centers,
            [1] * len(valid_bin_centers),
            "x",
            color="red",
            label="valid_bin_centers",
        )
...
def calculate_average_event_rate(event_times, valid_time_intervals):
    ...
    """
    if (
        len(np.shape(valid_time_intervals)) != 2
        or np.shape(valid_time_intervals)[1] != 2
    ):
        raise Exception(
            f"valid_intervals must be n by 2 array, but has shape {np.shape(valid_time_intervals)}"
        )
    _, valid_event_times = event_times_in_intervals(
        event_times, valid_time_intervals
    )
    return len(valid_event_times) / np.sum(np.diff(valid_time_intervals))


def ideal_poisson_error(observed_counts, num_ideal_trials=100):
    observed_counts_arr = np.tile(observed_counts, (num_ideal_trials, 1))
    return mean_squared_error(
        np.random.poisson(observed_counts_arr), observed_counts_arr
    )
...
def not_small_diff_bool(x, diff_threshold):
    ...
    return np.concatenate(([True], valid_bool)) * np.concatenate(
        (valid_bool, [True])
    )  # True if x value is NOT part of small gap pair


def get_event_times_relative_to_trial_start(
    event_times, trial_intervals, trial_start_time_shift=0
):
    """
    ...
    event_times_in_trials_idxs, event_times_trials = event_times_in_intervals(
        event_times, trial_intervals
    )
    ...
    event_times_rel_all_trials_start = (
        np.tile(event_times_trials, (len(trial_intervals), 1))
    ...
    event_times_rel_trial_start = (
        min_positive_val_arr(event_times_rel_all_trials_start, axis=0)
        + trial_start_time_shift
    )
    return event_times_rel_trial_start, event_times_in_trials_idxs


def get_full_event_times_relative_to_trial_start(
    event_times, trial_intervals, trial_start_time_shift=0
):
    ...
    valid_relative_times, valid_idxs = get_event_times_relative_to_trial_start(
        event_times, trial_intervals, trial_start_time_shift
    )
