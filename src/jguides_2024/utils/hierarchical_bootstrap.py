    ...
    for (
        ...
        data_list.append(
            tuple(
                ...
                + [average_fn(resample_df[resample_quantity])]
            )
        )
    boot_ave_df = df_from_data_list(
        data_list, ave_group_column_names + ["boot_ave"], empty_df_has_cols=True
    )
...
def hierarchical_bootstrap(
    ...
    ave_conf_df = get_conf_df(
        boot_ave_dfs_concat, ave_group_column_names, "boot_ave", alphas
    )
    ...
    if ave_diff_column_name is not None:
        ave_diff_conf_df = get_conf_df(
            ...
            ave_diff_group_column_names + ave_diff_pair_column_names,
            "boot_ave_diff",
            alphas,
        )
    ...
    ave_df = df_from_data_list(
        [
            tuple(
                ...
                + [
                    average_fn(
                        df_filter_columns(df, column_set)[resample_quantity]
                    )
                ]
            )
            for column_set in ave_group_column_sets
        ],
        ave_group_column_names + [f"ave_{resample_quantity}"],
        empty_df_has_cols=True,
    )
