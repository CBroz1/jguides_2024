...
def check_membership(
    set_1, set_2, set_1_name="set_1", set_2_name="set_2", tolerate_error=False
):
    ...
    invalid_set_1_members = np.asarray(list(set_1))[
        [x not in set_2 for x in set_1]
    ]
    passed_check = len(invalid_set_1_members) == 0
    if not passed_check and not tolerate_error:  # raise error if indicated
        raise Exception(
            f"All elements in {set_1_name} should be contained within {set_2_name}. "
...
def check_set_equality(
    ...
    passed_check = check_membership(
        set_1, set_2, set_1_name, set_2_name, tolerate_error
    )
    passed_check *= check_membership(
        set_2, set_1, set_2_name, set_1_name, tolerate_error
    )
