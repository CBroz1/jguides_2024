...
def format_ax(
    ...
    if axis_off_list is not None:
        check_membership(
            axis_off_list, ["x", "y"], "axis_off_list", "valid axis_off_list"
        )
    ...
    plot_setting_objs = [
        ...
        for x in [
            ...
            (
                title,
                "set_title",
                {"label": title, "fontsize": fontsize, "color": title_color},
            ),
            ...
            (
                yticklabels,
                "set_yticklabels",
                {"labels": yticklabels, "fontsize": ticklabels_fontsize},
            ),
            (
                zticklabels,
                "set_zticklabels",
                {"labels": zticklabels, "fontsize": ticklabels_fontsize},
            ),
...
def get_gridspec_ax_maps(
    ...
    if mega_column_gap_factor is None:
        mega_column_gap_factor = 0.2
    if add_subplot_args is None:
        add_subplot_args = dict()

    # Define gaps between mega rows and between mega columns
    total_num_mega_rows = (
        len(mega_row_iterables) + num_mega_rows_top + num_mega_rows_bottom
    )
    total_num_mega_columns = (
        len(mega_column_iterables)
        + num_mega_columns_left
        + num_mega_columns_right
    )
    ...
    def _get_stack_values(
        idx, num_mega_iterables_before, total_num_mega_iterables, plot_gap
    ):
        shift = num_mega_iterables_before / total_num_mega_iterables
        return [
            idx / total_num_mega_iterables + shift + plot_gap,
    ...
    if fig is None:
        fig = plt.figure(
            figsize=get_figsize(
                num_mega_rows * num_rows,
                num_mega_columns * num_columns,
                subplot_width,
                subplot_height,
            ),
    ...
    for mega_row_idx, mega_row_val in enumerate(mega_row_iterables):
        for mega_column_idx, mega_column_val in enumerate(
            ...
            stack_args = {
                k: v
                for k, v in zip(
                    stack_names, row_stack_values + column_stack_values
                )
            }
            ...
            for row_idx, row_val in enumerate(row_iterables):
                for column_idx, column_val in enumerate(column_iterables):
                    ...
                    share_ax_map = {
                        "rows": (mega_row_val, mega_column_val, 0, column_idx),
                        "columns": (mega_row_val, mega_column_val, row_idx, 0),
                        True: (mega_row_val, mega_column_val, 0, 0),
                    }
                    share_args = dict()
                    for share_keyword, x in zip(
                        ["sharex", "sharey"], [sharex, sharey]
                    ):
                        if x in share_ax_map:
                            if share_ax_map[x] in ax_dict:
                                share_args[share_keyword] = ax_dict[
                                    share_ax_map[x]
                                ]
                    ax_dict[
                        (mega_row_val, mega_column_val, row_val, column_val)
                    ] = fig.add_subplot(
...
def get_plot_idx_map(
    mega_row_iterables, mega_column_iterables, row_iterables, column_iterables
):
    PlotIdx = namedtuple(
        "PlotIdx", "mega_row_idx mega_column_idx row_idx column_idx"
    )
    plot_idx_map = dict()
        ...
        for mega_column_idx, mega_column in enumerate(mega_column_iterables):
            for row_idx, row in enumerate(row_iterables):
                for column_idx, column in enumerate(column_iterables):
                    plot_idx_map[(mega_row, mega_column, row, column)] = (
                        PlotIdx(
...
def get_fig_axes(
    ...
    if (
        ...
        and (num_rows == 1 or num_columns == 1)
        and num_rows * num_columns < num_subplots
    ):
        if num_rows == 1:
    ...
    if (
        figsize is None
        and subplot_width is not None
        and subplot_height is not None
    ):
        figsize = get_figsize(
            num_rows, num_columns, subplot_width, subplot_height
        )
...
def get_ax_for_layout(axes, plot_num, layout="left_right"):
    ...
    # Check layout valid
    check_membership(
        [layout], ["left_right", "top_bottom"], "layout", "valid layouts"
    )
...
def save_figure(
    fig, file_name, figure_type=None, dpi=2000, save_dir=None, save_fig=True
):
    ...
    if save_fig:
        ...
        fig.savefig(
            file_name, transparent=True, dpi=dpi, bbox_inches="tight"
        )  # transparent=True gets rid of white background
...
def plot_histogram(
    ...
    if arrow_at_median and len(x) != 0:
        ...
        ax.arrow(
            ...
            -0.0001,
            head_width=0.05,
            head_length=line_extent[1] * 0.05,
            overhang=0,
            color="black",
        )  # arrow at median
...
def plot_true_ranges(ax, valid_bool, x_vals=None, y_val=None, **kwargs):
    ...
    if x_vals is not None:
        if len(x_vals) != len(valid_bool):
            raise Exception("x_vals must be same length as valid_bool")
    # Otherwise define
    else:
...
def plot_intervals(
    ...
    if len(interval_list) != len(val_list):
        raise Exception(
            "must be as many values in val_list as intervals in interval_list"
        )
...
def add_colorbar(
    ...
    cbar = fig.colorbar(
        img, ax=[ax], cax=cax, orientation=orientation, ticks=ticks
    )
...
def return_n_cmap_colors(
    num_colors=10,
    cmap=plt.get_cmap("jet"),
    squeeze_lower_range=None,
    squeeze_upper_range=None,
):
    ...
    colors = np.asarray(
        [cmap(n / total_num_colors) for n in np.arange(0, total_num_colors)]
    )
...
def get_ticklabels(
    ticks, label_every_n=None, idxs=None, include_endpoints=False, round_n=None
):
    ...
    if idxs is None:
        ticklabels = [
            str(tick) if idx % label_every_n == 0 else ""
    ...
    # Method 2: label ticks specified by passed idxs
        ...
        ticklabels = [
            str(ticks[idx]) if idx in idxs else ""
            for idx in np.arange(0, len(ticks))
        ]
...
def add_jitter(x, jitter_extent=0.1, unpack_single_value=True):
    x = np.asarray(x)
    jittered_x = x + np.random.uniform(
        -jitter_extent / 2, jitter_extent / 2, len(x)
    )
...
def tint_color(color, num_tints):
    ...
    if len(color) != 3:
        raise Exception(
            f"color should be array with 3 values denoting RGB setting. color: {color}"
        )
    return np.asarray(
        [
            color + (1 - color) * tint_factor
            for tint_factor in np.arange(0, 1, 1 / num_tints)
        ]
    )
...
def plot_violin_or_box(
    ...
    ghost_handles = [
        plt.Rectangle((0, 0), 0, 0, facecolor=violin_color, edgecolor="black")
        for violin_color in violin_colors
    ]
    for violin_plot, violin_color in zip(
        ax.findobj(PolyCollection), violin_colors
    ):
    ...
    sns.despine(
        bottom=True
    )  # note that ax.get_xaxis().set_visible(False) seemed to not work with seaborn or matplotlib plot
...
def plot_ave_conf(ax, x_vals, mean, conf_bounds, **plot_params):
    ...
    fill_alpha = plot_params.pop("fill_alpha", 0.3)
    # Get color
...
def get_centered_axis_val(
    val, vals, val_separation, extra_shift=0, jitter_extent=0
):
    idx = unpack_single_element(
        np.where(np.asarray(vals) == val)[0]
    )  # where brain region falls in list
    shift = idx * val_separation - (len(vals) - 1) * val_separation / 2
    return add_jitter([shift + extra_shift], jitter_extent)


def plot_errorbar(
    lower_conf, upper_conf, ave_val, x_val, ax, plot_average=True, **plot_params
):
    ...
    if plot_average:
        params = {
            k: v for k, v in plot_params.items() if k in ["color", "markersize"]
        }
    ...
    params = {
        ...
        if k in ["color", "solid_capstyle", "capsize", "linewidth"]
    }
