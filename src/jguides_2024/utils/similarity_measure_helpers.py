...
from jguides_2024.utils.tuple_helpers import (
    index_list_of_tuples,
    tuples_index_list,
)
...
def mean_vector_around_event(
    time_series, event_times, forward_shift, backward_shift, t_step
):
    peri_event_times_list = [
        np.arange(
            event_time + backward_shift,
            event_time + forward_shift + t_step,
            t_step,
        )
        for event_time in event_times
    ]  # times around each event
    vector_peri_events = [
        np.interp(peri_event_times, time_series.index, time_series.values)
        for peri_event_times in peri_event_times_list
    ]
    return np.mean(np.vstack(vector_peri_events), axis=0)


def sliding_overlap(vector, profile):
    """
    ...
    num_samples_after = (
        len(profile) - num_samples_before
    )  # number of samples to look forwards from center value
    profile_overlap = np.asarray([np.nan] * len(vector))  # initialize
    for idx in np.arange(num_samples_before, len(vector) - num_samples_after):
        vector_subset = vector[
            idx - num_samples_before : idx + num_samples_after
        ]
        if all(
            np.isfinite(vector_subset)
        ):  # can only calculate overlap with finite vectors
...
class SimilarOverlapPeriods:
    def __init__(
        ...
        self.external_time_series_lower_threshold = (
            external_time_series_lower_threshold
        )
        self.external_time_series_upper_threshold = (
            external_time_series_upper_threshold
        )
        ...
        self.valid_above_overlap_thresh_spans = (
            self._get_valid_above_overlap_thresh_spans()
        )
    ...
    def _get_average_profile(self):
        ...
        return mean_vector_around_event(
    ...
    def _get_ts_profile_overlap(self):
        ...
        return pd.Series(
            sliding_overlap(
                vector=self.time_series.values, profile=self.average_profile
            ),
    ...
    def _get_above_overlap_thresh_idxs(self):
        ...
        return np.where(self.ts_profile_overlap > self.overlap_threshold)[
            0
        ]  # where overlap exceeds threshold

    def _get_above_overlap_thresh_spans(self):
        return find_spans_increasing_list(self.above_overlap_thresh_idxs)[
            0
        ]  # spans where overlap exceeds threshold
    ...
    def _get_valid_above_overlap_thresh_spans(self):
        ...
        if self.valid_intervals is not None:  # if valid intervals passed
            ...
            valid_spans_bool = np.logical_and(
                event_times_in_intervals_bool(
                    spans_start_time, self.valid_intervals
                ),
                event_times_in_intervals_bool(
                    spans_end_time, self.valid_intervals
                ),
            )
            valid_above_overlap_thresh_spans = index_list_of_tuples(
                self.above_overlap_thresh_spans, valid_spans_bool
            )
        # Exclude spans with values outside valid range if indicated
        if self.external_time_series is not None:
            valid_above_overlap_thresh_spans_time = tuples_index_list(
                valid_above_overlap_thresh_spans,
                list(self.ts_profile_overlap.index),
            )  # times of spans where overlap threshold passed
            valid_above_overlap_thresh_spans_ext = [
                self.external_time_series.iloc[
                    event_times_in_intervals_bool(
                        self.external_time_series.index, [span]
                    )
                ].values
                for span in valid_above_overlap_thresh_spans_time
            ]  # external time series during spans where overlap threshold passed
            valid_spans_bool = []
            for span_ext in valid_above_overlap_thresh_spans_ext:
                if (
                    self.ignore_external_vector_nan
                ):  # if only want to consider finite values from external vector (e.g. only finite values of ppt)
                    span_ext = span_ext[np.isfinite(span_ext)]
                valid_spans_bool.append(
                    all(
                        np.logical_and(
                            span_ext
                            >= self.external_time_series_lower_threshold,
            ...
            valid_above_overlap_thresh_spans = index_list_of_tuples(
                valid_above_overlap_thresh_spans, valid_spans_bool
            )
    ...
    def _combine_close_spans(self):
        ...
        if self.combine_close_spans_threshold is not None:
            ...
            if (
                abs(
                    len(valid_above_overlap_thresh_spans_time)
                    * len(self.valid_above_overlap_thresh_spans)
                )
                > 0
            ):
                self.valid_above_overlap_thresh_spans = (
                    apply_merge_close_intervals(
    ...
    def _get_max_overlap_times(self):
        ...
        for span_start, span_end in self.valid_above_overlap_thresh_spans:
            span_overlap = self.ts_profile_overlap.iloc[
                span_start : span_end + 1
            ]
            max_overlap_times.append(
                span_overlap.index[np.argmax(span_overlap)]
            )
