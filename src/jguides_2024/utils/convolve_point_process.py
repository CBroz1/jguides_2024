...
def convolve_point_process(event_times, kernel_sd, fs=None, sample_times=None):
    ...
    if sample_times is None:
        sample_times = np.arange(
            np.min(event_times), np.max(event_times) + 1 / fs, 1 / fs
        )  # array with times at which to sample Gaussian
    # Sample Gaussian centered at each event
    with Pool(3) as p:
        gaussian_at_events_list = p.starmap(
            evaluate_gaussian,
            zip(
                [sample_times] * len(event_times),
                event_times,
                [kernel_sd] * len(event_times),
            ),
        )  # shape: events x samples
...
def convolve_point_process_efficient(
    event_times, kernel_sd, time_bins, error_tolerance=0.00001, verbose=False
):
    """
    ...
    mew = 0
    one_half_times = np.arange(
        fs, kernel_sd * 10, fs
    )  # one half of the times (excluding center)
    kernel_sample_times = np.concatenate(
        (-one_half_times[::-1], [mew], one_half_times)
    )
    kernel = evaluate_gaussian(
        sample_times=kernel_sample_times, mew=mew, kernel_sd=kernel_sd
    )
    ...
    data_element_size = (
        sys.getsizeof(kernel[0]) / 2
    )  # note that sys may overestimate size if used directly on array
    num_time_bins = len(time_bins) - 1
    arr_size = (
        data_element_size * len(event_times) * num_time_bins
    )  # size of one array element times number of spikes times number of time bins
    num_partitions = int(np.ceil(arr_size / (mem * 0.9)))
    partition_len = int(np.ceil(len(event_times) / num_partitions))
    partitioned_event_times = [
        event_times[idx : idx + partition_len]
        for idx in range(0, len(event_times), partition_len)
    ]
    if verbose:
        print(
            f"split data into {num_partitions} partitions so that processing respects memory"
        )

    # Smooth spike times in partitions
    arr_sum = np.zeros(
        (num_partitions, num_time_bins)
    )  # array for sum across each partition array
    for partition_idx, event_times_part in enumerate(
        partitioned_event_times
    ):  # for group of spike times
        # Histogram spikes
        event_counts, _ = np.histogram(
            event_times_part, time_bins
        )  # spike counts in sample bins
        ...
        for num_spikes in np.unique(
            event_counts[event_counts > 0]
        ):  # unique numbers of spikes found in bins
            ...
            for event_bin_idx in event_bin_idxs:
                ...
                end_idx_arr = np.min(
                    [event_bin_idx + kernel_center_idx + 1, np.shape(arr)[1]]
                )  # 1 accounts for slice
                ...
                if event_bin_idx + kernel_center_idx > final_arr_col_idx:
                    end_idx_kernel = len(kernel) - (
                        event_bin_idx + kernel_center_idx - final_arr_col_idx
                    )
                # Put kernel corresponding to current spike into array
                arr[row_counter, start_idx_arr:end_idx_arr] = (
                    kernel[start_idx_kernel:end_idx_kernel] * num_spikes
                )
                row_counter += 1
