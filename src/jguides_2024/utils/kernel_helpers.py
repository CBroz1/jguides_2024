...
class ExponentialKernel:
    ...
    def _get_kernel(self):
        ...
        kernel = sp.signal.exponential(
            params["kernel_num_samples"],
            params["kernel_center"],
            params["kernel_tau"],
            params["kernel_symmetric"],
        )
        kernel = np.concatenate(
            ([0] * params["kernel_offset"], kernel)
        )  # offset kernel with zeros
    ...
    def _get_kernel(self):
        ...
        if self.kernel_params["kernel_type"] == "exponential":
            return ExponentialKernel(
                self.kernel_params["kernel_num_samples"],
                self.kernel_params["kernel_tau"],
                kernel_center=0,
                kernel_symmetric=False,
                kernel_offset=1,
                density=True,
