...
class EventNamesMapDioStatescript(dj.Manual):
    ...
    def insert_defaults(self, **kwargs):
        for dio_event_name, statescript_event_name in {
            "HaightLeft_poke_center_SA_poke_center": "center_poke",
            ...
            "HaightRight_poke_left": "left_poke",
        }.items():
            self.insert1(
                {
                    "dio_event_name": dio_event_name,
...
class PumpTimes(ComputedBase):
    ...
    def make(self, key):
        if (PumpDiosComplete & key).fetch1(
            "dio_pumps_complete"
        ):  # if complete set of pump events available from DIO
            reward_times_source = "ProcessedDioEvents"
            pump_names, pump_times, pump_values = (
                eval(reward_times_source).Pumps() & key
            ).fetch1("dio_pump_names", "dio_pump_times", "dio_pump_values")
            ...
            environment = (TaskIdentification & key).fetch1(
                "task_environment"
            )  # get environment for this epoch

            # Only include pumps in the environment
            is_pump_env = np.asarray(
                [f"{environment}" in pump_name for pump_name in pump_names]
            )
            pump_names = pump_names[is_pump_env]
            pump_times = pump_times[is_pump_env]

            pump_names = np.asarray(
                [
                    pump_name.split(f"{environment}_pump_")[1].split("_")[0]
                    + "_well"
                    for pump_name in pump_names
                ]
            )  # get names of wells at which dio pump up event detected
        else:  # use statescript pump events
            reward_times_source = "ProcessedStatescriptEvents"
            pump_names, pump_times = (
                eval(reward_times_source).Pumps & key
            ).fetch1(
                "processed_statescript_pump_names",
                "processed_statescript_pump_times_ptp",
            )
            # Convert pump names to well names
            pump_names = np.asarray(
                [
                    f"{pump_name.split('rewarding_')[1]}_well"
                    for pump_name in pump_names
                ]
            )
        # Insert into table
...
class ContingencyEnvironmentColor(dj.Manual):
    ...
    def insert_defaults(self, **kwargs):
        task_environments = (
            ["HaightLeft"] * 2 + ["HaightRight"] * 2 + ["SA"] * 2
        )
        contingencies = ["centerAlternation", "handleAlternation"] * int(
            len(task_environments) / 2
        )
        colors = np.asarray(plt.cm.tab20.colors)[[0, 1, 6, 7, 10, 11]]
        for task_environment, contingency, color in zip(
            task_environments, contingencies, colors
        ):
            self.insert1(
                {
...
def get_contingency_task_environment_color(nwb_file_name, epoch):
    contingency, task_environment = (
        TaskIdentification & {"nwb_file_name": nwb_file_name, "epoch": epoch}
    ).fetch1("contingency", "task_environment")
    return (
        ContingencyEnvironmentColor
        & {"contingency": contingency, "task_environment": task_environment}
    ).fetch1("color")


def get_contingency_task_environment_colors(nwb_file_name, epoch_list):
    return [
        get_contingency_task_environment_color(nwb_file_name, epoch)
        for epoch in epoch_list
    ]


def populate_jguidera_task_event(
    key=None,
