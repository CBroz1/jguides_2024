...
from spyglass.common import Nwbfile, Session, AnalysisNwbfile
...
class DioEvents(ComputedBase):
    ...
    def make(self, key):
        ...
        nwbf_dios = nwbf.fields["processing"]["behavior"][
            "behavioral_events"
        ].fields["time_series"]
        dio_event_values_list, dio_event_times_list, dio_descriptions = (
        ...
        for dio_name, dios in nwbf_dios.items():  # for each DIO type
            ...
            dio_event_times_list.append(
                dio_event_times[valid_bool]
            )  # DIO event times
        ...
        dio_event_df = pd.DataFrame.from_dict(
            {
                ...
                "dio_int": [
                    convert_dio_description(x) for x in dio_descriptions
                ],
        ...
        # Store
        insert_analysis_table_entry(
            self, [dio_event_df], key, ["dio_events_object_id"]
        )
    ...
    def plot_dios(self, nwb_file_name, epoch):
        ...
        fig, axes = plt.subplots(
            len(df), 1, sharex=True, figsize=(15, 2 * len(df))
        )
        fig.tight_layout()
        for (_, df_row), ax in zip(df.iterrows(), axes):
            ax.plot(df_row["dio_event_times"], df_row["dio_event_values"], ".")
            format_ax(
                ax, title=f"{df_row['dio_name']} {df_row['dio_description']}"
            )

    def process_dio_events(
        self, nwb_file_name, epoch, diff_threshold, verbose=True
    ):
        """
        "Process" dio events:
        ...
        num_excluded_dio_events_dict = {
            dio_name: 0 for dio_name in dio_df["dio_name"]
        }  # track number of excluded dio events
        dio_poke_ints = df_filter_columns_contains(
            dio_df, "dio_name", "_poke"
        ).index  # dio pokes
        ...
        for dio_int in dio_poke_ints:
            ...
            valid_bool = not_small_diff_bool(
                df_subset.dio_event_times, diff_threshold=diff_threshold
            )
            new_dio_event_times, new_dio_event_values = return_n_empty_lists(
                2
            )  # initialize
            if len(df_subset.dio_event_times) > 0:  # if any dio events
                if (
                    len(df_subset.dio_event_times[valid_bool]) > 0
                ):  # if any valid dio events
                    new_dio_event_times = df_subset.dio_event_times[valid_bool]
                    new_dio_event_values = df_subset.dio_event_values[
                        valid_bool
                    ]
                    check_alternating_elements(new_dio_event_values, 0, 1)
                    num_excluded_dio_events_dict[
                        df_subset["dio_name"]
                    ] += np.sum(np.invert(valid_bool))
        ...
        df_pokes = df_filter_columns_contains(
            dio_df, "dio_name", "_poke"
        )  # dio pokes
        new_dio_event_times_dict = {
            df_pokes.index[-1]: df_pokes.iloc[-1].dio_event_times
        }  # initialize with "dio N"
        new_dio_event_values_dict = {
            df_pokes.index[-1]: df_pokes.iloc[-1].dio_event_values
        }  # initialize with "dio N"
        ...
        for dio_int in df_pokes.index[1::][::-1]:
            ...
            dio_event_values = (
                df_row.dio_event_values
            )  # event values for current dio
            dio_event_times = (
                df_row.dio_event_times
            )  # event times for current dio
            ...
            if (
                check_return_single_element(
                    dio_up_end_idxs - dio_up_idxs[: len(dio_up_end_idxs)]
                ).single_element
                != 1
            ):
                raise Exception(
            ...
            dio_up_periods = list(
                zip(
                    dio_event_times[dio_up_idxs],
                    dio_event_times[dio_up_end_idxs],
                )
            )
            ...
            invalid_bool = event_times_in_intervals_bool(
                df_pokes.loc[dio_int - 1].dio_event_times, dio_up_periods
            )
            valid_bool = np.invert(invalid_bool)
            new_dio_event_times = df_pokes.loc[dio_int - 1].dio_event_times[
                valid_bool
            ]
            new_dio_event_values = df_pokes.loc[dio_int - 1].dio_event_values[
                valid_bool
            ]
            check_alternating_elements(new_dio_event_values, 0, 1)
            ...
            num_excluded_dio_events_dict[
                df_pokes.loc[dio_int - 1]["dio_name"]
            ] += np.sum(
                invalid_bool
            )  # update excluded dio counter
            # Plot
            if verbose:
                fig, ax = plt.subplots(figsize=(15, 2))
                ax.plot(
                    dio_event_times, dio_event_values, label=f"dio {dio_int}"
                )
                label = None
                ...
                ax.plot(
                    dio_df.loc[dio_int - 1].dio_event_times,
                    dio_df.loc[dio_int - 1].dio_event_values + 1,
                    label=f"ORIGINAL dio {dio_int - 1}",
                )
                ax.plot(
                    new_dio_event_times,
                    new_dio_event_values + 1,
                    label=f"NEW dio {dio_int - 1}",
                )
        ...
        for (
            ...
            new_dio_event_times_dict.items()
        ):  # must use loop to replace using [row_indexer, col_indexer]
        ...
        dio_df["num_excluded_dio_events"] = [
            num_excluded_dio_events_dict[dio_name]
            for dio_name in dio_df["dio_name"]
        ]
...
class ProcessedDioEvents(ComputedBase):
    ...
    def make(self, key):
        ...
        if len(RunEpoch & key) == 0:
            print(
                f"Not populating ProcessedDioEvents for key {key} because no corresponding entry in RunEpoch"
            )
            return
        ...
        dio_df = DioEvents().process_dio_events(
            nwb_file_name, epoch, diff_threshold, verbose=True
        )
        ...
        for dio_name in dio_names:  # for each dio poke event
            ...
            dio_event_times = df_pop(
                dio_df, {"dio_name": dio_name}, "dio_event_times"
            )
        ...
        full_key = {
            **key,
            **{
                "num_excluded_dio_events": dio_df.set_index("dio_name")[
                    "num_excluded_dio_events"
                ].to_dict(),
        ...
        insert_analysis_table_entry(
            ...
            [all_dio_df_reset_index],
            full_key,
            ["processed_dio_events_object_id"],
        )
        ...
        dio_pokes_df = all_dio_df[
            all_dio_df["dio_event_names"].isin(poke_dio_names)
        ]
        ...
        if len(dio_pokes_downs_df) > 0:  # if down events
            ...
            first_up_time = (
                -1
            )  # value will not matter since no down pokes to filter
        ...
        if len(dio_pokes_first_ups_df) - len(dio_pokes_last_downs_df) not in [
            0,
            1,
        ]:
            raise Exception(
        ...
        insert1_print(
            self.Pokes,
            {
                **key,
                **{
                    "dio_poke_names": dio_pokes_df[
                        "dio_event_names"
                    ].to_numpy(),
                    "dio_poke_times": dio_pokes_df.index.to_numpy(),
                    "dio_poke_values": dio_pokes_df[
                        "dio_event_values"
                    ].to_numpy(),
                },
            },
        ...
        dio_pumps_df = all_dio_df[
            all_dio_df["dio_event_names"].isin(pump_dio_names)
        ]
        # Only consider dio down events that happen after first dio up event
        valid_idxs = []  # initialize valid idxs to empty list
        if (
            np.sum(dio_pumps_df["dio_event_values"] == 1) > 0
        ):  # if dio up events
        ...
        insert1_print(
            self.Pumps,
            {
                **key,
                **{
                    "dio_pump_names": dio_pumps_df[
                        "dio_event_names"
                    ].to_numpy(),
                    "dio_pump_times": dio_pumps_df.index.to_numpy(),
                    "dio_pump_values": dio_pumps_df[
                        "dio_event_values"
                    ].to_numpy(),
                },
            },
...
class PumpDiosComplete(ComputedBase):
    ...
    def make(self, key):
        ...
        subject_id = (
            Session() & {"nwb_file_name": key["nwb_file_name"]}
        ).fetch1("subject_id")
        task_environment, contingency = (TaskIdentification & key).fetch1(
            "task_environment", "contingency"
        )
        if any(
            [
                subject_id not in ["peanut", "J16"],
                np.logical_and(
                    subject_id == "fig",
                    int(key["nwb_file_name"].split(subject_id)[1].split("_")[0])
                    > 20211106,
                ),
                task_environment in ["HaightRight", "SleepBox", "HomeBox"],
                contingency == "handleAlternation",
            ]
        ):  # conditions for which dio pumps complete
            dio_pumps_complete = 1  # indicate that dio pumps complete

        # Insert into table
        insert1_print(
            self, {**key, **{"dio_pumps_complete": dio_pumps_complete}}
        )
