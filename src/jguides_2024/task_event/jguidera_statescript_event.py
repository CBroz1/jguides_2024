...
class StatescriptEvents(ComputedBase):
    ...
    def make(self, key):
        ...
        state_script_printouts = [
            ...
            if x[0] != "#"
        ]  # note that must first find lines with content to enable search for hash indicating comment
...
def get_ss_nwbf_dio_name_map():
    ...
    for environment in get_environments():  # for environments
        for ss_dio_name in ss_dio_names:  # for statescript dio names
            ss_nwbf_dio_name_map[(ss_dio_name, environment)] = (
                unpack_single_element(
                    [
                        x
                        for x in nwbf_dio_names
                        if environment in x
                        and ss_dio_name.split("_poke")[0].split("rewarding_")[
                            -1
                        ]
                        in x
                        and get_ss_trigger_type(ss_dio_name) in x
                    ]
                )
            )  # matching nwbf dio name
...
class ProcessedStatescriptEvents(ComputedBase):
    ...
    def make(self, key):
        ...
        def _get_ss_dio_times_as_ptp(
            epoch_all_dio_times_ss_trodes,
            ...
            epoch_all_dio_times_ss_ptp = (
                []
            )  # initialize list for a matched PTP times
            print(
                "Progress in matching Trodes times for DIO events to PTP times:"
            )
            ...
            for idx, dio_time_trodes in enumerate(
                epoch_all_dio_times_ss_trodes
            ):  # for each dio time from statescript file
                # Print progress of calculation
                if idx in np.arange(
                    0,
                    len(epoch_all_dio_times_ss_trodes),
                    np.round(len(epoch_all_dio_times_ss_trodes) / 10),
                ):
                    print(
                        f"{np.round(idx / len(epoch_all_dio_times_ss_trodes) * 100)}%"
                    )
                # Find PTP sample times corresponding to statescript DIO times
                dio_sample_count_trodes = (
                    dio_time_trodes * fs / 1000
                )  # convert Trodes time (in ms) to sample count
                matched_times_ptp = epoch_sample_times_ptp[
                    (epoch_sample_counts_trodes == dio_sample_count_trodes)
                ]
                ...
                if len(matched_times_ptp) > 0:
                    ...
                    matched_times_ptp_range = np.max(
                        matched_times_ptp
                    ) - np.min(matched_times_ptp)
                    ...
                    matched_time_ptp = np.median(
                        np.median(matched_times_ptp)
                    )  # define matched time as median of identified ptp times for this DIO event
                    epoch_all_dio_times_ss_ptp.append(matched_time_ptp)  # store
                    idx_has_nwbf_match, time_ptp_nwbf_match = copy.deepcopy(
                        idx
                    ), copy.deepcopy(
                        matched_time_ptp
                    )  # hold onto index and time
                # If no matching times in nwbf, extrapolate from closest DIO with nwbf match
                    ...
                    time_to_closest_match = (
                        dio_time_trodes
                        - epoch_all_dio_times_ss_trodes[idx_has_nwbf_match]
                    )
                    time_to_closest_match /= 1000  # convert to s
                    epoch_all_dio_times_ss_ptp.append(
                        time_to_closest_match + time_ptp_nwbf_match
                    )
            return np.asarray(epoch_all_dio_times_ss_ptp)

        def _process_statescript_events(
            ...
            idxs_rewarding_side = np.where(
                epoch_all_dio_events_ss == "rewarding_side"
            )[0]
            for (
                idx_rewarding_side
            ) in idxs_rewarding_side:  # for each instance of rewarding side
                side_well_name = [
                    event
                    for event in list(
                        epoch_all_dio_events_ss[:idx_rewarding_side]
                    )
                    if "_poke" in event
                ][-1].split("_poke")[0]
                ...
                if side_well_name not in valid_side_well_names:
                    raise Exception(
                        f"Side well name not in {valid_side_well_names}"
                    )
                rewarding_side_replacement = "rewarding_" + side_well_name
                print(
                    f"WARNING: replacing {epoch_all_dio_events_ss[idx_rewarding_side]} in statescript output with \
            ...
            idx_pump = np.asarray(
                [
                    idx
                    for idx, event_name in enumerate(epoch_all_dio_events_ss)
        ...
        def _check_dio_times_ss_breakoutboard(
            epoch_dio_times_ss_ptp,
            epoch_dio_up_times_ptp,
            max_expected_time_diff_dios,
            max_mean_expected_time_diff_dios,
            tolerate_error,
        ):
            # GOAL: Make sure distance between each ss event and closest dio on breakout board not "too much".
            times_dist = match_increasing_elements(
                epoch_dio_times_ss_ptp, epoch_dio_up_times_ptp
            )
            ...
            if any(times_dist > max_expected_time_diff_dios):
                ...
                insert_key = {
                    **key,
                    **{"dio_name_ss": dio_name_ss, "times_dist": times_dist},
                }
                ProcessedStatescriptEventsDioMismatch().insert1(
                    insert_key, skip_duplicates=True
                )
                ...
                if (
                    len(
                        ...
                        & {
                            k: insert_key[k]
                            for k in ProcessedStatescriptEventsDioMismatch.primary_key
                        }
                    )
                    == 1
                ):
                ...
                failed_check(
                    tolerate_error=tolerate_error_,
        ...
        def _plot_epoch_dios(
            ...
            fig, axes = plt.subplots(
                1, 2, figsize=(10, 2), gridspec_kw={"width_ratios": [3, 1]}
            )
            ...
            ax_temp.plot(
                ...
                color="black",
                alpha=0.5,
            )  # plot events from nwbf
            ax_temp.plot(
                epoch_dio_times_ss_ptp,
                [1] * len(epoch_dio_times_ss_ptp),
                "x",
                color="red",
            )  # plot events from statescript
            ...
            counts, _, _ = ax_temp.hist(
                times_dist
            )  # plot difference in DIO times
            ax_temp.plot(
                [0, 0], [0, np.max(counts)], color="red"
            )  # plot line at zero
            ax_temp.set_title("event time difference \n(statescript - nwbf)")

        def _get_analogous_nwbf_events(
            epoch_all_dio_times_ss_ptp,
            epoch_all_dio_events_ss,
            nwb_file_name,
            epoch,
            ...
            epoch_all_dio_events_bb_analogue = np.asarray(
                [""] * len(epoch_all_dio_events_ss)
            )  # use quotes b/c None cannot be stored as hdf5
            epoch_all_dio_events_bb_analogue_value = np.asarray(
                [np.nan] * len(epoch_all_dio_events_ss)
            )
            for idx, (ss_dio_time, ss_dio_name) in enumerate(
                zip(epoch_all_dio_times_ss_ptp, epoch_all_dio_events_ss)
            ):
                # Current understanding is "0 0" lines are dio downs for poke events only (not pump events)
                # POKES
                # UP events
                if "poke" in ss_dio_name:
                    nwbf_dio_name = ss_nwbf_dio_name_map[
                        (ss_dio_name, environment)
                    ]
                ...
                if "rewarding" in ss_dio_name:
                    epoch_all_dio_events_bb_analogue[idx] = (
                        ss_nwbf_dio_name_map[(ss_dio_name, environment)]
                    )
                    epoch_all_dio_events_bb_analogue_value[idx] = 1  # dio up
            return (
                epoch_all_dio_events_bb_analogue,
                epoch_all_dio_events_bb_analogue_value,
            )
        ...
        # *** Hard code ***
        max_expected_time_span = 0.0011  # max tolerated range of PTP times matched to Trodes DIO times from statescript
        max_expected_time_diff_dios = 0.018  # max tolerated distance between nwbf and statescript DIO times
        max_mean_expected_time_diff_dios = 0.005  # max tolerated mean distance between nwbf and statescript DIO times
        ...
        print(
            f"Matching statescript DIO event times to PTP times for {nwb_file_name} epoch {epoch}"
        )
        ...
        epoch_all_dio_times_ss_trodes = ss_events[
            "statescript_event_times_trodes"
        ]
        ...
        epoch_all_dio_times_ss_ptp = _get_ss_dio_times_as_ptp(
            ...
            epoch_sample_counts_trodes=epoch_timestamps_df[
                "trodes_sample_count"
            ].values,
        ...
        epoch_all_dio_times_ss_ptp, epoch_all_dio_events_ss = (
            _process_statescript_events(
                epoch_all_dio_events_ss,
                epoch_all_dio_times_ss_ptp,
                key["nwb_file_name"],
                key["epoch"],
        ...
        ss_dio_df = pd.DataFrame.from_dict(
            {
                "epoch_all_dio_times_ss_ptp": epoch_all_dio_times_ss_ptp,
                "epoch_all_dio_events_ss": epoch_all_dio_events_ss,
            }
        ).set_index("epoch_all_dio_times_ss_ptp")
        ss_dio_df.sort_index(inplace=True)  # sort events by time
        ss_dio_df.reset_index(
            inplace=True
        )  # get back time column so can unpack below
        ...
        dio_names_map = {
            k: v
            for k, v in get_nwbf_ss_dio_name_map().items()
            if environment in k
        }  # restrict to DIOs in current environment
        ...
        for dio_name_nwbf, dio_name_ss in dio_names_map.items():  # for each DIO
            ...
            epoch_dio_times_ss_ptp = epoch_all_dio_times_ss_ptp[
                epoch_all_dio_events_ss == dio_name_ss
            ]
            ...
            dio_df_subset = df_filter_columns(
                dio_df, {"dio_event_names": dio_name_nwbf}
            )  # restrict to current dio
            ...
            epoch_dio_up_times_ptp = df_filter_columns(
                dio_df_subset, {"dio_event_values": 1}
            ).index  # up events
            if len(epoch_dio_times_ss_ptp) > 0:  # if epoch DIOs in statescript
                ...
                _check_dio_times_ss_breakoutboard(
                ...
                _plot_epoch_dios(
                    ...
                    epoch_dio_up_times_ptp=epoch_dio_up_times_ptp,
                    epoch_dio_times_ss_ptp=epoch_dio_times_ss_ptp,
                    title_text=f"{nwb_file_name} ep{epoch}\n{dio_name_nwbf} (nwbf) \n{dio_name_ss} (statescript) \nepoch {epoch}",
                )
        ...
        (
            ...
            processed_statescript_event_values_bb_analogue,
        ) = _get_analogous_nwbf_events(
            epoch_all_dio_times_ss_ptp,
            epoch_all_dio_events_ss,
            nwb_file_name,
            epoch,
        ...
        insert_analysis_table_entry(
            self, [ss_event_df], key, ["processed_statescript_events_object_id"]
        )

        # Populate subtable for well poke events
        subtables_key = {k: key[k] for k in ["nwb_file_name", "epoch"]}
        poke_names, poke_times = zip(
            *[
                (event_name, event_time)
                for event_name, event_time in zip(
                    epoch_all_dio_events_ss, epoch_all_dio_times_ss_ptp
                )
        ...
        insert1_print(
            self.Pokes,
            {
                **subtables_key,
                **{
                    "processed_statescript_poke_names": np.asarray(poke_names),
                    "processed_statescript_poke_times_ptp": np.asarray(
                        poke_times
                    ),
                },
            },
        ...
        poke_names_consecutive_elements_removed, idxs = remove_repeat_elements(
            poke_names, keep_first=True
        )
        insert1_print(
            self.FirstPokes,
            {
                **subtables_key,
                **{
                    "processed_statescript_first_poke_names": poke_names_consecutive_elements_removed,
                    "processed_statescript_first_poke_times_ptp": np.asarray(
                        poke_times
                    )[idxs],
                },
            },
        ...
        insert1_print(
            self.LastPokes,
            {
                **subtables_key,
                **{
                    "processed_statescript_last_poke_names": poke_names_consecutive_elements_removed,
                    "processed_statescript_last_poke_times_ptp": np.asarray(
                        poke_times
                    )[idxs],
                },
            },
        ...
        insert1_print(
            self.Pumps,
            {
                **subtables_key,
                **{
                    "processed_statescript_pump_names": np.asarray(pump_names),
                    "processed_statescript_pump_times_ptp": np.asarray(
                        pump_times
                    ),
                },
            },
...
def get_nwbf_ss_dio_name_map():
    ...
    # Pokes
    dio_ss_names_map.update(
        {f"HaightLeft_poke_{x}_SA_poke_{x}": f"{x}_poke" for x in well_names}
    )
    dio_ss_names_map.update(
        {f"HaightRight_poke_{x}": f"{x}_poke" for x in well_names}
    )
    # Pumps
    dio_ss_names_map.update(
        {
            f"HaightLeft_pump_{x}_SA_pump_{x}": f"rewarding_{x}"
            for x in well_names
        }
    )
    dio_ss_names_map.update(
        {f"HaightRight_pump_{x}": f"rewarding_{x}" for x in well_names}
    )
    return dio_ss_names_map


def populate_jguidera_statescript_event(
    key=None,
