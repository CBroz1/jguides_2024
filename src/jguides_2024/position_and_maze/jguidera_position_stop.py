...
class StopLikeWellArrival(ComputedBase):
    ...
    def make(self, key):
        ...
        ).fetch1_dataframe()["head_speed"]
        da_trial_intervals, _ = _get_da_trials_info(
            key
        )  # departure to arrival time_and_trials

        similar_overlap_periods_obj = SimilarOverlapPeriods(
            time_series=speed,
            event_times=well_event_times,
            forward_shift=float(
                params_entry[f"{well_event_name}_forward_shift"]
            ),
            backward_shift=float(
                params_entry[f"{well_event_name}_backward_shift"]
            ),
            overlap_threshold=float(
                params_entry["speed_profile_overlap_threshold"]
            ),
            valid_intervals=da_trial_intervals,
            external_time_series=(Ppt & key).ppt_all_time(),
            external_time_series_lower_threshold=float(
                params_entry["ppt_lower_threshold"]
            ),
            external_time_series_upper_threshold=float(
                params_entry["ppt_upper_threshold"]
            ),
            combine_close_spans_threshold=float(
                params_entry["combine_spans_within"]
            ),
            ignore_external_vector_nan=True,
        )  # ignore nans in ppt

        key.update(
            {
                "position_stop_times": np.asarray(
                    similar_overlap_periods_obj.max_overlap_times
                ),
    ...
    def plot_overlap_at_well_arrivals(self, well_event_name):
        ...
        nwb_file_name, epoch, well_arrival_times = self.fetch1(
            "nwb_file_name", "epoch", "well_arrival_times"
        )
        speed_profile_overlap = self.fetch1_dataframe()
        fig, ax = plt.subplots(figsize=(3, 3))
        well_arrival_idxs = np.searchsorted(
            speed_profile_overlap.index, well_arrival_times
        )  # indices in speed profile close to well events
        counts = ax.hist(
            speed_profile_overlap.iloc[well_arrival_idxs], color="gray"
        )[
            0
        ]  # overlap between speed and average speed profile near well events
        speed_profile_overlap_threshold = float(
            (StopLikeWellArrivalParams & self.fetch1()).fetch1(
                "speed_profile_overlap_threshold"
            )
        )
        plot_spanning_line(
            counts, speed_profile_overlap_threshold, span_axis="y", ax=ax
        )
        format_ax(
            ax,
            title=f"{nwb_file_name}\nep{epoch}",
            ylabel="count",
    ...
    def plot_average_speed_profile(self):
        ...
        (well_arrival_backward_shift, well_arrival_forward_shift) = (
            StopLikeWellArrivalParams
            & {
                "stop_like_well_arrival_param_name": stop_like_well_arrival_param_name
            }
        ).fetch1("well_arrival_backward_shift", "well_arrival_forward_shift")
        well_arrival_backward_shift = float(well_arrival_backward_shift)
        well_arrival_forward_shift = float(well_arrival_forward_shift)
        fig, ax = plt.subplots(figsize=(3, 3))
        rel_time = np.linspace(
            float(well_arrival_backward_shift),
            float(well_arrival_forward_shift),
            len(average_speed_profile),
        )  # time relative to position_and_maze stop
    ...
    def plot_stop_periods(self, well_event_name, ax=None, plot_legend=True):
        ...
        key = {
            ...
            if k in IntervalPositionInfoRelabel.RelabelEntries.primary_key
        }
        pos_df = (
            IntervalPositionInfoRelabel.RelabelEntries & key
        ).fetch1_dataframe()  # position_and_maze data
        epoch_start_time = pos_df["head_speed"].index[0]  # epoch start time
        t = (
            pos_df["head_speed"].index - epoch_start_time
        ...
        for idx, (interval, path_name) in enumerate(
            zip(trial_intervals, trial_path_names)
        ):
            label = None
            if idx == 0:
        ...
        speed_profile_overlap = self.fetch1_dataframe()
        ax.plot(
            speed_profile_overlap.index - epoch_start_time,
            speed_profile_overlap["speed_profile_overlap"] * 100,
            alpha=0.5,
        ...
        ax.plot(
            t[above_overlap_thresh_idxs],
            [60] * len(above_overlap_thresh_idxs),
            "o",
            label="above overlap thresh samples",
        )
        # Spans of above threshold samples
        for idx, (span_start, span_end) in enumerate(
            table_entry["above_overlap_thresh_spans"]
        ):
            label = None
            if idx == 0:
                label = "above overlap thresh spans"
            ax.plot(
                [t[span_start], t[span_end]],
                [70] * 2,
                "-o",
                color="gray",
                label=label,
        ...
        for idx, (span_start, span_end) in enumerate(
            table_entry["valid_above_overlap_thresh_spans"]
        ):
            label = None
            if idx == 0:
        ...
        ax.scatter(
            position_stop_times - epoch_start_time,
            [80] * len(position_stop_times),
            color="red",
            s=50,
            label="position_and_maze stops",
        )
        # Legend
        if plot_legend:
            ax.legend()
