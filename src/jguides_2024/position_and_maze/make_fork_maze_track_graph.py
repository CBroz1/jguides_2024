...
def simulate_fork_maze(arm_length=50, connector_length=20):
    ...
    node_positions_dict = {
        "center_well": {"x": connector_length, "y": arm_length * 2},
        "center_maze": {"x": connector_length, "y": arm_length},
        "right_corner": {"x": connector_length * 2, "y": arm_length},
        "right_well": {"x": connector_length * 2, "y": arm_length * 2},
        "left_corner": {"x": 0, "y": arm_length},
        "left_well": {"x": 0, "y": arm_length * 2},
        "handle_well": {"x": connector_length, "y": 0},
    }
...
def make_fork_maze_track_graph(node_positions_dict):
    ...
    node_positions = tuple(
        [
            (
                node_positions_dict[node_name]["x"],
                node_positions_dict[node_name]["y"],
            )
    ...
    # Define edges
    edge_list = [(0, 1), (6, 1), (1, 2), (2, 3), (1, 4), (4, 5)]
...
def make_track_graph(node_names, node_positions, edge_list):
    ...
    track_graph.add_nodes_from(
        [
            (node_num, {"node_name": node_name, "pos": node_pos})
            for node_num, (node_pos, node_name) in enumerate(
                zip(node_positions, node_names)
            )
        ]
    )  # add nodes to graph
        ...
        [
            (
                ...
                edge[1],
                {
                    ...
                    "edge_name": tuple(
                        [
                            track_graph.nodes[edge[i]]["node_name"]
                            for i in [0, 1]
                        ]
                    ),
                },
