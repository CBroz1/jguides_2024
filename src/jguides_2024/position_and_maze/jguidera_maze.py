...
class MazePathWell:
    ...
    @staticmethod
    def get_rewarded_path_name_tuples(
        nwb_file_name=None, epoch=None, contingency=None
    ):

        # Check inputs (must pass either nwb_file_name and epoch, OR contingency
        if (nwb_file_name is not None and epoch is not None) + (
    ...
    def get_well_names(
        ...
        epoch=None,
        contingency=None,
        rewarded_wells=False,
        well_order=None,
    ):
        # Get well names for fork maze

        # Check inputs
        if rewarded_wells and (
            (nwb_file_name is not None and epoch is not None)
            + (contingency is not None)
            != 1
        ):
            raise Exception(
        ...
        if rewarded_wells:
            _, destination_wells = zip(
                *cls.get_rewarded_path_name_tuples(
        ...
        if organize_by_same_turn:
            return np.concatenate(
                cls.get_same_turn_path_names(
                    nwb_file_name, epoch, contingency, rewarded_paths=True
                )
            )
    ...
    @classmethod
    def get_rewarded_paths_turn_map(
        cls, nwb_file_name=None, epoch=None, contingency=None
    ):
    ...
    def get_same_turn_path_names_across_epochs(
        ...
        path_names = np.concatenate(
            [
                cls.get_same_turn_path_names(
                    nwb_file_name, epoch, rewarded_paths=rewarded_paths
                )
                for epoch in epochs
            ],
            dtype=object,
    ...
    def get_path_with_home_well_at_position(
        ...
        path_names = {
            home_well_name: [
                ...
                if path_name.split(split_char)[position] == home_well_name
            ]
    ...
    @classmethod
    def get_outbound_path_names(
        cls, nwb_file_name=None, epoch=None, contingency=None, as_dict=False
    ):
        position = 0
        return cls.get_path_with_home_well_at_position(
            position, nwb_file_name, epoch, contingency, as_dict=as_dict
        )

    @classmethod
    def get_inbound_path_names(
        cls, nwb_file_name=None, epoch=None, contingency=None, as_dict=False
    ):
        position = 1
        return cls.get_path_with_home_well_at_position(
    ...
    @classmethod
    def split_non_low_speed_low_speed_trial_path_name(
        cls, path_name, trial_text=None
    ):
    ...
    def low_speed_trial_text(text_type=None):
        text_map = {
            "low_speed": "low_speed_trial",
            "non_low_speed": "non_low_speed_trial",
        }
        if text_type is not None:
            return text_map[text_type]
        return list(text_map.values())
    ...
    def correct_incorrect_trial_text(text_type=None, previous_trial=False):
        ...
        text_map = {
            "correct": f"{previous_trial_text}correct_trial",
            "incorrect": f"{previous_trial_text}incorrect_trial",
        }
        if text_type is not None:
            return text_map[text_type]
        return list(text_map.values())
    ...
    def get_correct_incorrect_trial_text(
        cls, path_name, correct_incorrect_text=None, previous_trial=False
    ):
        ...
        check_membership(
            [correct_incorrect_text],
            cls.correct_incorrect_trial_text(previous_trial=previous_trial),
        )
    ...
    def _access_get_path_fn(cls, name, params):
        ...
        if name in [
            ...
            "inbound_even_odd_correct_stay_trials",
            "outbound_even_odd_correct_stay_trials",
            ...
            "same_path_outbound_correct_incorrect_trials",
        ...
        # If want even vs. odd stay correct trials, add stay to path names, then correct, then even and odd
            ...
            return [
                (
                    cls.get_even_odd_trial_name(
                        cls.get_correct_incorrect_trial_text(
                            cls.get_stay_leave_trial_path_name(x1, stay_text),
                            correct_text,
                        ),
                        even_odd_text=t1,
        ...
        # If want correct/incorrect trials, add correct and incorrect to path name
            ...
            [
                ...
                for x in [
                    ...
                    "correct_correct_stay_trials",
                    "incorrect_incorrect_stay_trials",
                ]
            ]
        ):
            ...
            if "correct_correct" in name:
                t1 = t2 = cls.correct_incorrect_trial_text(
                    "correct", previous_trial
                )
            elif "incorrect_incorrect" in name:
                t1 = t2 = cls.correct_incorrect_trial_text(
                    "incorrect", previous_trial
                )
            ...
            return [
                (
                    cls.get_correct_incorrect_trial_text(
                        stay_trials_fn(x1, trial_text=stay_text),
                        t1,
                        previous_trial,
                    ),
                    cls.get_correct_incorrect_trial_text(
                        stay_trials_fn(x2, trial_text=stay_text),
                        t2,
                        previous_trial,
                    ),
                )
        ...
        # If want
            ...
            return [
                (
                    ...
                    cls.get_low_speed_trial_label_name(x2, t2),
                )
                for x1, x2 in path_name_pairs
                for t1, t2 in suffix_sets
            ]
    ...
    def get_well_name_pair_types_map(
        ...
        well_names = cls.get_well_names(
            nwb_file_name, epoch, contingency, rewarded_wells=rewarded_wells
        )
        return {
    ...
    def get_end_well_name_pair_types_map(
        ...
        for epoch in epochs:
            well_names = cls.get_well_names(
                nwb_file_name, epoch, rewarded_wells=rewarded_wells
            )
