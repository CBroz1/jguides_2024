...
def linear_positions_to_edge_idxs(linear_positions, edge_linear_position):
    ...
    edge_idxs = (
        []
    )  # initialize list for edges identified using edge_linear_position
    for pos in linear_positions:
        if np.isnan(pos):  # if pos is nan, set edge number to nan
            edge_idxs.append(np.nan)
        else:
            z = np.where(np.product(edge_linear_position - pos, axis=1) <= 0)[
                0
            ]  # edge number matches
            if (
                len(z) != 1
            ):  # check that one edge found for this position_and_maze
                raise Exception(
                    f"Should have found exactly one edge for position_and_maze but found {len(z)}"
                )
...
def linear_positions_to_edge_names(
    linear_positions, edge_linear_position, edge_names
):
    linear_position_edge_idxs = linear_positions_to_edge_idxs(
        linear_positions, edge_linear_position
    )
...
def get_fractional_edge_position(
    linear_position, linear_position_edge_idxs, edge_linear_position
):
    pos_edge_lengths = np.ndarray.flatten(
        np.diff(edge_linear_position)[linear_position_edge_idxs]
    )  # array with edge lengths for linear position_and_maze sample edges
    edge_startpoints = edge_linear_position[:, 0][
        linear_position_edge_idxs
    ]  # linear position_and_maze of end node for edges of linear position_and_maze samples
    fractional_edge_position = (
        np.asarray(linear_position) - edge_startpoints
    ) / pos_edge_lengths
    if not all(
        fractional_edge_position[np.invert(np.isnan(fractional_edge_position))]
        >= 0
...
class IntervalPositionInfoRelabelParams(SecKeyParamsBase):
    ...
    def insert_single_epoch(
        self,
        nwb_file_name,
        epoch,
        position_info_param_name=None,
        tolerate_no_entry=False,
    ):
        interval_list_name = EpochIntervalListName().get_interval_list_name(
            nwb_file_name, epoch, tolerate_no_entry
        )
        # If no epoch found and tolerating this, print message and exit
        if interval_list_name is None:
            print(
                f"Could not populate IntervalLinearizedPositionRelabelParams for {nwb_file_name}, epoch {epoch} "
    ...
    def insert_defaults(self, **kwargs):
        ...
        key_ = {
            ...
            if k in ["nwb_file_name", "epoch"]
        }
        if len(key_) > 0:
            keys = [key_]
        else:
            keys = [
                {**k, **{"position_info_param_name": position_info_param_name}}
                for k in get_jguidera_nwbf_epoch_keys()
                for position_info_param_name in get_valid_position_info_param_names()
            ]
...
class IntervalLinearizedPositionRelabelParams(SecKeyParamsBase):
    ...
    def insert_single_epoch(
        ...
        key.update(
            {
                "track_graph_name": (IntervalLinearizedPosition & key).fetch1(
                    "track_graph_name"
                )
            }
        )
        ...
        key.update(
            {
                self.meta_param_name(): self._make_param_name(
                    {
                        "linearization_param_name": key[
    ...
    def cleanup(self, nwb_file_name):
        ...
        epochs, param_names = (self & key).fetch(
            "epoch", "interval_linearized_position_relabel_param_name"
        )
        for position_info_param_name in ["default", "default_decoding"]:
            for epoch in epochs:
                ...
                correct_interval_list_name = (
                    EpochIntervalListName & key
                ).fetch1("interval_list_name")
                entries = self & key
                interval_linearized_position_relabel_param_names = (
                    entries.fetch(
                ...
                for (
                    param_name
                ) in interval_linearized_position_relabel_param_names:
                    if correct_interval_list_name not in param_name:
                        ...
                        (
                            ...
                            & {
                                **key,
                                **{
                                    "interval_linearized_position_relabel_param_name": param_name
                                },
                            }
                        ).delete()
...
class IntervalLinearizedPositionRelabel(ComputedBase):
    ...
    class RelabelEntries(dj.Part):
        ...
        def fetch1_dataframe(self):
            return fetch1_dataframe(self, "linearized_position").set_index(
                "time"
            )
    ...
    def make(self, key):
        relabel_params = (
            IntervalLinearizedPositionRelabelParams & key
        ).fetch1()
...
class IntervalLinearizedPositionRescaled(ComputedBase):
    ...
    def make(self, key, verbose=False):
        ...
        universal_track_graph_name = (
            TrackGraphUniversalTrackGraphMap & key
        ).fetch1("universal_track_graph_name")
        universal_track_graph_parameters = (
            AnnotatedUniversalTrackGraph
        ...
        linear_position_df = (
            IntervalLinearizedPositionRelabel.RelabelEntries() & key
        ).fetch1_dataframe()
        edge_linear_position = np.asarray(
            track_graph_parameters["edge_linear_position"]
        )
        edge_idxs = linear_positions_to_edge_idxs(
            linear_position_df["linear_position"], edge_linear_position
        )  # get indices of edges to which positions belong
        valid_pos_bool = np.isfinite(linear_position_df["linear_position"])
        track_segment_ids = linear_position_df[
            "track_segment_id"
        ].to_numpy()  # unpack track segment IDs
        if not all(
        ...
        if not all(
            np.ndarray.flatten(
                np.asarray(universal_track_graph_parameters["edge_names"])
                == np.asarray(track_graph_parameters["edge_names"])
            )
        ):
            raise Exception(
        ...
        fractional_edge_position = get_fractional_edge_position(
        ...
        universal_edge_linear_position = np.asarray(
            universal_track_graph_parameters["edge_linear_position"]
        )
        universal_pos_edge_lengths = np.ndarray.flatten(
            np.diff(universal_edge_linear_position)[track_segment_ids]
        )
        universal_edge_startpoints = universal_edge_linear_position[:, 0][
            track_segment_ids
        ]
        linear_position_rescaled = pd.Series(
            universal_edge_startpoints
            + universal_pos_edge_lengths * fractional_edge_position,
        ...
        edge_names[finite_bool] = np.asarray(
            universal_track_graph_parameters["edge_names"]
        )[
            [
                np.where(
                    np.prod(
                ...
                for pos in linear_position_rescaled[finite_bool]
            ]
        ]  # match positions to nondirectional edges
        ...
        linear_position_rescaled_diff = pd.Series(
            np.diff(linear_position_rescaled.values),
            index=vector_midpoints(linear_position_rescaled.index),
        )  # linear position_and_maze difference between samples
        # Find periods where same edge consecutively ("bouts") and in each find direction of travel along edge
        _, same_edge_bout_start_idxs = remove_repeat_elements(
            edge_names
        )  # bouts of same edge
        same_edge_bout_end_idxs = np.append(
            same_edge_bout_start_idxs[1:] - 1, len(edge_names) - 1
        )  # include last pos sample idx
        d_linear_position_rescaled = np.concatenate(
            [
                (
                    np.interp(
                        x=linear_position_rescaled.index[idx_1 : idx_2 + 1],
                ...
                for idx_1, idx_2 in list(
                    zip(same_edge_bout_start_idxs, same_edge_bout_end_idxs)
                )
            ]
        )  # define direction
        # Define directional edge
        directional_edge_names = np.asarray(
            [None] * len(linear_position_rescaled)
        ).astype(
            "object"
        )  # array for directional edges at positions
        directional_edge_names[d_linear_position_rescaled > 0] = edge_names[
            d_linear_position_rescaled > 0
        ]
        directional_edge_names[d_linear_position_rescaled < 0] = [
            flip_nodes_edge_name(edge_name)
            for edge_name in edge_names[d_linear_position_rescaled < 0]
        ]
        # Define percent directional edge traversed
        edge_linear_position_dict = {
            k: v
            for k, v in zip(
        ...
        percent_directional_edge_traversed = return_constant_vector(
            np.nan, len(linear_position_rescaled)
        )
        valid_directional_edge_names = np.unique(
            [
                edge_name
        ...
        for (
            directional_edge_name
        ) in (
            valid_directional_edge_names
        ):  # assess percent edge traversed for each edge type
            edge_name_bool = (
                directional_edge_names == directional_edge_name
            )  # where this edge type occurs
            linear_position_subset = linear_position_rescaled[
                edge_name_bool
            ]  # positions on edge
            directional_edge_start_linear_position = edge_linear_position_dict[
                directional_edge_name
            ][
                0
            ]  # position_and_maze of edge start
            edge_length = abs(
                np.diff(edge_linear_position_dict[directional_edge_name])
            )  # edge length
            percent_directional_edge_traversed[edge_name_bool] = (
                abs(
                    linear_position_subset
                    - directional_edge_start_linear_position
                )
                / edge_length
            )
        ...
        if verbose:
            ...
            ax.plot(
                linear_position_rescaled,
                ".-",
                color="orange",
                label="rescaled linear position_and_maze",
            )
            for bool_temp, color_temp in zip(
                [
                    (linear_position_rescaled_diff <= 0),
                    (linear_position_rescaled_diff > 0),
                ],
                ["red", "blue"],
            ):
                ax.scatter(
                    linear_position_rescaled_diff.index[bool_temp],
                    linear_position_rescaled_diff[bool_temp],
                    s=20,
                    color=color_temp,
                    alpha=0.5,
                    label="difference, linear position_and_maze rescaled",
                )
            for bool_temp, color_temp in zip(
                [
                    (d_linear_position_rescaled <= 0),
                ...
                ax.scatter(
                    d_linear_position_rescaled.index[bool_temp],
                    d_linear_position_rescaled[bool_temp],
                    s=20,
                    color=color_temp,
                    alpha=0.9,
                    label="difference, INTERPOLATED linear position_and_maze rescaled",
                )

                plot_edge_linear_position(
                    edge_linear_position=universal_track_graph_parameters[
                        "edge_linear_position"
                    ],
                    x_extent=[
                        linear_position_rescaled.index[0],
        ...
        key.update(
            {
                ...
                for k in ["interval_list_name", "linearization_param_name"]
            }
        )  # additional attributes
        insert_analysis_table_entry(
            self,
            nwb_objects=[linear_position_rescaled_df],
            key=key,
            nwb_object_names=[
                "interval_linearized_position_rescaled_object_id"
            ],
        )
...
def digitize_linear_position_rescaled_wrapper(
    nwb_file_name, epoch, new_index, bin_width_pos=4
):
    edge_linear_position = (
        AnnotatedUniversalTrackGraph()
        & {
            "universal_track_graph_name": get_universal_track_graph_name(
                nwb_file_name, epoch
            )
        }
    ).fetch("edge_linear_position")
    bin_edges = make_bin_edges(
        x=[
            np.min(np.min(edge_linear_position)),
    ...
    ).fetch1_dataframe()["linear_position_rescaled"]
    return digitize_indexed_variable(
        indexed_variable=lin_pos_original,
        bin_edges=bin_edges,
        new_index=new_index,
    )
...
def populate_jguidera_position(key=None, tolerate_error=False):
    from jguides_2024.position_and_maze.populate_position_tables import (
