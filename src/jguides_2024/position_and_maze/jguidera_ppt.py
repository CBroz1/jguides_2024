...
from jguides_2024.datajoint_nwb_utils.trials_container_helpers import (
    trials_container_default_time_vector,
)
...
class PptParams(SecKeyParamsBase):
    ...
    def insert_defaults(self, **kwargs):
        ...
        key.update(
            {
                "interval_linearized_position_relabel_param_name": interval_linearized_position_relabel_param_name,
                "environment_track_graph_name_map": get_default_environment_track_graph_name_map(),
                "ppt_param_name": ppt_param_name,
            }
        )  # must update to make param name
        self.insert1(key)
...
class PptSel(SelBase):
    ...
    def insert_defaults(self, **kwargs):
        ...
        for key in (
            DioWellDATrials * IntervalLinearizedPositionRescaled * PptParams
        ).fetch("KEY"):
            super().insert1(
                intersect_tables([RunEpoch, PptParams], key).fetch1("KEY")
            )
...
class Ppt(ComputedBase):
    ...
    def make(self, key):
        ...
        environment_track_graph_name_map = upstream_key.pop(
            "environment_track_graph_name_map"
        )
        upstream_key["track_graph_name"] = environment_track_graph_name_map[
            TaskIdentification.get_environment(
        ...
        universal_track_graph_name = (
            TrackGraphUniversalTrackGraphMap & upstream_key
        ...
        # Get pos info
        pos_df = (
            IntervalLinearizedPositionRescaled & upstream_key
        ).fetch1_dataframe()
        directional_edge_names_pos_map = {
            k: v
            for k, v in list(
                zip(
                    *(
        ...
        for trial_start_time, trial_end_time, path_name in zip_df_columns(
            trials_df, ["trial_start_times", "trial_end_times", "path_names"]
        ):  # path names for da trials

            pos_trial_df = df_filter_index(
                pos_df, [[trial_start_time, trial_end_time]]
            )  # pos during trial
            ...
            for pos, edge_name in zip_df_columns(
                ...
                path_fraction_map_entry = df_filter_columns(
                    path_fraction_map,
                    {"path_names": path_name, "path_edge_names": edge_name},
                )
                ...
                if len(path_fraction_map_entry) > 1:
                    ...
                    len(path_fraction_map_entry) == 0
                ):  # occurs if edge not on path
                    ppt_list.append(
                        np.nan
                    )  # in this case set trajectory phase to nan

                elif len(path_fraction_map_entry) == 1:
                    (
                        _,
                        path_edge_name,
                        path_directional_edge_name,
                        path_fraction,
                    ) = df_filter1_columns(
                        path_fraction_map,
                        {"path_names": path_name, "path_edge_names": edge_name},
                    ...
                    directional_edge_pos = directional_edge_names_pos_map[
                        path_directional_edge_name
                    ]
                    check_in_range(
                        pos, directional_edge_pos
                    )  # check that position_and_maze on directional edge

                    # Find percent directional edge traversed
                    percent_directional_edge_traversed = abs(
                        (pos - directional_edge_pos[0])
                        / np.diff(directional_edge_pos)[0]
                    )

                    # Convert to fraction of path traversed
                    mult_factor, shift_factor = find_scale_factors(
                        x=path_fraction, y=ppt_range
                    )
                    ppt_list.append(
                        percent_directional_edge_traversed * mult_factor
                        + shift_factor
                    )
        ...
        ppt_df = pd.DataFrame.from_dict(
            {
                "trials_time": trials_time_list,
    ...
    def ppt_df_all_time(self, new_index=None):
        ...
        time_vector = trials_container_default_time_vector(
            nwb_file_name, [epoch], new_index
        )
        ...
        for epoch_trial_number, df_row in ppt_df.iterrows():  # trials
            ...
            idxs, ppt_trial_t = event_times_in_intervals(
                time_vector, [[trial_t[0], trial_t[-1]]]
            )
        ...
        return pd.DataFrame.from_dict(
            {
                ...
                "epoch_trial_number": epoch_trial_numbers,
            }
        ).set_index("time")

    def digitized_ppt_df_all_time(
        self, new_index=None, bin_width=None, bin_edges=None, verbose=False
    ):
        # Check inputs
        ...
        ppt_df["digitized_ppt"] = digitize_indexed_variable(
            indexed_variable=ppt_df["ppt"], bin_edges=bin_edges, verbose=verbose
        )
    ...
    def ppt_df_all_trials(self):
        ...
        trials_paths = np.concatenate(
            [
                [path_name] * len(trial_ppt)
                for trial_ppt, path_name in zip_df_columns(
                    ppt_df, ["trials_ppt", "trials_path_name"]
                )
            ]
        )
        return pd.DataFrame.from_dict(
            {
                "all_trials_ppt": trials_ppt,
                "all_trials_paths": trials_paths,
                "all_trials_time": np.concatenate(ppt_df["trials_time"]),
            }
        ).set_index("all_trials_time")

    def trial_intervals(self):
        ppt_df = self.fetch1_dataframe()
        return [
            (trial_time[0], trial_time[-1])
            for trial_time in ppt_df["trials_time"]
        ]
    ...
    def upsample_trials(self, upsample_fs):
        ...
        ppt_df = self.fetch1_dataframe()
        upsample_trials_time, upsample_trials_ppt, upsample_path_names = (
            return_n_empty_lists(3)
        )
        for trial_time, trial_ppt, trial_path_name in zip_df_columns(
            ppt_df, ["trials_time", "trials_ppt", "trials_path_name"]
        ):
            new_trial_time = np.arange(
                trial_time[0], trial_time[-1], upsample_t_bin_width
            )
        ...
        return pd.DataFrame.from_dict(
            {
                "time": upsample_trials_time,
                "ppt": upsample_trials_ppt,
                "path_name": upsample_path_names,
            }
        ).set_index("time")
    ...
    def get_ppt_trials(self, valid_ppt_intervals, verbose=False):
        ...
        for trial_start_epoch_trial_number, df_row in ppt_df.iterrows():
            ...
            for valid_ppt_interval in valid_ppt_intervals:
                ...
                valid_ppt_idxs = np.where(
                    in_intervals_bool(trial_ppt.values, [valid_ppt_interval])
                )[0]
                valid_ppt_idx_spans = find_spans_increasing_list(
                    valid_ppt_idxs
                )[0]
                ...
                for span_start_idx, span_end_idx in valid_ppt_idx_spans:
                    ...
                    start_time, end_time = (
                        trial_ppt.index[span_start_idx],
                        trial_ppt.index[span_end_idx],
                    )
                    # Unpack valid ppt interval bounds
                    valid_ppt_interval_start, valid_ppt_interval_end = (
                        valid_ppt_interval
                    )
                    ...
                    if span_start_idx != 0:
                        ...
                        if (
                            not np.isnan(previous_ppt)
                            and previous_ppt < valid_ppt_interval_start
                        ):
        ...
        if verbose:
            ...
            # Plot ppt
            ppt_all_time = pd.Series(
                np.concatenate(ppt_df.trials_ppt),
                index=np.concatenate(ppt_df.trials_time),
            )
            ax.plot(ppt_all_time, "o", color="gray")
            valid_time_intervals = list(
                zip(
                    trials_map["trial_start_times"],
                    trials_map["trial_end_times"],
                )
            )
            valid_bool = in_intervals_bool(
                ppt_all_time.index, valid_time_intervals
            )
            ax.plot(ppt_all_time[valid_bool], "x", color="green")
            # Plot valid ppt intervals
            xlims = [np.min(ppt_all_time.index), np.max(ppt_all_time.index)]
            for valid_ppt_interval in valid_ppt_intervals:
    ...
    @classmethod
    def get_ppt_bin_edges(cls, ppt_bin_width=0.1):
        if ppt_bin_width < 0 or ppt_bin_width > 1:
            raise Exception(
                f"ppt_bin_width must be on [0, 1] but is {ppt_bin_width}"
            )
    ...
    def get_first_crossed_junction_df(self, verbose=False):
        ...
        ppt_df = df_filter_columns_isin(
            ppt_df, {"trials_path_name": valid_path_names}
        )
        ...
        for epoch_trial_number, trials_ppt, trials_time in zip(
            ppt_df.trial_start_epoch_trial_numbers,
            ppt_df.trials_ppt,
            ppt_df.trials_time,
        ):
            for junction_num, junction_fraction in enumerate(
                junction_fractions
            ):
                ...
                time_estimate = np.interp(
                    junction_fraction,
                    trials_ppt[crossed_idx - 1 : crossed_idx],
        ...
        first_crossed_junction_df = df_from_data_list(
            data_list,
            ["epoch_trial_number", "junction_number", "time_estimate", "ppt"],
        ).set_index("epoch_trial_number")
...
class PptBinEdges(ComputedBase):
    ...
    def make(self, key):
        ...
        ppt_bin_edges = np.arange(
            ppt_start, ppt_end + ppt_bin_width, ppt_bin_width
        )  # make bins
        key.update({"ppt_bin_edges": ppt_bin_edges})  # update key
        insert1_print(self, key)


def populate_jguidera_ppt(
    key=None,
