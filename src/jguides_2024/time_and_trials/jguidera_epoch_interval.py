...
class EpochInterval(ComputedBase):
    ...
    def make(self, key):
        interval_list_name = (TaskEpoch & key).fetch1(
            "interval_list_name"
        )  # get interval list name for epoch
        interval_list = NewIntervalList(
            [interval_list_name], key["nwb_file_name"], NO_PREMAZE=True
        ).new_interval_list
        epoch_start = interval_list[0][0]
        epoch_end = interval_list[-1][-1]
        key.update(
            {"trial_start_times": [epoch_start], "trial_end_times": [epoch_end]}
        )
    ...
    def fetch1_dataframe(
        ...
        trial_start_times, trial_end_times = self.fetch1(
    ...
    def get_epoch_duration(self):
        return np.diff(
            [
                unpack_single_element(x)
                for x in self.fetch1("trial_start_times", "trial_end_times")
            ]
        )[0]


def populate_jguidera_epoch_interval(
    key=None,
