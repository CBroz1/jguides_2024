...
class ResSetParamName(ParamNameBase):
    ...
    # of param names
    def get_full_param_name(
        self, trials_pool_cohort_param_name, combination_params
    ):
        ...
        def get_combination_params_text(combination_params):
            return make_param_name(
                list(sort_dict_by_keys(combination_params).values()),
                separating_character="_",
                tolerate_non_unique=True,
            )

        combination_params_text = get_combination_params_text(
            combination_params
        )  # default
        # Make map with available abbreviations for combinations params text
        no_combination_params_text = get_combination_params_text(
            self._get_params_table().get_no_combination_params()
        )
        ...
        return make_param_name(
            [trials_pool_cohort_param_name, combination_params_text],
            separating_character="_",
            tolerate_non_unique=True,
        )
...
class ResSetParams(SecKeyParamsBase):
    ...
    def insert1(self, key, **kwargs):
        ...
        combination_params = self.convert_combination_params(
            combination_params
        )  # convert ["none"] to None
        CombineIntervalLists(interval_lists).convert_check_combination_params(
            **combination_params
        )
        # ...Get interval lists: these are sets of trial times from specified sources
        interval_lists = TrialsPoolCohort().get_cohort_trial_intervals(key)
    ...
    def get_no_combination_params():
        ...
        return {
            "combination_interval_list_names": ["none"],
    ...
    def lookup_no_combination_param_name(self, trials_pool_cohort_param_name):
        return self.lookup_param_name(
            trials_pool_cohort_param_name, self.get_no_combination_params()
        )

    def _insert_from_combination_params(
        self, combination_params, key, use_full_param_name
    ):
        # Get param name and insert into param name table
        res_set_param_name = self._get_insert_param_name(
            key["trials_pool_cohort_param_name"],
            combination_params,
            use_full_param_name,
        )
        # Update key
        key.update(
            {**combination_params, **{"res_set_param_name": res_set_param_name}}
        )
        # Insert into table
        self.insert1(key)
    ...
    def insert_no_combination(self, key):
        ...
        trials_pool_param_names = (
            TrialsPoolCohort().get_upstream_pool_table_param_names(key)
        )
        ...
        self._insert_from_combination_params(
            combination_params, key, use_full_param_name=True
        )
    ...
    def insert_simple_intersection(self, key):
        ...
        combination_interval_list_names = zip_adjacent_elements(
            trials_pool_param_names
        )
        num_combinations = len(combination_interval_list_names)
        combination_params = {
            "combination_interval_list_names": combination_interval_list_names,
            "combination_sources": [["original", "original"]]
            * num_combinations,
            "combination_operations": ["intersection"] * num_combinations,
        }

        self._insert_from_combination_params(
            combination_params, key, use_full_param_name=False
        )
    ...
    def insert_test(self):
        ...
        table_intersection = (
            TrialsPoolCohort * TrialsPoolCohortParams
        )  # helpful if cohort table not fully populated
        valid_bool = (
            np.asarray(
                list(
                    map(
                        len, table_intersection.fetch("trials_pool_param_names")
                    )
                )
            )
            > 1
    ...
    def insert_no_combination_defaults(self, **kwargs):
        ...
        keys = [
            k
            for k in keys
            if "PptTrials" not in k["trials_pool_cohort_param_name"]
        ]
        for key in keys:
    ...
    def fetch1_combination_params(self):
        ...
        return self.convert_combination_params(
            combination_params
        )  # convert ["none"] to None
...
class ResSet(ComputedBase):
    ...
    def _get_upstream_key(key):
        ...
        return {
            **key,
            **{
                "trials_pool_cohort_param_name": (ResSetParams & key).fetch1(
                    "trials_pool_cohort_param_name"
                )
            },
        }
    ...
    def get_trial_intervals(self, key):
        return TrialsPoolCohort().get_cohort_trial_intervals(
            self._get_upstream_key(key)
        )
    ...
    def make(self, key):
        ...
        key["valid_time_intervals"] = CombineIntervalLists(
            trial_intervals
        ).get_combined_intervals(**combination_params)
    ...
    def apply_restriction(
        self, key, time_bin_centers, time_bin_edges=None, verbose=False
    ):
        ...
        # intervals
        valid_time_intervals = (self & key).fetch1(
            "valid_time_intervals"
        )  # valid time intervals
        valid_bool = bins_in_intervals_bool(
            valid_time_intervals, time_bin_centers, time_bin_edges
        )
        ...
        if verbose:
            ...
            ax.plot(
                time_bin_centers,
                [1.1] * len(time_bin_centers),
                "o",
                label="time bin centers",
                color="gray",
            )
            # Plot time bin edges if passed
            if time_bin_edges is not None:
                for label, x in zip(
                    ["bin_starts", "bin_stops"], zip(*time_bin_edges)
                ):  # for bin edge starts and stops
                    ax.plot(
                        x,
                        "o",
                        label=label,
            ...
            ax.plot(
                valid_time_bin_centers,
                [1.1] * len(valid_time_bin_centers),
                "o",
                label="valid time bin centers",
                color="orange",
            )
            ...
            ax.plot(
                ...
                valid_bool,
                "o",
                label="valid time bin centers bool",
                color="blue",
            )
            # Legend
            ax.legend()
    ...
    def plot_result(self, key):
        ...
        trial_intervals = self.get_trial_intervals(key)
        for idx, (intervals_name, intervals) in enumerate(
            trial_intervals.items()
        ):
            plot_intervals(
                intervals,
                val_list=[idx] * len(intervals),
                ax=ax,
                label=intervals_name,
            )
        # Plot valid intervals from combination
        valid_time_intervals = (self & key).fetch1("valid_time_intervals")
        plot_intervals(
            valid_time_intervals,
            val_list=[idx + 1] * len(valid_time_intervals),
            ax=ax,
            color="green",
