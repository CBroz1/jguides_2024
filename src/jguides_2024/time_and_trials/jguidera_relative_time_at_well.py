...
class RelTimeBase(CovDigmethBase):
    ...
    def make(self, key):
        ...
        df = pd.DataFrame.from_dict(
            {"time": time_vector, self._covariate_name(): rel_time_vec}
        ).set_index("time")
...
class RelTimeWellPostDelay(RelTimeBase):
    ...
    def digitized_rel_time_well_post_delay(
        self, bin_width=None, bin_edges=None, verbose=False
    ):
        # Check inputs
        ...
        if bin_edges is None:
            bin_edges = RelTimeWellPostDelayDigParams().make_bin_edges(
                key={"bin_width": bin_width}
            )
        ...
        rel_time_well_post_delay_df[
            "digitized_relative_time_at_well_post_delay"
        ] = digitize_indexed_variable(
            indexed_variable=rel_time_well_post_delay_df[
                "relative_time_at_well_post_delay"
            ],
            bin_edges=bin_edges,
            verbose=verbose,
        )
...
class RelTimeWellPostDelayDig(ComputedBase):
    ...
    def make(self, key):
        ...
        rel_time_well_post_delay_bin_width = (
            RelTimeWellPostDelayDigParams & key
        ).fetch1("rel_time_well_post_delay_bin_width")

        # Replace relative time column values with digitized version
        dig_df = (
            RelTimeWellPostDelay & key
        ).digitized_rel_time_well_post_delay(
    ...
    def plot_results(self):
        ...
        ax.plot(
            df.digitized_relative_time_at_well_post_delay, current_table_color
        )
        ax.plot(
            df.dd_path_names,
            current_table_color,
            linewidth=current_table_linewidth,
        )
