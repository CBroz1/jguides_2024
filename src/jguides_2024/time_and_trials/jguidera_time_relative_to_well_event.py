...
class TimeRelWA(ComputedBase):
    ...
    def make(self, key):
        ...
        time_vector = (
            (ResTimeBinsPool & key).fetch1_dataframe().time_bin_centers.values
        )
        ...
        time_tile = np.tile(
            time_vector, (len(well_arrival_times), 1)
        )  # columns correspond to time
        wa_tile = np.tile(
            well_arrival_times, (len(time_vector), 1)
        ).T  # each row corresponds to a well arrival
        ...
        df = pd.DataFrame.from_dict(
            {
                "time": time_vector,
                "time_to_wa": time_to_wa,
                "time_from_wa": time_from_wa,
            }
        ).set_index("time")
        ...
        key.update(
            {
                dd_trials_meta_param_name: DioWellDDTrialsParams().lookup_no_shift_param_name()
            }
        )
        column_names = [
        ...
        trials_info_df = (DioWellDDTrials & key).label_time_vector(
            time_vector, column_names, add_dd_text=True
        )
        # ...Flag above column names for replacing None with "none"
        replace_none_col_names = [
            x
            for x in trials_info_df.columns
            if any([y in x for y in column_names])
        ]
        ...
        insert_analysis_table_entry(
            ...
            main_key,
            reset_index=True,
            replace_none_col_names=replace_none_col_names,
        )
    ...
    def get_bin_edges_map(self, bin_width):
        ...
        df = self.fetch1_dataframe()
        return {
            column_name: get_peri_event_bin_edges(
                [0, np.nanmax(df[column_name])], bin_width=bin_width
            )
...
class TimeRelWADigParams(CovariateDigParamsBase):
    ...
    def _default_params(self):
        return [
            [0.1],  # GLM analysis
            [0.25],  # firing rate vector analysis
        ]
    ...
    def get_bin_edges_map(self, **kwargs):
        ...
        if "epoch" not in key and "epochs_description" in key:
            key["epoch"] = unpack_single_element(
    ...
    def _get_potential_keys(self, key_filter=None):
        ...
        for key in (TimeRelWA & key_filter).fetch("KEY"):
            ...
            if (
                ...
                time_rel_wa_dig_param_name = glm_default_params_map[
                    "time_rel_wa_dig_param_name"
                ]
            elif (
                key["res_time_bins_pool_param_name"]
                == wa_fr_vec_default_params_map["res_time_bins_pool_param_name"]
            ):
                time_rel_wa_dig_param_name = wa_fr_vec_default_params_map[
                    "time_rel_wa_dig_param_name"
                ]
...
class TimeRelWADigSingleAxisParams(SecKeyParamsBase):
    ...
    def get_bin_edges(self, key):
        ...
        if len(zero_idxs) == 2:
            single_axis_bin_edges = np.delete(
                single_axis_bin_edges, zero_idxs[0]
            )
        # ...Restrict to bins that fall within relative time bounds
        valid_bool = np.logical_and(
            single_axis_bin_edges >= rel_time_start,
            single_axis_bin_edges <= rel_time_end,
        )
    ...
    def get_valid_bin_nums(self, key):
        ...
        return np.sort(
            np.concatenate((-np.arange(0, num_neg), np.arange(1, num_pos + 1)))
        )
...
class TimeRelWADigSingleAxisSel(SelBase):
    ...
    def _get_potential_keys(self, key_filter=None):
        ...
        for params_map, meta_shorthand_name in zip(
            ...
            if (
                time_bins_shorthand
                in ResTimeBinsPoolSel().get_shorthand_params_map()
            ):
                domain = (
                    ResTimeBinsPoolSel()
                    .get_shorthand_params_map()[time_bins_shorthand]
                    .domain
                )
                key_filter_.update(
                    {
...
class TimeRelWADigSingleAxis(ComputedBase):
    ...
    def make(self, key):
        ...
        neg_bool = time_rel_wa_dig_df.digitized_time_to_wa.between(
            0, num_bins_neg
        )
        pos_bool = time_rel_wa_dig_df.digitized_time_from_wa.between(
            0, num_bins_pos
        )
        pos_bool[neg_bool * pos_bool] = (
            False  # if in pre well period, remove from post well period
        )
        ...
        time_rel_wa_comb = pd.Series(
            [np.nan] * len(time_rel_wa_dig_df), index=time_rel_wa_dig_df.index
        )
        time_rel_wa_comb[neg_bool] = [
            self._convert_pre_wa_int(x)
            for x in time_rel_wa_dig_df.digitized_time_to_wa[neg_bool]
        ]
        time_rel_wa_comb[pos_bool] = time_rel_wa_dig_df.digitized_time_from_wa[
            pos_bool
        ]
        time_rel_wa_dig_single_axis_df = pd.DataFrame.from_dict(
            {
                time_rel_wa_comb.index.name: time_rel_wa_comb.index,
                "time_rel_wa": time_rel_wa_comb,
            }
        )
...
class TimeRelWARCBSel(SelBase):
    ...
    def _get_potential_keys(self, key_filter=None):
        ...
        default_params.update(
            {
                "res_time_bins_pool_param_name": glm_params[
        ...
        for k1 in (RaisedCosineBasis & key_filter).fetch("KEY"):
            ...
            bin_width_rcb, domain_rcb = (RaisedCosineBasisParams & k1).fetch1(
                "bin_width", "domain"
            )
            # Get domain and bin width for upstream tables param names
            bin_width = (TimeRelWADigParams & key_filter).fetch1(
                "time_bin_width"
            )
            domain = (TimeRelWADigSingleAxisParams & key_filter).fetch1(
                "rel_time_start", "rel_time_end"
            )
            if not np.logical_and(
                bin_width_rcb == bin_width, tuple(domain_rcb) == domain
            ):
                continue
...
class TimeRelWARCB(CovariateRCB):
    ...
    def make(self, key):
        ...
        time_rel_wa_dig = (
            (TimeRelWADigSingleAxis & key)
            .fetch1_dataframe()
            .time_rel_wa.astype(int)
        )
        ...
        insert_analysis_table_entry(
            self, [time_rel_wa_rcb_df], key, reset_index=True
        )
