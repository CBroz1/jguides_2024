...
class NewIntervalList:
    ...
    def __init__(
        ...
        check_membership(
            starting_interval_list_names,
            self._valid_starting_interval_list_names(),
        )
    ...
    def _valid_starting_interval_list_names():
        ...
        if any(
            [
                x in y
                for idx_x, x in enumerate(valid_starting_interval_list_names)
                for idx_y, y in enumerate(valid_starting_interval_list_names)
                if idx_x != idx_y
            ]
        ):
            raise Exception(
    ...
    @classmethod
    def get_starting_interval_list_names_from_new_interval_list_name(
        cls, new_interval_list_name
    ):
        ...
        if version_num.isdigit():
            new_interval_list_name = "_v".join(
                split_new_interval_list_name[:-1]
            )
        ...
        starting_interval_list_names = [
            x
            for x in cls._valid_starting_interval_list_names()
            if x in new_interval_list_name
        ]
        if len(" ".join(starting_interval_list_names)) != len(
    ...
    @classmethod
    def get_epochs_for_new_interval_list_name(
        cls, nwb_file_name, new_interval_list_name
    ):
        ...
        return np.concatenate(
            [
                _get_epoch(nwb_file_name, interval_list_name)
    ...
    def _define_exclusion_periods(self):
        ...
        epoch_names = (TaskEpoch & {"nwb_file_name": self.nwb_file_name}).fetch(
            "interval_list_name"
        )
        ...
        def _get_epoch_names_nums(epoch_names, identifier):
            ...
            epoch_names = [
                ...
                if identifier in epoch_name
            ]
            epoch_nums = [
                int(epoch_name.split("_")[0]) for epoch_name in epoch_names
            ]
            return epoch_names, epoch_nums

        if (
            self.NO_PREMAZE
        ):  # if want to exclude period when rat being carried to track during run sessions
            # Get names and epoch integers (use to get premaze durations) of run epochs
            run_epoch_names, run_epoch_nums = _get_epoch_names_nums(
                epoch_names, "_r"
            )
            print(f"\nIdentified run epochs: {run_epoch_names}")
            # Add intervals corresponding to run epochs to list of intervals to exclude
            for run_epoch_name, run_epoch_num in zip(
                run_epoch_names, run_epoch_nums
            ):  # for each run epoch
                ...
                interval_list = (
                    ...
                    & {
                        ...
                        "interval_list_name": run_epoch_name,
                    }
                ).fetch1("valid_times")
                ...
                ).fetch1("premaze_duration")
                # Append premaze period to list of intervals to exclude
                exclude_interval_list.append(
                    [
                        interval_list[0][0],
                        interval_list[0][0] + premaze_duration,
                    ]
                )
        ...
        if self.NO_HOME:  # if want to exclude home epochs
            ...
            for home_epoch_name in home_epoch_names:  # for each home epoch
                ...
                interval_list = (
                    ...
                    & {
                        "nwb_file_name": self.nwb_file_name,
                        "interval_list_name": home_epoch_name,
                    }
                ).fetch1("valid_times")
                # Append home epoch period to list of intervals to exclude
                exclude_interval_list.append(
                    [interval_list[0][0], interval_list[-1][-1]]
                )
        ...
        if self.NO_SLEEP:  # if want to exclude sleep epochs
            ...
            for sleep_epoch_name in sleep_epoch_names:  # for each sleep epoch
                ...
                interval_list = (
                    ...
                    & {
                        "nwb_file_name": self.nwb_file_name,
                        "interval_list_name": sleep_epoch_name,
                    }
                ).fetch1("valid_times")
    ...
    def _widen_exclusion_periods(self, exclude_interval_list):
        ...
        return [
            [
                exclude_interval[0] - self.widen_exclusion_factor,
                exclude_interval[1] + self.widen_exclusion_factor,
            ]
    ...
    def _get_new_interval_list(self):
        ...
        # Widen exclusion periods
        exclude_interval_list = self._widen_exclusion_periods(
                ...
                (
                    itertools.chain.from_iterable(
                        [
                            ...
                            for interval_list_name in self.starting_interval_list_names
                        ]
                    )
                )
            )
        )  # get concatenation of interval lists that we will change

        new_interval_list = exclude_intervals(
            starting_interval_list=starting_interval_list,
            exclude_interval_list=exclude_interval_list,
        )  # exclude periods from starting interval list
