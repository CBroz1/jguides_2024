    ...
    def get_shank_map():
        ...
        return {
            "J16": {
                ...
                #
                "24": {
                    "below_dorsal_limit_lens": np.asarray([1, 1, 0.9, 0.92])
                    * 1000,
            ...
            "mango": {
                ...
                "25": {
                    "below_dorsal_limit_lens": np.asarray([1, 0.79, 0.4, 0.48])
                    * 1000,
                ...
                "24": {
                    "below_dorsal_limit_lens": np.asarray(
                        [0.77, 0.8, 0.71, 0.8][::-1]
                    )
                    * 1000,
                    "below_ventral_limit_lens": np.zeros(4)[::-1] * 1000,
            ...
            "june": {
                ...
                "24": {
                    "below_dorsal_limit_lens": np.ones(4)[::-1] * 1000,
                    "below_ventral_limit_lens": np.asarray([0, 0, 1, 1])[::-1]
                    * 1000,
                },
                # right hemisphere mPFC probe:
                # reverse typed order to get left to right shank order since implanted in
            ...
            "fig": {
                ...
                # left hemisphere with same side stereotax arm
                "24": {
                    "below_dorsal_limit_lens": np.asarray([1, 0.95, 1, 0][::-1])
                    * 1000,
                    "below_ventral_limit_lens": np.zeros(4)[::-1] * 1000,
                }
            },
            ...
            "peanut": {
                ...
                # left hemisphere with opposite side stereotax arm
                "25": {
                    "below_dorsal_limit_lens": np.asarray([1, 1, 1, 0.6])
                    * 1000,
                    "below_ventral_limit_lens": np.zeros(4) * 1000,
                },
            },
...
class LivermoreD2:
    ...
    def __init__(self):
        ...
        self.tip_to_middle_of_top_contact_dist = (
            self._get_tip_to_middle_of_top_contact_dist()
        )
        self.tip_to_middle_of_lowest_contact_dist = (
            self._get_tip_to_middle_of_lowest_contact_dist()
        )
    ...
    def _get_tip_to_middle_of_top_contact_dist(self):
        ...
        return (
            self.electrode_edge_to_tip_dist
            + self.electrode_pitch * self.num_contacts_per_shank
    ...
    def get_num_contacts_below_dorsal_limit(self, below_dorsal_limit_len):
        ...
        num_valid_contacts = int(
            np.floor(
                (
                    below_dorsal_limit_len - self.electrode_edge_to_tip_dist
                )  # subtract off contactless shank tip
                / self.electrode_pitch
            )
        )  # divide by distance between electrodes
    ...
    def get_num_invalid_dorsal_contacts(self, below_dorsal_limit_len):
        return (
            self.num_contacts_per_shank
            - self.get_num_contacts_below_dorsal_limit(below_dorsal_limit_len)
        )
    ...
    def get_num_invalid_ventral_contacts(self, below_ventral_limit_len):
        ...
        num_contacts = int(
            np.floor(
                (
                    below_ventral_limit_len - self.electrode_edge_to_tip_dist
                )  # subtract off contactless shank tip
                / self.electrode_pitch
            )
        )  # divide by distance between electrodes
    ...
    def get_valid_idxs(
        ...
        # check ranges valid
        for k, v in {
            "below_dorsal_limit_lens": below_dorsal_limit_lens,
            "below_ventral_limit_lens": below_ventral_limit_lens,
        }.items():
            if len(v) != self.num_shanks:
                raise Exception(
                    f"{k} must have exactly as many elements as self.num_shanks: {self.num_shanks}"
                )

        # Get number of invalid dorsal channels for each shank
        num_invalid_dorsal_contacts = [
            self.get_num_invalid_dorsal_contacts(x)
            for x in below_dorsal_limit_lens
        ]

        # Get number of invalid ventral channels for each shank
        num_invalid_ventral_contacts = [
            self.get_num_invalid_ventral_contacts(x)
            for x in below_ventral_limit_lens
        ]
        ...
        if electrode_order == "dorsal_to_ventral":
            return np.concatenate(
                [
                    np.arange(
                        shank_num * self.num_contacts_per_shank
                        + num_invalid_dorsal,
                        (shank_num + 1) * self.num_contacts_per_shank
                        - num_invalid_ventral,
                    )
                    for shank_num, (
                        num_invalid_dorsal,
