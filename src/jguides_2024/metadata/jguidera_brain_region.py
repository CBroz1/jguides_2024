...
class BrainRegionColor(dj.Manual):
    ...
    def return_brain_region_color_map(self, brain_regions=None):
        ...
        return {
            brain_region: (self & {"brain_region": brain_region}).fetch1(
                "color"
            )
    ...
    def get_brain_region_color(self, brain_region):
        brain_region_color_map = self.return_brain_region_color_map(
    ...
    def tint_colors(self, brain_regions, num_tints, concatenate=False):
        brain_region_colors = {
            x: self.return_brain_region_color_map()[x] for x in brain_regions
        }
        brain_region_colors_tint = {
            x: tint_color(color, num_tints)
            for x, color in brain_region_colors.items()
        }
    ...
    def insert_defaults(self, **kwargs):
        ...
        for brain_region, color in zip(
            ["OFC_2", "OFC", "mPFC_2", "mPFC", "CA1_2", "CA1"],
            plt.cm.Paired.colors,
        ):
            self.insert1(
                {"brain_region": brain_region, "color": color},
                skip_duplicates=True,
            )
        # Insert for targeted regions -- same as corresponding brain regions
        for targeted_location, color in zip(
            ["OFC", "mPFC", "CA1"], plt.cm.Paired.colors[1::2]
        ):
...
def _get_jguidera_keys(key=None):
    ...
    if key is not None:
        if len(TaskIdentification & key) == 0:
            raise Exception(
                f"There must be entry in TaskIdentification to populate ElectrodeGroupTargetedLocation. "
...
class ElectrodeGroupTargetedLocation(ComputedBase):
    ...
    def make(self, key):
        ...
        key = {
            **key,
            **{
                "targeted_location": nwbf.electrode_groups[
...
class SortGroupTargetedLocation(ComputedBase):
    ...
    def make(self, key):
        ...
        if (
            len(electrode_group_names) != 1
        ):  # check that only found one electrode group for sort group
            raise Exception(
                f"Should have found exactly one electrode group for "
    ...
    def _get_sort_group_id_targeted_location(
        self, nwb_file_name, exclude_no_unit_sort_group_ids=False
    ):

        sort_group_ids, targeted_locations = (
            self & {"nwb_file_name": nwb_file_name}
        ).fetch("sort_group_id", "targeted_location")
    ...
    def return_targeted_location_sort_group_map(
        self, nwb_file_name, exclude_no_unit_sort_group_ids=False
    ):
        sort_group_ids, targeted_locations = (
            self._get_sort_group_id_targeted_location(
                nwb_file_name, exclude_no_unit_sort_group_ids
            )
        )
        return {
            targeted_location: sort_group_ids[
                targeted_locations == targeted_location
            ]
            for targeted_location in set(targeted_locations)
        }

    def return_sort_group_targeted_location_map(
        self, nwb_file_name, exclude_no_unit_sort_group_ids=False
    ):
        sort_group_ids, targeted_locations = (
            self._get_sort_group_id_targeted_location(
    ...
    def get_sort_group_ids(self, nwb_file_name, targeted_locations=None):
        sort_group_map = self.return_targeted_location_sort_group_map(
            nwb_file_name
        )
        if (
            targeted_locations is not None
        ):  # take only those in list of brain regions if passed
            sort_group_ids = [
                sort_group_map[x]
                for x in targeted_locations
                if x in sort_group_map
            ]
        else:
            sort_group_ids = list(sort_group_map.values())
        return np.sort(np.concatenate(sort_group_ids))

    def lookup_targeted_location(
        self, nwb_file_name, sort_group_id=None, unit_name=None
    ):
        check_one_none([sort_group_id, unit_name])
        if sort_group_id is None:
            sort_group_id, _ = split_unit_name(unit_name)
        return self.return_sort_group_targeted_location_map(nwb_file_name)[
            sort_group_id
        ]
...
class BrainRegionSortGroupParams(SecKeyParamsBase):
    ...
    def _get_source_table_name_suffix_map():
        ...
        return {
            "SortGroupTargetedLocation": "targeted",
        }
    ...
    def insert_defaults(self, **kwargs):
        ...
        for (
            source_table_name,
            suffix,
        ) in self._get_source_table_name_suffix_map().items():
            ...
            for nwb_file_name, targeted_location in unique_table_column_sets(
                ...
                brain_region = self.get_brain_region_text(
                    targeted_location, source_table_name
                )
                key = {
                    "nwb_file_name": nwb_file_name,
                    "brain_region": brain_region,
                    "source_brain_region": targeted_location,
...
class BrainRegionSortGroup(ComputedBase):
    ...
    def make(self, key):
        ...
        source_table_name, source_brain_region = (
            BrainRegionSortGroupParams & key
        ).fetch1("source_table_name", "source_brain_region")
        if source_table_name == "SortGroupTargetedLocation":
            sort_group_ids = SortGroupTargetedLocation().get_sort_group_ids(
                key["nwb_file_name"], [source_brain_region]
            )
            ...
            for sort_group_id in sort_group_ids:
                insert1_print(
                    self.SortGroupTargetedLocation,
    ...
    def get_brain_region_sort_group_id_map(self):
        ...
        return df_from_data_list(
            [
                (entry["nwb_file_name"], entry["brain_region"], sort_group_id)
...
class BrainRegionCohort(dj.Manual):
    ...
    def insert_defaults(self, **kwargs):
        ...
        for nwb_file_name in JguideraNwbfile.fetch("nwb_file_name"):
            for cohort_name_suffix, targeted in zip(
                ["targeted", ""], [True, False]
            ):
                ...
                brain_regions = get_brain_regions(
                    nwb_file_name, targeted=targeted
                )
                self.insert1(
                    {
                        "nwb_file_name": nwb_file_name,
            ...
            self.insert1(
                {
                    ...
                    "brain_regions": ["mPFC_targeted", "OFC_targeted"],
                },
                skip_duplicates=True,
            )
    ...
    def lookup_cohort_name(self, nwb_file_name, brain_regions):
        return unpack_single_element(
            [
                k["brain_region_cohort_name"]
                for k in (self & {"nwb_file_name": nwb_file_name}).fetch("KEY")
                if check_set_equality(
                    (self & k).fetch1("brain_regions"),
                    brain_regions,
                    tolerate_error=True,
                )
            ]
...
class CurationSet(ComputedBase):
    ...
    def make(self, key):
        ...
        for brain_region in brain_regions:
            ...
            objs = define_sort_intervals(
                targeted_location, nwb_file_name, curation_set_name
            )
            for obj in objs:

                # Get curation ID
                curation_name = get_curation_name(
                    obj.sort_interval_name, curation_id
                )
                ...
                if curation_set_name in [
                    ...
                    if "pos" in obj.interval_list_name:
                        ...
                        interval_list_name = obj.interval_list_name.split(
                            " no premaze"
                        )[0].split(" no home")[0]
                        epoch = EpochIntervalListName().get_epoch(
                    ...
                    # across runs (and potentially sleeps) sort
                        ...
                        epochs_descriptions = (
                            EpochsDescription().get_single_run_descriptions(
                                nwb_file_name
                            )
                        )
                ...
                # All run sessions
                    ...
                    data_list.append(
                        (curation_name, nwb_file_name, brain_region, "runs")
                    )

                else:
                    raise Exception(
                        f"curation_set_name {curation_set_name} not accounted for"
                    )

        curation_names_df = df_from_data_list(
            data_list,
            [
                "curation_name",
                "nwb_file_name",
    ...
    def get_split_curation_names_map(self):
        ...
        for brain_region, curation_name in curation_names_map.items():
            ...
            split_curation_names_map[brain_region] = {
    ...
    def get_curation_name(
        self, brain_region, epochs_description=None, tolerate_no_entry=False
    ):
        ...
        return check_return_single_element(
            df_subset.curation_name.values
        ).single_element


def populate_jguidera_brain_region(
    key=None,
