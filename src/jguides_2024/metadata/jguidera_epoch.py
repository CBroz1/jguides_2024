...
class EpochFullLen(dj.Computed):
    ...
    def make(self, key):
        key.update(
            {
                "epoch_full_len": int(
                    (EpochInterval & key).get_epoch_duration() > 19 * 60
                )
            }
        )
        insert1_print(self, key)

    def exclude_non_full_len_epochs_descriptions(
        self, nwb_file_name, epochs_descriptions
    ):
        return [
            x
            for x in epochs_descriptions
            if (
                self
                & {
                    ...
                    "epoch": (
                        ...
                        & {
                            "nwb_file_name": nwb_file_name,
                            "epochs_description": x,
                        }
                    ).get_epoch(),
                }
            ).fetch1("epoch_full_len")
        ]
...
class EpochCohortParams(SecKeyParamsBase):
    ...
    def insert_epoch_combinations(
        self, epoch_source_table, num_in_comb, key_filter
    ):
        ...
        for nwb_file_name in (
            (JguideraNwbfile * epoch_source_table) & key_filter
        ).fetch("nwb_file_name"):
...
class RunEpoch(ComputedBase):
    ...
    def get_run_num(self, nwb_file_name, epoch):
        return (
            unpack_single_element(
                np.where(self.get_epochs(nwb_file_name) == epoch)[0]
            )
            + 1
        )
    ...
    def get_single_contingency_epochs(self, nwb_file_name, contingency=None):
        ...
        return [
            epoch
            for epoch in (self & {"nwb_file_name": nwb_file_name}).fetch(
                "epoch"
            )
            if TaskIdentification().is_single_contingency_epoch(
                nwb_file_name, epoch, contingency
            )
        ]
...
class EpochsDescription(dj.Manual):
    ...
    def valid_epochs_descriptions(cls):
        return [cls.get_all_runs_description()] + [
            cls.format_single_run_description(x) for x in np.arange(1, 10)
        ]

    def get_single_contingency_descriptions(
        self,
        nwb_file_name,
        exclude_non_full_len=False,
        ...
        # Check inputs
        check_membership(
            [contingency], [None] + TaskIdentification().single_contingencies()
        )

        # Get single contigency epoch descriptions
        epochs_descriptions = [
            epochs_description
            for epochs_description, epochs in fetch_iterable_array(
                (self & {"nwb_file_name": nwb_file_name}),
                ["epochs_description", "epochs"],
            )
            if len(epochs) == 1  # single epochs
            and TaskIdentification().is_single_contingency_epoch(
                nwb_file_name, epochs[0], contingency
            )
        ]
        ...
        if exclude_non_full_len:
            epochs_descriptions = (
                EpochFullLen().exclude_non_full_len_epochs_descriptions(
        ...
        if exclude_artifact:
            epochs_descriptions = (
                EpochArtifactFree().exclude_artifact_epoch_descriptions(
                    nwb_file_name, epochs_descriptions
                )
            )
    ...
    def get_single_run_descriptions(self, nwb_file_name):
        epochs_descriptions = (self & {"nwb_file_name": nwb_file_name}).fetch(
            "epochs_description"
        )
        return [
            x for x in epochs_descriptions if x[:3] == "run" and x[3:].isdigit()
        ]
    ...
    def get_runs_description(self, nwb_file_name, epochs):
        run_nums = [
            RunEpoch().get_run_num(nwb_file_name, epoch) for epoch in epochs
        ]
    ...
    def insert1(self, key, **kwargs):
        kwargs = add_defaults(
            kwargs, {"skip_duplicates": True}, add_nonexistent_keys=True
        )
        super().insert1(key, **kwargs)
    ...
    def insert_defaults(self, **kwargs):
        ...
        for nwb_file_name in set(
            ((JguideraNwbfile * RunEpoch) & key_filter).fetch("nwb_file_name")
        ):
            ...
            for epoch in epochs:
                ...
                key.update(
                    {
                        "epochs": epochs_,
                        "epochs_id": get_epochs_id(epochs_),
                        "epochs_description": self.format_single_run_description(
                            run_num
                        ),
                    }
                )
    ...
    def insert_runs(self, nwb_file_name, epochs):
        ...
        if len(epochs) == 1:  # single run gets "run" prefix
            epochs_description = self.get_single_run_description(
                nwb_file_name, unpack_single_element(epochs)
            )
        elif len(epochs) == len(
            run_epochs
        ):  # all runs gets just "runs", nothing else
            epochs_description = self.get_all_runs_description()
        else:  # multiple but not all runs get "runs" prefix
            epochs_description = self.get_runs_description(
...
class EpochArtifactFree(dj.Computed):
    ...
    def exclude_artifact_epoch_descriptions(
        self, nwb_file_name, epochs_descriptions
    ):
        return [
            x
    ...
    def insert_defaults(self, **kwargs):
        ...
        if key_filter is not None:
            if "nwb_file_name" in key_filter:
                nwb_file_names = [
                    x
                    for x in nwb_file_names
                    if x == key_filter["nwb_file_name"]
                ]
        ...
        for nwb_file_name in nwb_file_names:
            ...
            epochs_descriptions = (
                EpochsDescription().get_single_contingency_descriptions(
                    nwb_file_name,
                    exclude_non_full_len=True,
            ...
            self.insert1(
                {**key, **{"epochs_descriptions": epochs_descriptions}},
        ...
        for nwb_file_name in nwb_file_names:
            ...
            for epochs_description in epochs_descriptions:
                ...
                key = {
                    "nwb_file_name": nwb_file_name,
                    "epochs_descriptions_name": epochs_description,
                }
                self.insert1(
                    {**key, **{"epochs_descriptions": [epochs_description]}},
                    skip_duplicates=True,
                )
        ...
        if key_filter is not None:
            if "nwb_file_name" in key_filter:
                nwb_file_name_epochs_descriptions = [
                    x
                    for x in nwb_file_name_epochs_descriptions
                    if x[0] == key_filter["nwb_file_name"]
                ]

        for (
            nwb_file_name,
            epochs_description,
        ) in nwb_file_name_epochs_descriptions:
...
class NwbfSetBase(dj.Manual):
    ...
    def lookup_rat_cohort_set_name(self):
        ...
        return (
            self
            & {
                self._meta_set_name(): self.get_Haight_single_contingency_rotation_set_name(
                    subject_ids
                )
            }
        ).fetch1(self._meta_set_name())
    ...
    @staticmethod
    def get_Haight_single_contingency_rotation_set_name(
        subject_ids, contingency=None
    ):
    ...
    @staticmethod
    def get_single_epochs_description_set_name(
    ...
    def get_subject_id(self):
        ...
        return check_return_single_element(
            [get_subject_id(x) for x in nwb_file_names]
        ).single_element
...
class RecordingSet(NwbfSetBase):
    ...
    def _insert_tables(
        self, recording_set_name, nwb_file_names, epochs_descriptions_names
    ):
        ...
        self.insert1(
            {
                ...
                **{
                    ...
                    "epochs_descriptions_names": epochs_descriptions_names,
                },
            },
            skip_duplicates=True,
        )
    ...
    def insert_defaults(self, **kwargs):
        ...
        for subject_id, nwb_file_names in get_reliability_paper_nwb_file_names(
            as_dict=True
        ).items():
            recording_set_name = (
            ...
            epochs_descriptions_names = [epochs_descriptions_name] * len(
                nwb_file_names
            )
            self._insert_tables(
                recording_set_name, nwb_file_names, epochs_descriptions_names
            )
        ...
        epochs_descriptions_name = "valid_single_contingency_runs"
        for entry in fetch_entries_as_dict(
            (
                EpochsDescriptions
                & {"epochs_descriptions_name": epochs_descriptions_name}
            )
            ...
            recording_set_name = self.get_single_nwbf_set_name(
        ...
        recording_set_name = (
            self.get_Haight_single_contingency_rotation_set_name(subject_ids)
        )
        self._insert_tables(
            recording_set_name, nwb_file_names_list, epochs_descriptions_names
        )
        ...
        for nwb_file_name in ["J1620210529_.nwb"]:
            ...
            for epochs_description in epochs_descriptions:
                recording_set_name = (
                    self.get_single_epochs_description_set_name(
                        nwb_file_name, epochs_description
                    )
                )
                self._insert_tables(
                    recording_set_name, [nwb_file_name], [epochs_description]
                )
        ...
        for (
            nwb_file_name,
            epochs_description,
        ) in _get_single_epoch_testing_metadata():
            recording_set_name = self.get_single_epochs_description_set_name(
                nwb_file_name, epochs_description
            )
            self._insert_tables(
                recording_set_name, [nwb_file_name], [epochs_description]
    ...
    def get_matching_nwb_file_names(self, key):
        ...
        if all(
            [
                x is not None
                for x in [nwb_file_names, recording_set_nwb_file_names]
            ]
        ):
    ...
    def get_matching_recording_set_names(self, key):
        ...
        if "recording_set_name" in key:
            return (self & key).fetch1(
                "recording_set_name"
            )  # this ensures the passed recording set name is in table
        ...
        return [
            recording_set_name
            for recording_set_name, nwb_file_names in zip(
                *self.fetch("recording_set_name", "nwb_file_names")
            )
            if np.logical_or(
    ...
    def get_recording_set_names(
        self, key_filter=None, recording_set_names_types=None
    ):
        ...
        if recording_set_names_types is None:
            recording_set_names_types = [
                "Haight_rotation",
                "Haight_rotation_single_nwb_files",
            ]
        ...
        valid_recording_set_names_types = [
            ...
            "single_epoch_testing",
        ]
        check_membership(
            recording_set_names_types, valid_recording_set_names_types
        )
        ...
        if "recording_set_name" in key_filter:
            recording_set_names = [
                x
                for x in recording_set_names
                if x == key_filter["recording_set_name"]
            ]
        ...
        if "nwb_file_name" in key_filter:
            recording_set_names = [
                ...
                for x in recording_set_names
                if x
                in self.get_matching_recording_set_names(
    ...
    def get_nwb_file_name_single_epoch_keys(self):
        ...
        nwb_file_names, epochs_descriptions_names = (
            self & {"recording_set_name": recording_set_name}
...
def _get_single_epoch_testing_metadata():
    return [
        ("J1620210606_.nwb", "run1"),
        ("mango20211207_.nwb", "run3"),
        ("june20220419_.nwb", "run1"),
    ]
...
class TrainTestEpoch(dj.Manual):
    ...
    def insert_defaults(self, **kwargs):
        ...
        for nwb_file_name in get_reliability_paper_nwb_file_names():
            ...
            for epochs_description in epochs_descriptions:
                ...
                train_test_epoch_name = self.make_param_name(
    ...
    def lookup_train_test_epoch_name(
        self, train_epochs, test_epochs, nwb_file_name=None
    ):
        if nwb_file_name is None:
    ...
    def get_train_test_epoch(self):
        ...
        return unpack_single_element(train_epochs), unpack_single_element(
            test_epochs
        )
    ...
    def get_epochs_description(self):
        ...
        return EpochsDescription().lookup_epochs_description(
            self.fetch1("nwb_file_name"), epochs
        )
...
class TrainTestEpochSet(NwbfSetBase):
    ...
    def get_single_nwbf_same_train_test_set_name(
        self, nwb_file_name, train_test_epoch_set_name
    ):
        return self.get_single_nwbf_set_name(
            nwb_file_name, train_test_epoch_set_name
        )

    def _insert_tables(
        self, train_test_epoch_set_name, nwb_file_names, train_test_epoch_names
    ):
        # Insert into main table
        ...
        self.insert1(
            {
                ...
                **{
                    "nwb_file_names": nwb_file_names,
                    "train_test_epoch_names": train_test_epoch_names,
                },
            },
            skip_duplicates=True,
        )
        ...
        for nwb_file_name, train_test_epoch_name in zip(
    ...
    def insert_defaults(self, **kwargs):
            ...
            for nwb_file_name in nwb_file_names:
                ...
                for epochs_description in epochs_descriptions:
                    ...
                    train_test_epoch_names.append(
                        (
                            TrainTestEpoch & {"nwb_file_name": nwb_file_name}
                        ).lookup_train_test_epoch_name([epoch], [epoch])
                    )
        ...
        # Insert into tables
        self._insert_tables(
            train_test_epoch_set_name, nwb_file_names_, train_test_epoch_names
        )
    ...
    def get_matching_train_test_epoch_set_names(self, key):
        ...
        return [
            train_test_epoch_set_name
            for train_test_epoch_set_name, nwb_file_names in zip(
                *self.fetch("train_test_epoch_set_name", "nwb_file_names")
            )
            if np.logical_or(
                nwb_file_name is None, nwb_file_name in nwb_file_names
            )
    ...
    def get_train_test_epoch_set_names(
        self, key_filter=None, train_test_epoch_set_names_types=None
    ):
        ...
        if train_test_epoch_set_names_types is None:
            train_test_epoch_set_names_types = [
                "Haight_rotation",
                "Haight_rotation_single_nwb_files",
            ]
        ...
        if (
            "Haight_rotation_single_nwb_files"
            in train_test_epoch_set_names_types
        ):
        ...
        if "train_test_epoch_set_name" in key_filter:
            train_test_epoch_set_names = [
                x
                for x in train_test_epoch_set_names
                if x == key_filter["train_test_epoch_set_name"]
            ]
        ...
        if "nwb_file_name" in key_filter:
            train_test_epoch_set_names = [
                ...
                for x in train_test_epoch_set_names
                if x
                in self.get_matching_train_test_epoch_set_names(
                    {"nwb_file_name": key_filter["nwb_file_name"]}
                )
            ]
