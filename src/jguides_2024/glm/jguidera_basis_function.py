...
class RaisedCosineBasisParams(SecKeyParamsBase):
    ...
    def shorthand_params_map():
        ...
        ShorthandParams = namedtuple(
            "ShorthandParams",
            "covariate_group_name domain bin_width num_basis_functions",
        )
        return {
            "ppt": ShorthandParams("ppt", Ppt.get_range(), 0.05, 14),
            "delay": ShorthandParams("delay", [0, 2], 0.1, 14),
        }

    def _default_params(self):
        shorthand_params_map = self.shorthand_params_map()
        return [
            list(x._asdict().values()) for x in shorthand_params_map.values()
        ]
    ...
    def _make_param_name(
        self, secondary_key_subset_map, separating_character=None
    ):
        param_name = super()._make_param_name(
            secondary_key_subset_map, separating_character
        )
    ...
    def lookup_param_name_from_shorthand(self, shorthand_param_name):
        return self.lookup_param_name(
            list(
                self.shorthand_params_map()[shorthand_param_name]
                ._asdict()
                .values()
            )
        )
...
class RaisedCosineBasis(ComputedBase):
        ...
        self.insert1(
            {
                ...
                **{
                    ...
                    "basis_functions": raised_cosine_basis_obj.basis_functions.to_dict(
                        orient="list"
                    ),
                },
            }
    ...
    def plot_basis_functions(self, raised_cosine_basis_param_name):
        ...
        entry = (
            self
            & {"raised_cosine_basis_param_name": raised_cosine_basis_param_name}
        ).fetch1()
        basis_function_names, basis_functions = zip(
            *entry["basis_functions"].items()
        )
        plot_basis_functions(
            np.asarray(basis_functions).T,
