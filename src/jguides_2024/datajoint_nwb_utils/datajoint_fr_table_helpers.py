...
def get_well_trials_bin_edges(
    ...
    trials_bin_edges = [
        ...
        for unshifted_trial_duration in unshifted_trial_durations
    ]
    ...
    bin_edges = trials_bin_edges[
        np.argmax(list(map(len, trials_bin_edges)))
    ]  # if "ties", argmax takes first
...
def _get_trial_time_info(trials_table, trials_params_table, key):
    ...
    trial_start_time_shift, trial_end_time_shift = map(
        float,
        (trials_params_table & trials_entry).fetch1(
    ...
    trial_intervals = np.asarray(
        list(
            zip(
                trials_entry["trial_start_times"],
                trials_entry["trial_end_times"],
            )
        )
    )
...
def _get_info_for_well_trial_table_fr_df(
    key, trials_table, trials_params_table, firing_rate_map_params_table
):

    # Get trial time info
    (trial_start_time_shift, trial_end_time_shift, trial_intervals) = (
        _get_trial_time_info(trials_table, trials_params_table, key)
    )
    ...
    ).fetch1_dataframe()["epoch_spike_times"]
...
def _get_fr_df_column_names(trial_feature_name=None):
    ...
    meta_data_column_names = ["unit_id"] + [trial_feature_name] * (
        trial_feature_name is not None
    )
    data_column_names = [
        "rate_map",
        "time_bin_centers",
...
def make_well_trial_table_fr_df(
    key, trials_table, trials_params_table, firing_rate_map_params_table
):
    ...
    (
        ...
        bin_centers,
        epoch_spike_times,
    ) = _get_info_for_well_trial_table_fr_df(
        key, trials_table, trials_params_table, firing_rate_map_params_table
    )
...
def make_well_trial_table_fr_df_trial_subsets(
    ...
    trial_feature_values = (
        (trials_table & key)
        .fetch1_dataframe(strip_s=True)[trial_feature_name]
        .values
    )
    trial_intervals_map = {
        trial_feature_value: trial_intervals[
            trial_feature_values == trial_feature_value
        ]
        for trial_feature_value in set(trial_feature_values)
    }  # map from trial feature value to trial intervals

    # Make rate maps
    data_list = []
    for (
        trial_feature_value,
        trial_intervals_subset,
    ) in trial_intervals_map.items():  # trial feature values
        for unit_id, unit_spike_times in epoch_spike_times.items():  # units
            rate_map = _get_well_trial_rate_map(
                unit_spike_times,
                trial_intervals_subset,
                trial_start_time_shift,
                trials_bin_centers,
            ...
            data_list.append(
                (
                    ...
                    bin_centers,
                    bin_edges,
                    len(trial_intervals_subset),
                )
            )
    return pd.DataFrame.from_dict(
        {k: v for k, v in zip(column_names, zip(*data_list))}
    )
...
def make_well_single_trial_table_fr_df(
    key, trials_table, trials_params_table, firing_rate_map_params_table
):
    ...
    extra_info_names_no_s = get_well_single_trial_table_fr_df_extra_info_names(
        strip_s=True
    )
        ...
        (
            ...
            make_rate_map_constant_fs(
                ...
                measurements_at_events=np.asarray(
                    [
                        event_times_in_intervals(
                            unit_spike_times, [trial_interval]
                        )[1]
                        - trial_interval[0]
                        + trial_start_time_shift
                    ]
                ),
                bin_edges=np.asarray([time_bin_edges]),
            )[0],
            time_bin_centers,
            time_bin_edges,
        )
        for trial_interval, time_bin_centers, time_bin_edges in zip(
            trial_intervals, trials_bin_centers, trials_bin_edges
        )
    ...
    rate_map_dict = {
        k: v
        for k, v in zip(
            ["unit_id", "rate_map", "time_bin_centers", "time_bin_edges"],
            zip(*data_list),
        )
    }
    ...
    trials_entry = (trials_table & key).fetch1()
    extra_info_dict = {
        k_no_s: duplicate_elements(list(trials_entry[k]), len(unit_ids))
...
def smooth_datajoint_table_fr(fr_table, params_table, key, data_type):
    ...
    edges_rate_map_smoothed_list = [
        smooth_mask_rate_map(
            event_rate=convert_inf_to_nan(event_rate),
            bin_edges=bin_edges,
            sigma=(
                float(kernel_sd) / np.mean(np.diff(bin_edges))
            ),  # express sigma in samples
            nan_treatment="interpolate",
        )
        for bin_edges, event_rate in zip(
    ...
    return pd.DataFrame.from_dict(
        {
            "unit_id": unit_ids,
            "smoothed_rate_map": edges_rate_map_smoothed_list,
            get_bin_edges_name(data_type): rate_map_df[
                get_bin_edges_name(data_type)
            ],
            get_bin_centers_name(data_type): rate_map_df[
                get_bin_centers_name(data_type)
            ],
        }
    )
...
def get_smoothed_fr_temporal_event_table_map():
    ...
    trial_feature_names = ["none", "well_name", "none", "well_name", "none"]
    trial_table_names = [
    ...
    return pd.DataFrame.from_dict(
        {
            "trial_align_event": trial_align_events,
            ...
            "trial_param_name": trial_param_names,
        }
    ).set_index("trial_align_event")
...
def get_smoothed_fr_spatial_table_map():
    ...
    trial_table_names = [
        "DioWellDATrials"
    ]  # pupt table depends on ppt which depends on DioWellDATrials
...
def get_smoothed_fr_table_map():
    ...
    smoothed_fr_temporal_event_table_map = (
        get_smoothed_fr_temporal_event_table_map()
    )
    ...
    return pd.concat(
        (smoothed_fr_spatial_table_map, smoothed_fr_temporal_event_table_map)
    )
...
def insert_firing_rate_map_unique_well_table(
    ...
    firing_rate_map_df = make_well_trial_table_fr_df_trial_subsets(
        ...
        trials_params_table=trials_params_table,
        firing_rate_map_params_table=firing_rate_map_params_table,
    ...
    for trial_feature_value in set(firing_rate_map_df[trial_feature_name]):
        firing_rate_map_df_subset = df_filter_columns(
            firing_rate_map_df, {trial_feature_name: trial_feature_value}
        )
        # Update key for table entry with trial feature value
        key.update({trial_feature_name: trial_feature_value})
        insert_analysis_table_entry(
            table, nwb_objects=[firing_rate_map_df_subset], key=key
        )


def insert_single_trial_firing_rate_map_smoothed_well_table(
    fr_smoothed_table, fr_table, params_table, key, data_type
):
    ...
    insert_analysis_table_entry(
        fr_smoothed_table, [smoothed_firing_rate_map_df], key, [object_id_name]
    )
