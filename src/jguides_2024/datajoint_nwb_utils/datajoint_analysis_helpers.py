...
def get_trial_info_by_condition(
    trial_type, group_trials_by, key, restrict_conditions_map
):
    ...
    trial_type_map = {
        "well_arrival": {
            "trial_table": DioWellArrivalTrials,
            "performance_outcomes_name": "performance_outcomes",
        },
        "path_traversals": {
            "trial_table": DioWellDATrials,
    ...
    if trial_type not in trial_type_map:
        raise Exception(
            f"trial_type must be in {list(trial_type_map.keys())} but is {trial_type}"
        )
    ...
    if restrict_conditions_map:
        ...
        if any(
            [
                restrict_name not in trial_df
                for restrict_name in restrict_conditions_map.keys()
            ]
        ):
            raise Exception(
    ...
    performance_outcomes_name = trial_type_map[trial_type][
        "performance_outcomes_name"
    ]
    performance_outcomes = np.asarray(trial_df[performance_outcomes_name])
    # ...Get trial times
    trial_times = np.asarray(
        list(zip_df_columns(trial_df, ["trial_start_times", "trial_end_times"]))
    )
    ...
    # ...Otherwise group by condition restriction
        ...
        performance_outcomes = [
            performance_outcomes[group_bool] for group_bool in group_bools
        ]
        return pd.DataFrame.from_dict(
            {
...
def get_plot_marker(subject_id=None, nwb_file_name=None):
    ...
    marker_map = dict_comprehension(
        get_ordered_subject_ids(), ["P", "s", "o", "p", "D"]
    )
...
def get_subject_id_shorthand(subject_id):
    subject_id_int_map = {
        subject_id: idx + 1
        for idx, subject_id in enumerate(get_ordered_subject_ids())
    }
...
def plot_task_phases(ax, axis_type):
    ...
    ylims = ax.get_ylim()
    y_start = ylims[1]
    y_extent = (ylims[1] - ylims[0]) * 0.1

    if axis_type == "path_progression":

        color = task_period_color_map["path traversal"]
        ax.add_patch(
            Rectangle((x_start, y_start), x_extent, y_extent, color=color)
        )
...
def key_text(key, separating_character="_"):
    ...
    if "nwb_file_name" in key:
        key.update(
            {"nwb_file_name": format_nwb_file_name(key["nwb_file_name"])}
        )
    ...
    x = [
        f"{prefix}{key[k]}" for k, prefix in key_prefix_map.items() if k in key
    ]
...
def get_relationship_alpha(relationship):
    ...
    alpha_map = {
        # PATH RELATIONSHIPS
        "same_path": alpha1,
        "different_path": alpha2,
        "same_turn": alpha1,
        "different_turn_well": alpha2,
        "outbound": alpha1,
        "inbound": alpha2,
        # WELL RELATIONSHIPS
        "same_well": alpha1,
        "different_well": alpha2,
    }
    check_membership(
        [relationship],
        alpha_map.keys(),
...
def get_reliability_paper_nwb_file_names(
    as_dict=False, restrict_dual_mPFC_OFC=False
):
    ...
    if restrict_dual_mPFC_OFC:
        paper_nwb_file_names_map = {
            k: v
            for k, v in paper_nwb_file_names_map.items()
            if k in ["J16", "mango", "june"]
        }
    if as_dict:
...
def plot_horizontal_lines(xlims, y_vals, ax):
    ...
    for y_val in np.unique(y_vals):
        ax.plot(xlims, [y_val] * 2, color="gray", alpha=0.3)


def get_task_period_color_map():
    return {
        "path traversal": "rosybrown",
        "delay": "burlywood",
        "post delay": "navajowhite",
    }
