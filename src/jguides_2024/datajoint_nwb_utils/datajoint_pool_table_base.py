...
class PoolCohortParamsBase(SecKeyParamsBase):
    ...
    def insert_single_member_cohort(self, param_name_dict):
        ...
        pool_table_entries = (
            self._get_pool_selection_table()().get_entries_with_param_name_dict(
                param_name_dict
            )
        )
        ...
        for entry in pool_table_entries:
            ...
            key = {
                k: entry[k] for k in get_non_param_name_primary_key_names(self)
            }
...
class EpsCohortParamsBase(SecKeyParamsBase):
    ...
    def _update_key(key):
        ...
        key.update(
            {
                "epochs_id": EpochCohortParams().lookup_epochs_id(
                    key["nwb_file_name"], key["epochs"]
                )
            }
        )
    ...
    def insert_single_member_cohort_defaults(self):
        ...
        candidate_keys = [
            self._update_key(key) for key in self._upstream_table().fetch("KEY")
        ]
