...
def _get_dfs(table_key):
    ...
    df["nwb_file_name_epochs_description"] = (
        get_nwb_file_name_epochs_description(
    ...
    additional_columns = [
        "label_train_test_order",
        "epoch_train_test_order",
        "total_num_train_trials",
        "total_num_test_trials",
        "num_train_trials_per_iteration",
...
def get_eps_labels(epochs_1, epochs_2, labels_1=None, labels_2=None):
    ...
    no_labels_none = all(
        [x is not None for x in np.concatenate((labels_1, labels_2))]
    )
...
class DecodeCovFRVecParamsBase(SecKeyParamsBase):
    ...
    def get_params(self):
        return self.fetch1(
            unpack_single_element(get_table_secondary_key_names(self))
        )
...
class DecodeCovFRVecSelBase(PopulationAnalysisSelBase):
    ...
    def _get_param_name_map(self, key_filter, brain_region_units_cohort_types):
        ...
        for (
            ...
            for (
                ...
                param_names = [
                    key[params_table.meta_param_name()]
                    for key in params_table_keys
                    if all(
                        [
                            (params_table & key).get_params()[x] == y
                            for x, y in zip(
                                [
                                    "boot_set_name",
                                    "brain_region_units_cohort_type",
                ...
                decode_cov_fr_vec_params_table = (
                    self._get_main_table()
                    ._upstream_table()()
                    ._get_params_table()()
                )
                decode_cov_fr_vec_meta_param_name = (
                    decode_cov_fr_vec_params_table.meta_param_name()
                )
                decode_cov_fr_vec_param_names = (
                    decode_cov_fr_vec_params_table.fetch(
                        decode_cov_fr_vec_meta_param_name
                    )
                )
                ...
                if boot_set_name in boot_set_name_param_name_map:
                    decode_cov_fr_vec_param_names = [
                        x
                        for x in decode_cov_fr_vec_param_names
                        if x in boot_set_name_param_name_map[boot_set_name]
                    ]

                for (
                    decode_cov_fr_vec_param_name
                ) in decode_cov_fr_vec_param_names:
    ...
    def _format_param_name_map_key(self, **kwargs):
        ...
        decode_cov_fr_vec_params_table = (
            self._get_main_table()._upstream_table()()._get_params_table()()
        )
        decode_cov_fr_vec_meta_param_name = (
            decode_cov_fr_vec_params_table.meta_param_name()
        )
        if decode_cov_fr_vec_meta_param_name not in kwargs:
            kwargs[decode_cov_fr_vec_meta_param_name] = kwargs["key"][
                decode_cov_fr_vec_meta_param_name
            ]
    ...
    def insert1(self, key, **kwargs):
        ...
        }  # just check for match at primary key
        ...
        if len(self & table_key) == 0:
            ...
            df_concat = (
                self._get_main_table()()
                ._upstream_table()()
                .get_concat_metric_df(key["upstream_keys"])
            )
            ...
            expected_empty_df_concat_keys = [
                {
                    ...
                    "recording_set_name": "fig_Haight_single_contingency_rotation",
                    "brain_region_cohort_name": "all_targeted",
                    "curation_set_name": "runs_analysis_v1",
                    "upstream_keys": [
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run1_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                            "res_epoch_spikes_sm_param_name": "0.1",
                            ...
                            "zscore_fr": 0,
                            "time_rel_wa_fr_vec_param_name": "stay_leave_trials_pre_departure",
                            "epochs_description": "run1",
                        },
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run2_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                            ...
                            "zscore_fr": 0,
                            "time_rel_wa_fr_vec_param_name": "stay_leave_trials_pre_departure",
                            "epochs_description": "run2",
                        },
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run3_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                            ...
                            "zscore_fr": 0,
                            "time_rel_wa_fr_vec_param_name": "stay_leave_trials_pre_departure",
                            "epochs_description": "run3",
                        },
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run4_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                            ...
                            "zscore_fr": 0,
                            "time_rel_wa_fr_vec_param_name": "stay_leave_trials_pre_departure",
                            "epochs_description": "run4",
                        },
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run5_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                            ...
                            "zscore_fr": 0,
                            "time_rel_wa_fr_vec_param_name": "stay_leave_trials_pre_departure",
                            "epochs_description": "run5",
                        },
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run6_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                            ...
                            "zscore_fr": 0,
                            "time_rel_wa_fr_vec_param_name": "stay_leave_trials_pre_departure",
                            "epochs_description": "run6",
                        },
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run7_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                            ...
                            "zscore_fr": 0,
                            "time_rel_wa_fr_vec_param_name": "stay_leave_trials_pre_departure",
                            "epochs_description": "run7",
                        },
                        {
                            "nwb_file_name": "fig20211108_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run8_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                            "time_rel_wa_dig_param_name": "0.25",
                            "time_rel_wa_dig_single_axis_param_name": "0^2",
                        ...
                        {
                            "nwb_file_name": "fig20211110_.nwb",
                            "brain_region": "OFC_targeted",
                            "brain_region_units_param_name": "0.1_run1_rand_target_region_50_0",
                            "curation_name": "raw data valid times no premaze no home_3",
                            "res_time_bins_pool_param_name": "ResEpochTimeBins#0.1#EpochInterval_no_comb",
                    ...
                    "decode_time_rel_wa_fr_vec_summ_param_name": "default^0.1_1_rand_target_region_50_single_iteration_0",
                },
            ]
            ...
            key = insert_analysis_table_entry(
                self, [df_concat], key, skip_insertion=True
            )
            ...
            key_subset = {
                ...
                if k not in ["analysis_file_name", "df_concat_object_id"]
            }
...
class DecodeCovFRVecBase(ComputedBase):
    ...
    def _get_vector_df_subset_quantities(
        self, vector_df, x, labels, decode_var
    ):
        ...
        if decode_var in [
            "path_progression",
            "path_progression_collapse_path",
            "time_in_delay",
        ]:
        ...
        return self._vector_df_subset_quantities(
            vectors, classes, epoch_trial_numbers, iloc_idxs
        )

    @staticmethod
    def _vector_df_subset_quantities(
        vectors, classes, epoch_trial_numbers, iloc_idxs
    ):
        return namedtuple(
            "VectorDfSubsetQuantities",
            "vectors classes epoch_trial_numbers iloc_idxs",
        )(vectors, classes, epoch_trial_numbers, iloc_idxs)
    ...
    def get_confusion_matrix(test_classes, predicted_classes):
        ...
        cmat = confusion_matrix(
            test_classes, predicted_classes, labels=classes, normalize="true"
        )
    ...
    def _get_train_test_data(self, epochs, key, vector_df, verbose=False):
        ...
        decode_cov_fr_vec_params_meta_name = (
            params_table_subset.meta_param_name()
        )
        decode_cov_fr_vec_params_name = params_table_subset.fetch1(
            decode_cov_fr_vec_params_meta_name
        )
        cov_fr_vec_meta_param_name = (
            self._fr_vec_table()()._get_params_table()().meta_param_name()
        )
        cov_fr_vec_param_name = decode_cov_fr_vec_params[
            cov_fr_vec_meta_param_name
        ]
        decode_var = decode_cov_fr_vec_params["decode_var"]
        classifier_name = decode_cov_fr_vec_params["classifier_name"]
        cross_validation_method = decode_cov_fr_vec_params[
            "cross_validation_method"
        ]
        cross_validation_always = decode_cov_fr_vec_params.pop(
            "cross_validation_always", False
        )
        ...
        if decode_var in [
            ...
            cov_fr_vec_params_table = (
                self._fr_vec_table()()._get_params_table()()
            )
            cov_fr_vec_meta_param_name = (
                cov_fr_vec_params_table.meta_param_name()
            )
            cov_fr_vec_params_table_subset = cov_fr_vec_params_table & {
            ...
            if decode_var in ["path", "previous_path"]:
                ...
                raise Exception(
                    f"Case for {decode_var} not covered in defining labels"
                )
            ...
            if decode_cov_fr_vec_params_name in [
                ...
                raise Exception(
                    f"Finding valid labels. {decode_cov_fr_vec_params_name} not accounted for"
                )
            ...
            if cross_validation_always or len(labels) == 1:
                ...
                for x in iterate_x_list:
                    ...
                    # Store
                    labels_1 = labels_2 = train_labels = test_labels = (
                        valid_labels
                    )
                    data_list.append(
                        (
                ...
                raise Exception(
                    f"case where decode var is {decode_var} and not cross validation is"
                    f" not accounted for"
                )
        ...
        # PATH NAME PAIR TYPES (E.G. SAME PATH, SAME TURN PATH)
            ...
            for epoch in epochs:
                path_name_pair_types_map.update(
                    MazePathWell.get_path_name_pair_types_map(
            ...
            if len(path_name_pair_types_map) == 0:
                raise Exception(
                    f"path_name_pair_types_map is empty. This is not expected. Check that "
            ...
            for (
                label_pair_name,
                label_pairs,
            ) in path_name_pair_types_map.items():
                ...
                for label_1, label_2 in label_pairs:
                    ...
                    label_train_test_orders = np.arange(
                        0, len(np.unique([label_1, label_2]))
                    )

                    # If leave one out cross validation in all cases, no need to reverse train/test labels
                    if (
                        cross_validation_always
                        and cross_validation_method == "loocv"
                    ):
                    ...
                    for label_train_test_order in label_train_test_orders:
                        ...
                        for epoch_train_test_order in epoch_train_test_orders:
                            ...
                            train_epoch, test_epoch = reverse_pair(
                                [epoch_1, epoch_2], reverse
                            )
                            ...
                            train_label, test_label = reverse_pair(
                                [label_1, label_2], reverse
                            )
                            ...
                            same_train_test_labels_epochs = np.logical_and(
                                ...
                                set(train_epochs) == set(test_epochs),
                            )
                            cross_validate = np.logical_or(
                                same_train_test_labels_epochs,
                                cross_validation_always,
                            )

                            # Define minimum number of classes required to proceed
                            min_num_classes = (
                                self._get_min_num_samples_train_test(
                                    classifier_name,
                                    cross_validation_method,
                                    cross_validate,
                                )
                            ...
                            for x in iterate_x_list:
                                ...
                                for label in [label_1, label_2]:
                                    ...
                                    if (
                                        len(set(df_subset.epoch_trial_number))
                                        < min_num_classes
                                    ):
                                ...
                                if insufficient_trials:
                                    if verbose:
                                        print(
                                            f"On x {x}. {label_1} {label_2} pair does not have sufficient trials. "
                                            f"Skipping..."
                                        )
    ...
    def get_cmat_df(self, key):
        ...
        if np.logical_or(
            table_name == "DecodeTimeRelWAFRVec" and decode_var == "well",
            table_name == "DecodePathFRVec"
            and decode_var == "destination_well",
        ):
            vector_df.label = [
                extract_from_path_name(x, "end_well_plus_descriptor")
                for x in vector_df.label
            ]
        elif decode_var in ["path_progression_collapse_path"]:
            vector_df.label = [
                extract_from_path_name(x, "descriptor") for x in vector_df.label
            ]
        # Do not alter
        elif decode_var in [
            "correct_incorrect",
            ...
            raise Exception(
                f"Altering labels. Case decode_var = {decode_var} not accounted for. "
                f"set(vector_df.label): {set(vector_df.label)}"
            )

        # Get train/test data to loop through
        train_test_df, num_possible_iterations, num_skipped_iterations = (
            self._get_train_test_data(epochs, key, vector_df)
        )
        ...
        for _, df_row in train_test_df.iterrows():
            ...
            if df_row.cross_validate:
                ...
                if cross_validation_method == "loocv":
                    ...
                    unique_epoch_trial_numbers = np.unique(
                        data.epoch_trial_numbers
                    )
                    ...
                    for epoch_trial_number in unique_epoch_trial_numbers:
                        ...
                        test_bool = (
                            data.epoch_trial_numbers == epoch_trial_number
                        )
            ...
            # test_label
                ...
                cmat, cmat_classes = self.get_confusion_matrix(
                    test_data.classes, predicted_labels
                )
            ...
            # ...Original
            original_eps_labels = get_eps_labels(
                df_row.epochs_1,
                df_row.epochs_2,
                df_row.labels_1,
                df_row.labels_2,
            ...
            data_list.append(
                (
                    df_row.x,
                    df_row.label_pair_name,
                    original_eps_labels,
                    eps_labels,
                    df_row.epoch_train_test_order,
                    df_row.label_train_test_order,
                    df_row.train_labels,
                    df_row.test_labels,
                    df_row.train_epochs,
                    df_row.test_epochs,
        ...
        if len(cmat_df) == 0:
            ...
            total_num_test_labels = len(
                np.concatenate(np.vstack(cmat_df.test_labels))
            )
        if (
            total_num_test_labels + num_skipped_iterations
            != num_possible_iterations
        ):
            raise Exception(
                f"cmat_df does not have expected length and number of test labels. total_num_test_labels: "
        ...
        for column_name in none_columns:
            if np.sum(cmat_df[column_name] == "none") > 0:
                raise Exception(
                    f"the string none already present in cmat_df columnn {column_name}"
                )
            cmat_df[column_name] = [
                "none" if x is None else x for x in cmat_df[column_name]
            ]
    ...
    def make(self, key):
        ...
        # nwb file
        cmat_df.drop(
            columns=["train_iloc_idxs", "test_iloc_idxs"], inplace=True
        )
        ...
        for column_name in self._char_column_names():
            if any(
                [isinstance(x, str) for y in cmat_df[column_name] for x in y]
            ):
    ...
    def fetch1_dataframe(
        ...
        # Get df
        cmat_df = super().fetch1_dataframe(
        ...
        for column_name in column_names:
            ...
            if np.sum(str_bool) not in [0, len(str_bool)]:
                raise Exception(
                    f"Expect {column_name} rows to have all strings or no strings"
                )
    ...
    def plot_ave_confusion_matrices(self):
        ...
        for x in x_list:
            for label_pair_name in label_pair_names:
                df_subset = df_filter_columns(
                    df, {"label_pair_name": label_pair_name, "x": x}
                )

                confusion_matrix_ave = np.mean(
                    np.stack(df_subset.confusion_matrix), axis=0
                )

                # Store
                data_list.append((x, label_pair_name, confusion_matrix_ave))

        cmat_ave_df = df_from_data_list(
            data_list, ["x", "label_pair_name", "confusion_matrix_ave"]
        )
        ...
        for label_pair_name_idx, label_pair_name in enumerate(label_pair_names):
            for x_idx, x in enumerate(x_list):
                ...
                plot_num += 1
                confusion_matrix_ave = df_pop(
                    cmat_ave_df,
                    {"x": x, "label_pair_name": label_pair_name},
                    "confusion_matrix_ave",
                )

                ax.imshow(confusion_matrix_ave)

                format_ax(
                    ax, title=label_pair_name.replace("_", " "), fontsize=6
                )
    ...
    def plot_performance(self, **kwargs):
        ...
        groupby_cols = [
            x for x in df.columns if x not in ave_across_column_names + ["val"]
        ]
        ...
        for label_pair_name in label_pair_names:
            vals = (
                df_filter_columns(df, {"label_pair_name": label_pair_name})
                .set_index("x")
                .sort_index()["val"]
            )
            ax.plot(vals)
    ...
    def fetch1_performance_df(
        ...
        if confusion_matrix_class_name is not None:
            ...
            for _, df_row in df.iterrows():
                ...
                for confusion_matrix_class, val in zip(
                    df_row.confusion_matrix_classes, df_row.val
                ):
                    data_list.append(cols + [confusion_matrix_class, val])

            df_column_names = keep_df_columns + [
                confusion_matrix_class_name,
                "val",
            ]
            df = df_from_data_list(data_list, df_column_names)
            ...
            raise Exception(f"{meta_param_name} {param_name} not accounted for")
    ...
    def get_concat_metric_df(self, keys, debug_mode=False):
        ...
        print(
            f"Concatenating table entries from {self.table_name} across {len(keys)} keys..."
        )
...
class DecodeCovFRVecSummBase(PathWellFRVecSummBase):
    ...
    def _get_relationship_div_column_params(self, **kwargs):
        ...
        decode_cov_fr_vec_params = (
            upstream_table._get_params_table() & kwargs["key"]
        ).get_params()
        cov_fr_vec_meta_param_name = (
        ...
        decode_meta_param_name = (
            upstream_table._get_params_table()().meta_param_name()
        )
        decode_param_name = (
            upstream_table._get_params_table()() & kwargs["key"]
        ).fetch1(decode_meta_param_name)
        ...
        if cov_fr_vec_param_name in [
            ...
            if decode_var in ["path_progression", "time_in_delay"]:
                params = {
                    "denominator_column_name": "same_path_correct_correct_stay_trials",
    ...
    def make(self, key):
        ...
        groupby_cols = [
            ...
            if x not in average_across_cols + ["val"]
        ]
        metric_df = (
            df_concat.groupby(groupby_cols)
            .mean()
            .reset_index()
            .drop(columns=average_across_cols)
        )
        ...
        # path traversals and delay period
            ...
            if np.logical_and(
                ...
                in RecordingSet().get_recording_set_names(
                    dict(), ["first_day_learning_single_epoch"]
                ),
                key["decode_path_fr_vec_param_name"]
                == "LDA_path_progression_loocv_correct_stay_trials",
            ):
    ...
    def _get_multiplot_params(**kwargs):
        ...
        decode_cov_fr_vec_param_names = kwargs.pop(
            "decode_cov_fr_vec_param_names"
        )
