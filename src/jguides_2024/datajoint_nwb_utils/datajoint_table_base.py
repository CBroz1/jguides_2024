...
class SecKeyParamsBase(ParamsBase):
    ...
    def _make_param_name(
        ...
        return make_param_name(
            secondary_key_subset_map.values(),
    ...
    def _get_secondary_key(self, secondary_key_values, param_name_subset):
        ...
        secondary_key_names_subset = (
            None  # default, corresponds to all secondary key values passed
        )
        ...
        return package_secondary_key(
            ...
            get_table_secondary_key_names(self, secondary_key_names_subset),
        )

    def _param_name_from_secondary_key_values(
        self, secondary_key_values, param_name_subset
    ):

        # Construct param name from table secondary keys (optionally a subset of these)
        secondary_key_subset_map = self._get_secondary_key(
            secondary_key_values, param_name_subset
        )
    ...
    def get_insert_argument(self, secondary_key_values):
        ...
        param_name = self._param_name_from_secondary_key_values(
            secondary_key_values, param_name_subset
        )
    ...
    def lookup_param_name(
        ...
        as_dict=False,
        args_as_dict=False,
        tolerate_irrelevant_args=False,
    ):
        """
        Look up param name
        ...
        if args_as_dict:
            ...
            if not tolerate_irrelevant_args:
                check_membership(
                    secondary_key_columns, list(secondary_key_values.keys())
                )
            secondary_key_values = [
                secondary_key_values[x] for x in secondary_key_columns
            ]
        ...
        if len(self & param_name_dict) == 0:
            raise Exception(
                f"param name not found in {get_table_name(self)} for {secondary_key_values}"
            )
    ...
    def get_default_param_name(self):
        return self.lookup_param_name(
            unpack_single_element(self._default_params())
        )
    ...
    def _get_param_name_table(self):
        ...
        return get_table(
            unpack_single_element(
                [x for x in get_upstream_table_names(self) if "ParamName" in x]
            )
        )
...
class SelBase(dj.Manual):
    ...
    def _get_potential_keys(self, key_filter=None):
        ...
        return intersect_tables(
            [get_table(x) for x in get_upstream_table_names(self)], key_filter
        ).fetch("KEY")
...
class PartBase(dj.Part):
    ...
    def fetch1_dataframe(
        ...
        return fetch1_dataframe(
            self, object_id_name, restore_empty_nwb_object, df_index_name
        )
    ...
    def get_key(self, key):
        return intersect_tables(
            [get_table(x) for x in get_upstream_table_names(self)], key
        ).fetch1("KEY")
...
class ComputedBase(dj.Computed):
    ...
    def _get_params_table(self):
        ...
        if params_table is None and self._get_selection_table() is not None:
            params_table_name = get_params_table_name(
                self
            )  # expected name of params table for current table
            if any(
                np.asarray(
                    get_upstream_table_names(self._get_selection_table())
                )
                == params_table_name
            ):
    ...
    def meta_param_name(self):
        ...
        if self._get_params_table() is None:
            raise Exception(
                f"table {self.table_name} has no params table, so cant get meta param name"
            )
        ...
        **kwargs,
        ...
        if recursive:
            for upstream_table_name in set(
                ...
                try:
                    ...
                    print(
                        f"could not get table {upstream_table_name} during recursive populate for "
                        f"{get_table_name(self)}. Error: {error}"
                    )
                    upstream_table = None
                if upstream_table is not None:
                    populated_tables += populate_insert(
                        upstream_table,
                        key=key,
                        tolerate_error=tolerate_error,
                        populated_tables=populated_tables,
                        populate_all_upstream=recursive,
                    )
        ...
        # Otherwise just insert default arguments into params and selection tables if they exist
            ...
            if params_table is not None:
                if verbose:
                    print(
                        f"inserting defaults into {get_table_name(params_table)}..."
                    )
            ...
            if selection_table is not None:
                if verbose:
                    print(
                        f"inserting defaults into {get_table_name(selection_table)}..."
                    )
...
class FrmapSmBase(ComputedBase):
    ...
    def make(self, key):
        ...
        smoothed_firing_rate_map_df = smooth_datajoint_table_fr(
            self._get_firing_rate_map_table(),
            self._get_params_table()(),
            key,
            self._data_type(),
        )
        insert_analysis_table_entry(
            self,
            [smoothed_firing_rate_map_df],
            key,
            [get_table_object_id_name(self)],
        )
...
class EventTrialsParamsBase(SecKeyParamsBase):
    ...
    def trial_duration(self, param_name):
        return trial_duration_from_params_table(
            self, self.meta_param_name(), param_name
        )
        ...
        if as_dict:
            return {
                ...
                for epoch_trial_number, trial_interval in zip(
                    trials_df.index, trial_intervals
                )
            }
...
class WellEventTrialsBaseExt(WellEventTrialsBase):
    ...
    def _event_info(
        ...
        end_event_name=None,
        start_event_idx_shift=None,
        end_event_idx_shift=None,
        handle_trial_start_after_end=False,
    ):
    ...
    def _well_trial_characteristics(self, remove_trial_start_end_text=False):
        ...
        characteristics = np.unique(
            [
                x
                for x in get_table_secondary_key_names(self)
                if _remove_trial_start_end_text(x)
                in get_table_secondary_key_names(get_table("DioWellTrials"))
            ]
        )
        ...
        if remove_trial_start_end_text:
            characteristics = [
                _remove_trial_start_end_text(x) for x in characteristics
            ]
    ...
    def make(self, key):
        ...
        start_time_shift, end_time_shift = (
            self._get_params_table()().trial_shifts(key[self.meta_param_name()])
        )
        ...
        trial_characteristics = self._well_trial_characteristics(
            remove_trial_start_end_text=True
        )
        key.update(
            (get_table("DioWellTrials") & key).get_trial_event_info(
                event_info.start_event_name,
                event_info.end_event_name,
                start_time_shift,
                end_time_shift,
                event_info.start_event_idx_shift,
                event_info.end_event_idx_shift,
                trial_characteristics,
                event_info.handle_trial_start_after_end,
            )
        )
        ...
        if "path_names" in get_table_secondary_key_names(self):
            ...
            key.update(
                {
                    "path_names": RewardWellPath().get_path_names(
                        key["trial_start_well_names"],
                        key["trial_end_well_names"],
                    )
                }
            )
    ...
    def rt_df_all_time(self, nwb_file_name, epoch, param_name, new_index=None):
        ...
        key = {
            ...
            "epoch": epoch,
            self.meta_param_name(): param_name,
        }
        ...
        if new_index is None:
            ...
            time_vector = fetch1_IntervalPositionInfo(
                nwb_file_name, epoch
            ).index

        # Get time relative to trial start time, within trials
        trial_start_time_shift, trial_end_time_shift = (
            self._get_params_table() & key
        ).trial_shifts()
        ...
        trials_valid_bools = [
            event_times_in_intervals_bool(time_vector, [trial_interval])
        ...
        extra_info_dict = {
            column_name: fill_trial_values(
                trials_df[column_name], trials_valid_bools
            )
            for column_name in self._well_trial_characteristics()
        }
        ...
        return pd.DataFrame.from_dict(
            {
                ...
                **{
                    ...
                    "relative_time_in_trial": relative_time_in_trial.values,
                },
            }
        ).set_index("time")
...
class ParamNameBase(dj.Manual):
    ...
    def _get_insertion_key(
        self, full_param_name, use_full_param_name=False, filter_key=None
    ):
        ...
        # Optional 2:
            ...
            int_id = fetch1_tolerate_no_entry(
                self & {"full_param_name": full_param_name}, "int_id"
            ...
            filter_text = "_".join(
                list(filter_key.values())
            )  # join params with underscore
            filter_text += "_" * (
                len(filter_text) > 0
            )  # add trailing underscore if joined filter key not empty
            param_name = filter_text + str(int_id)

        return {
            get_meta_param_name(self): param_name,
            "int_id": int_id,
    ...
    def lookup_param_name(self, full_param_name=None, as_dict=False, **kwargs):
        return self.get_insert_param_name(
            full_param_name, as_dict, insert=False, **kwargs
        )

    def get_insert_param_name(
        self,
        full_param_name=None,
        use_full_param_name=False,
        filter_key=None,
        as_dict=False,
        insert=True,
...
def get_upstream_table(table, target_string):
    ...
    table_names = [
        x for x in get_upstream_table_names(table) if target_string in x
    ]
...
def _print_table_object_id(table):
    object_id_name = get_table_object_id_name(
        table, tolerate_none=True, unpack_single_object_id=True
    )
...
def _check_table_object_ids(table):
    ...
    if object_id_name is not None:
        table_name = camel_to_snake_case(
            get_table_name(table), group_uppercase=True
        )
...
class CohortBase(ComputedBase):
    ...
    def _get_upstream_table_name(self):
        ...
        return unpack_single_element(
            [
                x
                for x in get_upstream_table_names(self.CohortEntries)
                if x != get_table_name(self)
            ]
        )
    ...
    def _fetch(self, iterable_name, fetch_function_name, **kwargs):
        ...
        return {
            key[iterable_name]: getattr(
                self._get_upstream_table() & key, fetch_function_name
            )(**kwargs)
            for key in keys
        }
    ...
    def fetch_dataframes(self, **kwargs):
        ...
        fetch_function_name = kwargs.pop(
            "fetch_function_name", "fetch1_dataframe"
        )
        ...
        if concatenate:
            ...
            if add_iterable:
                df_list = [
                    add_column_to_df(df, k, iterable_name)
...
class AcrossFRVecTypeTableSelBase(SelBase):
    ...
    def _get_potential_keys(self, key_filter=None):
        ...
        for shorthand_param_name in [
            "epoch_100ms",  # embeddings, firing rate vector analysis
            ...
            for idx, key in enumerate(
                (self._fr_vec_table() & key_filter).fetch("KEY")
            ):
                epoch = key.pop("epoch")
...
class CovariateDigParamsBase(SecKeyParamsBase):
    ...
    def bin_width_name(self):
        return unpack_single_element(
            [x for x in get_table_secondary_key_names(self) if "bin_width" in x]
        )

    def get_bin_centers(self, **kwargs):
        return vector_midpoints(
            self.make_bin_edges(self.fetch1(self.bin_width_name()), **kwargs)
        )
...
class CovDigmethBase(ComputedBase):
    ...
    def digitized(
        ...
        # Get covariate
        cov_df = self.fetch1_dataframe_exclude(
        ...
        cov_df[f"digitized_{covariate_name}"] = digitize_indexed_variable(
            indexed_variable=cov_df[covariate_name],
            bin_edges=bin_edges,
            right=True,
            verbose=verbose,
        )
