...
def populate_schema(
    ...
    print(
        f"populate_upstream_limit: {populate_upstream_limit}, populate_upstream_num: {populate_upstream_num}\n"
    )
    ...
    if (
        populate_upstream_num < populate_upstream_limit
        and upstream_schema_populate_fn_list is not None
    ):
        ...
        for populate_fn in upstream_schema_populate_fn_list:
            populate_fn(
                key, tolerate_error, populate_upstream_limit, upstream_num
            )
...
def check_schema_populated(schema_name, key):
    ...
    for table_name in get_schema_table_names_from_file(schema_name):
        ...
        if len(table & key) == 0:
            print(
                f"no entry in {table.table_name} for {key}. Returning False from for schema {schema_name} "
            )
            return False
