...
class CovariateFRVecParamsBase(SecKeyParamsBase):
    ...
    def get_labels_description(self):
        ...
        # Multiple covariate table
            ...
            raise Exception(
                "case not accounted for in getting labels description"
            )
    ...
    def update_labels(self, labels):
        ...
        if labels_description == "none":
            ...
            labels = np.concatenate(
                [
                    [
                        MazePathWell.get_low_speed_trial_label_name(
                            x, trial_text=trial_text
                        )
                        for x in labels
                    ]
...
class CovariateFRVecBase(ComputedBase):
    ...
    def concat_across_epochs(self, key, object_id_name):
        ...
        for epoch in (EpochsDescription & key).fetch1("epochs"):
            ...
            if bout_idxs_name is not None:
                bout_idxs_midpoints = np.mean(
                    np.vstack(df[bout_idxs_name]), axis=1
                )
                dio_well_trials_key = {
                ...
                epoch_trial_numbers = (
                    DioWellTrials & dio_well_trials_key
                ).get_time_epoch_trial_nums(bout_idxs_midpoints)
    ...
    def alter_input_labels_correct_incorrect(
        self, labels, key, previous_trial=False
    ):
        ...
        correct_incorrect_trial_path_names = [
            (
                MazePathWell().get_correct_incorrect_trial_text(
                    x,
                    MazePathWell().correct_incorrect_trial_text(
                        y, previous_trial
                    ),
                    previous_trial,
                )
    ...
    def get_keys_across_entries(self, **kwargs):
        ...
        unit_subset_iterations = kwargs.pop(
            "unit_subset_iterations", np.arange(0, 10)
        )
        # min epoch mean firing rate
        min_epoch_mean_firing_rate = kwargs.pop(
        ...
        key = copy.deepcopy(
            kwargs
        )  # define starting key as all remaining kwargs
        default_key = {
        ...
        recording_set_names_map = {
            subject_id: RecordingSet().get_Haight_single_contingency_rotation_set_name(
                [subject_id]
            )
        ...
        for subject_id, recording_set_name in recording_set_names_map.items():
            ...
            for nwb_file_name, epochs_descriptions_name in zip(
                ...
                if contingencies is not None:
                    ...
                    contingency_epochs = np.concatenate(
                        [
                            (
                                TaskIdentification
                                & {
                ...
                brain_regions = (BrainRegionCohort & key).fetch1(
                    "brain_regions"
                )
    ...
    def get_vector_df_across_entries(
        self, keys=None, use_multiprocessing=True, **kwargs
    ):
...
class CovariateFRVecAveSelBase(SelBase):
    ...
    def _get_cov_fr_vec_param_names(self):
        ...
        return set(
            self._fr_vec_table().fetch(self._get_cov_fr_vec_meta_param_name())
        )
        ...
        for idx, key in enumerate(keys):
            ...
            if all(
                ...
                for param_key in params_table.fetch("KEY"):
                    ...
                    if not check_same_values_at_shared_keys(
                        [key, {**params_1, **params_2}], tolerate_error=True
                    ):
                        continue
...
class CovariateFRVecAveBase(ComputedBase):
    ...
    def _get_label_relationship_inputs(
        ...
        rewarded = True
        pairs_map = {
            epoch: MazePathWell().get_path_name_pair_types_map(
                nwb_file_name,
                epoch,
                rewarded_paths=rewarded,
    ...
    def get_label_relationship_metric_map(
        self, relationships=None, verbose=False
    ):
    ...
    def get_labels_description(self, **kwargs):
        ...
        return (
            self.get_cov_fr_vec_params_table() & params_table_key
    ...
    def _get_df_outer_loop(
        ...
        **kwargs,
        ...
        for k in [
            ...
            "multi_cov_ave_fr_vec_param_name",
            "multi_cov_fr_vec_st_ave_param_name",
        ]:
            if k in kwargs:
            ...
            **kwargs,
            ...
            for _, df_row in df.iterrows():
                ...
                x_vals = table_subset._metric_index_as_bin_centers(
                    vals.index, table_subset.fetch1("KEY")
                )
    ...
    def get_same_diff_x_ratio_df(
        ...
        valid_labels = (
            self._fr_vec_table()()._get_params_table()() & key
        ).update_labels(valid_labels)
    ...
    def get_df_across_entries(
        self,
        keys=None,
        fn=_get_df,
        store_key_names=None,
    ...
    def plot_x_relationship_metric_arr(
        ...
        **format_kwargs,
        ...
        x_vals = self._metric_index_as_bin_centers(
            metric_arr.index, key=self.fetch1("KEY")
        ).values
    ...
    def _metric_index_as_bin_centers(self, metric_index, key=None):
        ...
        bin_centers_map = (self._fr_vec_table() & key).get_bin_centers_map()[
            bin_centers_name
        ]
    ...
    def exclude_invalid_bins(self, df):
        ...
        valid_bin_nums = self._fr_vec_table()().get_valid_covariate_bin_nums(
            self.fetch1("KEY")
        )
            ...
            for (val_index, val), x_val in zip(vals.items(), x_vals):
                data_list.append(
                    (
                        ...
                        x_val,
                        val,
                        key["brain_region_units_param_name"],
                        df_row.relationship,
                        df_row.eps_labels,
                        nwb_file_name_epochs_description,
                    )
    ...
    def get_default_relationships(self, **kwargs):
        ...
        if labels_description == "even_odd_trials":
            ...
            # By end well
                ...
                return [
                    "same_end_well_even_odd_correct_stay_trials",
                    "different_end_well_even_odd_correct_stay_trials",
                ]
...
class PathWellPopSummBase(ComputedBase):
    ...
    def get_paired_metric_df(
        ...
        data_list = []
        print(
            "Making new df with difference in values of brain region pairs..."
        )
    ...
    def plot_results(self, **kwargs):
        ...
        for condition_key in condition_keys:
            ...
            if show_single_epoch:
                ...
                metric_epoch_df = (
                    metric_df.groupby(epoch_group_by_cols)
        ...
        if (
            ...
            if len(df_subset) > 0:
                ...
                if (
                    ...
                    choice_point_fraction = (
                        get_n_junction_path_junction_fractions(2)[0]
                    )
                    num_bonferroni_trials = np.sum(
                        np.unique(df_subset.x_val) < choice_point_fraction
                    )
                ...
                for sig_val, color, y_scale_factor in zip(
                    ...
                    df_key = {
                        "alpha": sig_val,
                        "joint_brain_region": unpack_single_element(
                            brain_region_vals
                        ),
                    }

                    if relationship_vals is not None:
                        relationship_val = unpack_single_element(
                            relationship_vals
                        )
                        df_key.update(
                            {relationship_meta_name: relationship_val}
                        )

                    ave_conf_df_subset = (
                        df_filter_columns(df_subset, df_key)
                        .set_index("x_val")
                        .sort_index()
                    )
                    ...
                    if plot_sig_type == "bars":
                        ...
                        significant_idxs = np.where(
                            ave_conf_df_subset.significant
                        )[0]
                        significant_spans, _ = find_spans_increasing_list(
                            significant_idxs
                        )
                        ...
                        for start_idx, stop_idx in significant_spans:
                            ...
                            if start_idx == 0:
                                ...
                                x_start = (
                                    ave_conf_df_subset.index[start_idx]
                                    - (
                                        ave_conf_df_subset.index[start_idx]
                                        - ave_conf_df_subset.index[
                                            start_idx - 1
                                        ]
                                    )
                                    / 10
                            ...
                            if stop_idx == len(ave_conf_df_subset) - 1:
                                ...
                                x_stop = (
                                    ave_conf_df_subset.index[stop_idx]
                                    - (
                                        ave_conf_df_subset.index[stop_idx]
                                        - ave_conf_df_subset.index[stop_idx + 1]
                                    )
                                    / 10
                                )
                            ...
                            significant_spans_list.append(
                                ave_conf_df_subset.index[start_idx]
                            )

                            ax.plot(
                                x_vals, [y_val] * 2, color=color, linewidth=1.5
                            )
        ...
        if "plot_horizontal_line" in kwargs:
            plot_spanning_line(
                ax.get_xlim(), kwargs["plot_horizontal_line"], ax, "x"
            )
    ...
    def get_default_plot_params(**kwargs):
        ...
        if smaller_plot:
            mega_row_gap_factor = 0.2
        # ...columns
        mega_column_gap_factor = 0.12
        if smaller_plot:
            mega_column_gap_factor = 0.07
        wspace = 0.15
        hspace = 0.05
    ...
    def initialize_multiplot(
        ...
        **params,
        ...
        if params["row_iterable_name"] is not None:
            raise Exception("must code this condition")
                ...
                ax_key.update(
                    {
                        ...
                        for k in [
                            "row_iterable_name",
                            "column_iterable_name",
                            "mega_column_iterable_name",
                        ]
                    }
                )
...
class PathWellFRVecSummBase(PathWellPopSummBase):
    ...
    def _get_multiplot_fig_name(self, brain_region_vals, keys, plot_name=""):
        ...
        if len(relationship_vals) > 0:
            num_relationships = np.unique(
                [len(key["relationship_vals"]) for key in keys]
            )
            if len(num_relationships) == 1:
                relationships_text = f"_rel{num_relationships[0]}"
            else:
                raise Exception(
                    "have not accounted for case with multiple numbers of relationships across keys"
                )
    ...
    def _get_boot_diff_params(
        ...
        print("Getting bootstrap difference parameters...")
...
class CovariateFRVecAveSummBase(PathWellFRVecSummBase):
    ...
    def make(self, key):
        ...
        # 4) Average difference of stay/stay values and stay/leave values across brain regions
            ...
            for idx, eps_labels in enumerate(eps_labels_list):
                ...
                nwb_file_name = df_filter_columns(
                    metric_df, {"eps_labels": eps_labels}
                )["nwb_file_name"].values[0]
                epoch_1, epoch_2, label_1, label_2 = (
        ...
        # 6) Average difference of (same outbound path correct - different outbound path correct) across brain regions
            ...
            num_samples_pre_choice_point = np.sum(
                np.unique(metric_df[vals_index_name]) < choice_point_fraction
            )
            new_alpha = 0.05 / num_samples_pre_choice_point
        ...
        # values across brain regions
            ...
            boot_set_name
            in params_table._valid_low_speed_non_low_speed_diff_boot_set_names()
            + params_table._valid_low_speed_non_low_speed_diff_brain_region_diff_boot_set_names()
        ):
            ...
            for idx, eps_labels in enumerate(eps_labels_list):
                ...
                label_1 = MazePathWell().split_non_low_speed_low_speed_trial_path_name(
                    label_1
                )
                label_2 = MazePathWell().split_non_low_speed_low_speed_trial_path_name(
                    label_2
                )
                split_eps_labels_map[eps_labels] = (
                    upstream_table._get_eps_labels(
            ...
            eps_labels_resample_col_name = (
                "eps_labels_no_non_low_speed_low_speed_info"
            )
            metric_df[eps_labels_resample_col_name] = [
                split_eps_labels_map[x] for x in metric_df["eps_labels"]
            ]
            ...
            if (
                boot_set_name
                in params_table._valid_low_speed_non_low_speed_diff_brain_region_diff_boot_set_names()
            ):
    ...
    def _get_eps_labels_ave_metric_df(self, metric_df, resample_quantity):
        ...
        print("Averaging across eps_labels in metric_df...")
        ...
        pool = mp.Pool(mp.cpu_count())
        data_list = pool.map(
            _average_across, [(metric_df, x) for x in column_sets]
        )
        pool.close()
        pool.join()  # waits until all processes done before running next line
...
class PopulationAnalysisParamsBase(ParamsBase):
    ...
    def get_boot_params(self, bonferroni_num_tests):
        return get_boot_params(
            self.get_params()["boot_set_name"],
            bonferroni_num_tests=bonferroni_num_tests,
        )
...
class PopulationAnalysisSecKeyParamsBase(
    SecKeyParamsBase, PopulationAnalysisParamsBase
):
    pass
    ...
    def _get_potential_keys(
        ...
        for brain_region_units_cohort_type in brain_region_units_cohort_types:
            ...
            for recording_set_name in recording_set_names:
                ...
                if (
                    "J1620210529" in recording_set_name
                    ...
                    if populated:
                        ...
                        if param_name_map_key in param_name_map:
                            for summ_param_name in param_name_map[
                                ...
                                if verbose:
                                    print(
                                        f"Adding keys including summary table param name: {summ_param_name}..."
                                    )
...
class CovariateFRVecAveSummSelBase(PopulationAnalysisSelBase):
    ...
    def insert1(self, key, **kwargs):
        ...
        if len(self & table_key) == 0:
            ...
            metric_processing_name = (
                self._get_params_table() & table_key
            ).fetch1("metric_processing_name")
            kwargs = {"metric_processing_name": metric_processing_name}
            df_concat = (
                self._get_main_table()()
                ._upstream_table()()
                .get_concat_metric_df(key["upstream_keys"], **kwargs)
            )
    ...
    def _get_param_name_map(self, key_filter, brain_region_units_cohort_types):
        ...
        for (
            ...
            boot_set_name,
            cov_fr_vec_param_name,
        ) in recording_set_names_boot_set_names_cov_fr_vec_param_names:
            for (
                metric_name,
                metric_processing_name,
                ...
                for (
                    ...
                    param_name_map_key = self._format_param_name_map_key(
                        cov_fr_vec_param_name=cov_fr_vec_param_name,
                        metric_name=metric_name,
                        recording_set_name=recording_set_name,
                        brain_region_units_cohort_type=brain_region_units_cohort_type,
                    )
    ...
    def _format_param_name_map_key(self, **kwargs):
        ...
        if "cov_fr_vec_param_name" not in kwargs:
            cov_fr_vec_params_table = (
                upstream_table._fr_vec_table()()._get_params_table()()
            )
            cov_fr_vec_meta_param_name = (
                cov_fr_vec_params_table.meta_param_name()
            )
            kwargs["cov_fr_vec_param_name"] = kwargs["key"][
                cov_fr_vec_meta_param_name
            ]
        if "metric_name" not in kwargs:
            upstream_params_table = upstream_table._get_params_table()()
            kwargs["metric_name"] = (
                upstream_params_table & kwargs["key"]
            ).fetch1("metric_name")
