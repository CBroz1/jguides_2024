...
class PlotSTFRMap:
    ...
    def _get_restrictions(self):
        ...
        fr_table_name_map = {
            "path": STFrmapPuptSm,
            "well_arrival": STFrmapWellArrivalSm,
        }
        # ...type of maze restriction
        restrict_maze_name_type_map = {
            "path": "path_name",
            "well_arrival": "well_name",
        }
        ...
        restrict_maze_names_map = {
            "path": MazePathWell.get_same_turn_path_names_across_epochs(
                self.nwb_file_name,
                self.epochs,
                rewarded_paths=self.rewarded_paths,
                collapse=True,
            ...
            "well_arrival": MazePathWell.get_well_names_across_epochs(
                self.nwb_file_name,
                self.epochs,
                rewarded_wells=self.rewarded_wells,
                well_order=self.well_order,
            ),
        }
        # ...performance outcome name
        performance_outcome_name_map = {
            "path": "trial_end_performance_outcomes",
            "well_arrival": "performance_outcome",
        }
        # ...xlabel
        xlabel_map = {
            "path": "Path fraction",
            "well_arrival": "Time from well arrival (s)",
        }
        Restriction = namedtuple(
            "Restriction",
            "restrict_time_period restrict_maze_name_type restrict_maze_name "
        ...
        return [
            Restriction(
                ...
                restrict_maze_name_type_map[restrict_time_period],
                restrict_maze_name,
                performance_outcome_name_map[restrict_time_period],
                fr_table_name_map[restrict_time_period],
                xlabel_map[restrict_time_period],
            )
            for restrict_time_period in self.restrict_time_periods
            for restrict_maze_name in restrict_maze_names_map[
                restrict_time_period
            ]
        ]

    def _get_fr_table_subset(
        self, key, epoch, restriction, populate_tables=False
    ):
        ...
        key.update(
            {
                "epoch": epoch,
                restriction.restrict_maze_name_type: restriction.restrict_maze_name,
            }
        )
        ...
        if len(table_subset) == 0:
            print(
                f"Warning! No entries found for fr_arr in table {restriction.fr_table} using key {key}. "
                f"Returning None"
            )
            return None
    ...
    def _get_fr_df(self, key, epoch, restriction, populate_tables):
        ...
        table_subset = self._get_fr_table_subset(
            key, epoch, restriction, populate_tables
        )
    ...
    def plot_single_trial_firing_rate_map(
        self,
        brain_regions=None,
        ...
        if firing_rate_bounds_type not in valid_firing_rate_bounds_type:
            raise Exception(
                f"firing_rate_bounds_type must be in {firing_rate_bounds_type} but is"
        ...
        if dio_well_arrival_trials_param_name is None:
            dio_well_arrival_trials_param_name = (
                DioWellArrivalTrialsParams().lookup_param_name([-2, 4])
            )
        ...
        brain_region_color_map = (
            BrainRegionColor().return_brain_region_color_map(brain_regions)
        )

        # Define epoch colors based on environment
        environment_color_map = EnvironmentColor().get_environment_color_map()
        epoch_colors = {
            epoch: environment_color_map[
                TaskIdentification().get_environment(self.nwb_file_name, epoch)
            ]
        ...
        brain_region_units_param_name = (
            BrainRegionUnitsParams().lookup_epochs_param_name(
        ...
        for unit_name in unit_names:
            ...
            _, ax_map, fig_ = get_gridspec_ax_maps(
                mega_row_iterables=self.epochs,
                mega_column_iterables=np.asarray([0]),
                row_iterables=row_iterables,
                column_iterables=np.arange(0, len(self.restrictions)),
                num_mega_rows_top=num_mega_rows_top,
                num_mega_rows_bottom=num_mega_rows_bottom,
                num_mega_columns_left=num_mega_columns_left,
                num_mega_columns_right=num_mega_columns_right,
                fig=fig,
                sharex="columns",
                sharey=sharey,
            ...
            for restriction_idx, restriction in enumerate(self.restrictions):
                for epoch_idx, epoch in enumerate(self.epochs):
                    ...
                    fr_list += list(
                        np.concatenate(
                            np.vstack(
                                df_filter_columns(fr_df, {"unit_id": unit_id})[
                                    "smoothed_rate_map"
                                ]
                            )
                        )
            ...
            for restriction_idx, restriction in enumerate(self.restrictions):
                ...
                for epoch_idx, epoch in enumerate(self.epochs):
                    ...
                    fr_df = self._get_fr_df(
                        key, epoch, restriction, populate_tables=False
                    )
                    if fr_df is None:
                        for row_iterable in row_iterables:
                            ax = ax_map[
                                (epoch, 0, row_iterable, restriction_idx)
                            ]
                            ax.axis("off")
                        continue
                    ...
                    if plot_performance_outcomes:
                        performance_outcome_colors = [
                            PerformanceOutcomeColors().get_performance_outcome_color(
                                x
                            )
                            for x in fr_df_subset[
                                restriction.performance_outcome_name
                            ]
                        ]
                        ax.scatter(
                            [performance_outcome_x_val] * len(fr_arr),
                            np.arange(0, len(fr_arr)) + 0.5,
                            color=performance_outcome_colors,
                            alpha=1,
                            s=5,
                        )
                    ...
                    if restriction_idx == 0:
                        yticklabels = [
                            int(x) for x in yticks
                        ]  # show trial number as int
                    ylim = ax.get_ylim()
                    format_ax(
                        ax=ax,
                        xticks=xticks,
                    ...
                    ax.spines["left"].set_bounds(
                        ylim
                    )  # set left spine to same range as data

                    # ...update maximum y lim
                    max_ylim_map[row_iterable] = np.max(
                        [max_ylim_map[row_iterable], ax.get_ylim()[1]]
                    )
                    ...
                    if (
                        np.shape(fr_arr)[0] == 1
                    ):  # if only one trial, firing rate bounds are just mean firing rate
                    ...
                    table_subset = self._get_fr_table_subset(
                        key, epoch, restriction
                    )
                    bin_centers_name = table_subset.get_bin_centers_name()
                    x_vals = unpack_single_vector(
                        fr_df_subset[bin_centers_name].values
                    )
            ...
            for (epoch, _, row_iterable, restriction_idx), ax in ax_map.items():
                ...
                if (
                    restriction.restrict_time_period == "path"
                    and restriction.restrict_maze_name_type == "path_name"
                ):
                    valid_path_names = RewardWellPath().fetch1("path_names")
                    if restriction.restrict_maze_name not in valid_path_names:
                        raise Exception(
                            f"restrict_maze_name must be in {valid_path_names} to plot maze corners but is "
                ...
                # Well arrival and reward delivery if well arrival firing rate maps
                    ...
                    if row_iterable == "trial_fr":
                        ...
                        # sample number
                        fr_table_subset = self._get_fr_table_subset(
                            key, epoch, restriction, populate_tables=False
                        )
                ...
                y_vals = expand_interval(
                    [0, max_y], expand_factor=0
                )  # set y value a bit larger than y lims
            ...
            if plot_environment_marker:
                for (epoch, _, row_iterable, _), ax in ax_map.items():
                    max_y = (
                        _get_max_y(sharey, ax, max_ylim_map, row_iterable)
                        * 1.02
                    )
                    ax.plot(
                        ax.get_xlim(),
            ...
            if save_fig:
                file_name = (
                    f"fr_{format_nwb_file_name(self.nwb_file_name)}_{format_epochs(self.epochs)}"
                    f"_{unit_name}"
                )
                save_figure(fig_, file_name, save_fig=save_fig)


def order_units_by_trial_segments_fr(
    nwb_file_name,
    epoch,
    df_names=None,
    well_name=None,
    ...
    if dio_well_arrival_trials_param_name is None:
        dio_well_arrival_trials_param_name = (
            DioWellArrivalTrialsParams().lookup_param_name([0, 2])
        )
    if df_names is None:
    ...
    if (
        any(
            [
                x in df_names
                for x in ["FrmapWADSmWT", "FrmapUniqueWellArrivalSm"]
            ]
        )
    ...
    for (
        target_region,
        sort_group_ids,
    ) in targeted_location_sort_group_map.items():
        for sort_group_id in sort_group_ids:
            ...
            for df_name in df_names:
                if df_name == "FrmapPuptSm":
                    ...
                    df_list.append(
                        (FrmapUniqueWellArrivalSm & key).fetch1_dataframe()
                    )
                    ...
                    df_list.append(
                        df_filter_columns(
                            (FrmapWADSmWT & key).fetch1_dataframe(),
                            {"well_name": well_name},
                        ).sort_index()
                    )
            ...
            rate_maps_list = [
                np.vstack(getattr(df, "smoothed_rate_map")) for df in df_list
            ]  # concatenate rate maps
            ...
            if verbose:
                rate_map_bounds = np.cumsum(
                    [np.shape(x)[1] for x in rate_maps_list]
                )
    ...
    peak_idxs_df = df_from_data_list(
        data_list, ["sort_group_id", "unit_id", "peak_idx"]
    )
...
def _sort_peak_idxs_df(peak_idxs_df):
    return peak_idxs_df.sort_values(
        by=["peak_idx", "sort_group_id", "unit_id"],
...
def _sorted_units(peak_idxs_df):
    ...
    return list(
        zip_df_columns(peak_idxs_df, ["sort_group_id", "unit_id"])
    )  # [(sg_1, unit_1), (sg_1, unit_2), ...]
