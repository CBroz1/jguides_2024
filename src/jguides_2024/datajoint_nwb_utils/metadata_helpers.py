...
def get_all_subject_id_recording_dates():
    ...
    return {
        "peanut": [str(x) for x in np.arange(20201101, 20201120)],
        "J16": [
            str(x)
            for x in np.concatenate(
                (np.arange(20210529, 20210532), np.arange(20210601, 20210621))
            )
        ],
...
def get_jguidera_nwbf_names(
    high_priority=True, highest_priority=False, as_dict=False
):
    ...
    nwbf_names = [
        nwbf_name_from_subject_id_date(subject_id, date)
    ...
    if highest_priority:
        nwbf_names = [
            x for x in get_highest_priority_nwb_file_names() if x in nwbf_names
        ]
    if high_priority:
        nwbf_names = [
            x for x in get_high_priority_nwb_file_names() if x in nwbf_names
        ]
...
def get_recording_spreadsheet_path(subject_id):
    ...
    return (
        f"/media/jguidera/5fa57296-2b8d-4c4f-90e7-7edf05167424/Frank Lab Dropbox/Jennifer Guidera/UCSF_PhD/"
...
def load_recording_notes(
    subject_id, recording_spreadsheet_path=None, tolerate_no_notes=False
):
    """
    ...
    return _load_notes(
        subject_id,
        spreadsheet_name=f"{subject_id}_experimentalPlan - Recordings.csv",
...
def get_video_files(path=None):
    ...
    return np.sort(
        [
            ...
            if any([y in x for y in video_file_endings])
        ]
    )
...
class Contingency:
    ...
    def check_valid_contingency(self, contingency):
        ...
        if contingency not in np.concatenate(
            list(self.valid_contingencies.values())
        ):
            raise Exception(
                f"Contingency {contingency} not accounted for in code."
            )
    ...
    def get_contingency_type(self, contingency):
        ...
        return unpack_single_element(
            [
                contingency_type
                for contingency_type, contingencies in self.valid_contingencies.items()
                if contingency in contingencies
            ]
        )
