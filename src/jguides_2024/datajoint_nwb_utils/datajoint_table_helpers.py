...
def fetch_nwb(table, **kwargs):
    ...
    return fetch_nwb_(
        table, (AnalysisNwbfile, "analysis_file_abs_path"), **kwargs
    )
...
def fetch1_dataframe(
    ...
    if object_id_name is None:
        object_id_name = get_table_object_id_name(
            table, leave_out_object_id=True
        )
    ...
    if len(entries) != 1:
        raise Exception(
            f"Should have found exactly one entry but found {len(entries)}"
        )
...
def fetch1_dataframe_from_table_entry(
    table, strip_s=False, index_column=None, column_subset=None
):
    ...
    return pd.DataFrame(
        {
            ...
            for k, v in zip(
                table_column_names,
                list(
                    zip(*fetch_iterable_array(table_subset, table_column_names))
                ),
            )
        }
    )
...
def fetch1_dataframes(table):
    ...
    return namedtuple(
        "dfs", " ".join(table.get_object_id_name(leave_out_object_id=True))
    )(
        *[
            table.fetch1_dataframe(object_id_name=object_id_name)
            for object_id_name in table.get_object_id_name(
                leave_out_object_id=True
            )
        ]
    )


def check_no_attribute_duplicates(table_subset, attribute):
    """
...
def get_pos_valid_times_interval_names(nwb_file_name):
    ...
    for n in (IntervalList & {"nwb_file_name": nwb_file_name}).fetch(
        "interval_list_name"
    ):
        if len(np.asarray(n.split(" "))) == 4:
            if all(
                np.asarray(n.split(" "))[[0, 2, 3]]
                == np.asarray(["pos", "valid", "times"])
            ):
...
def check_nwb_file_name(nwb_file_name):
    ...
    if nwb_file_name not in get_jguidera_nwbf_names(
...
def fetch_entries_as_dict(table, column_names=None):
    ...
    return [
        {k: v for k, v in zip(column_names, table_entry_values)}
        for table_entry_values in fetch_iterable_array(table, column_names)
    ]
...
def check_attributes_in_key(necessary_attributes, key):
    ...
    missing_attributes = [
        attribute not in key.keys() for attribute in necessary_attributes
    ]  # boolean indicating missing attributes in above list

    if any(missing_attributes):  # ensure required attributes in key
        raise Exception(
            f"Attributes {necessary_attributes[missing_attributes]} missing from passed key"
        )
...
def get_table_object_id_name(
    table,
    leave_out_object_id=False,
    unpack_single_object_id=True,
    tolerate_none=False,
):

    object_id_names = [
        x for x in get_table_column_names(table) if "object_id" in x
    ]
...
def get_table_key_names(table, secondary_key_names_subset=None):
    ...
    return list(table.primary_key) + get_table_secondary_key_names(
        table, secondary_key_names_subset
    )
...
def get_table_secondary_key_names(table, secondary_key_names_subset=None):
    ...
    secondary_key_names = [
        k for k in get_table_column_names(table) if k not in table.primary_key
    ]
    ...
    if secondary_key_names_subset is not None:
        check_membership(
            secondary_key_names_subset,
            secondary_key_names,
            "passed secondary key names",
...
def fetch_iterable_array(table, attributes):
    ...
    # Return as matrix
    return np.reshape(
        np.asarray((table).fetch(*attributes), dtype=object).T,
        (-1, len(attributes)),
    )
...
def make_param_name(
    ...
    if (
        any([separating_character in x for x in param_values])
        and not tolerate_non_unique
    ):
        raise Exception(
            f"separating character ({separating_character}) found in at least one param values; this is "
            f"not allowed"
        )
...
def make_params_string(param_values):
    ...
    return make_param_name(
        param_values, separating_character="_", tolerate_non_unique=True
    )
...
def plot_datajoint_table_rate_map(fr_table, key, ax, color, label=None):
    ...
    def _column_name(column_keyword, fr_table, key):
        return unpack_single_element(
            [
                column_name
                for column_name in (fr_table & key).fetch1_dataframe().columns
                if column_keyword in column_name
            ]
        )

    check_attributes_in_key(
        necessary_attributes=np.asarray(
            ["nwb_file_name", "epoch", "sort_group_id", "unit_id"]
        ),
        key=key,
    )  # require certain attributes passed
    ...
    # Plot
    ax.plot(
        np.asarray(fr_df.loc[key["unit_id"]][bin_centers_name]),
        np.asarray(fr_df.loc[key["unit_id"]][rate_map_name]),
        ".-",
        color=color,
        label=label,
    )
...
def handle_empty_nwb_object(nwb_object, from_empty):
    ...
    if isinstance(nwb_object, pd.DataFrame):
        ...
        if from_empty and len(nwb_object) == 0:  # convert empty df
            ...
            nwb_object[empty_df_tag] = [
                np.nan
            ]  # add flag so can know that empty df
        # TO EMPTY DF
...
def insert_analysis_table_entry(
    ...
    if reset_index:
        nwb_objects = [
            x.reset_index() if isinstance(x, pd.DataFrame) else x
            for x in nwb_objects
        ]
...
def trial_duration_from_params_table(trials_table, column_name, param_name):
    ...
    trial_start_time_shift, trial_end_time_shift = (
        trials_table & {column_name: param_name}
    ).fetch1("trial_start_time_shift", "trial_end_time_shift")
...
def abbreviate_path_name(path_name):
    ...
    return "-".join(
        [x[0].upper() for x in format_path_name(path_name).split("-")]
    )
...
def abbreviate_environment(environment):
    ...
    if environment not in valid_environments:
        raise Exception(
            f"environment must be in {valid_environments} but is {environment}"
        )
    return "".join(
        [x[0].upper() for x in camel_to_snake_case(environment).split("_")]
    )
...
def delete_(main_table, dependent_tables, key, safemode=True):
    ...
    print(
        f"deleting entries from {get_table_name(main_table)} and listed dependent tables for key: {key}"
    )
...
def delete_flexible_key(table, key=None, safemode=True):
    ...
    if key is None:
        ...
        (table & {k: key[k] for k in table.primary_key if k in key}).delete(
            safemode=safemode
        )
...
def populate_multiple_flexible_key(
    tables, key=None, tolerate_error=False, error_message=None
):
...
def split_unit_names(formated_sort_group_unit_ids):
    ...
    return list(
        zip(*[split_unit_name(x) for x in formated_sort_group_unit_ids])
    )
...
def get_next_int_id(table):
    ...
    if all(
        [x is None for x in int_ids]
    ):  # returns True both when no int_ids and when all int_ids are None
        return 0
    # Otherwise, define next int_id as one more than last number in stretch of consecutive integers from zero
    int_ids = list(
        map(int, [x for x in int_ids if x is not None])
    )  # convert to int in case string datatype
    last_int = find_spans_increasing_list(np.sort(int_ids), max_diff=1)[0][0][
        -1
    ]
...
def check_epochs_id(epochs_id, epochs):
    ...
    if expected_epochs_id != epochs_id:
        raise Exception(
            f"Passed epochs correspond to epochs_id of {expected_epochs_id}, but {epochs_id} was passed"
        )
...
def convert_long_table_name(long_table_name):
    ...
    x = strip_string(
        long_table_name.split(".")[-1], "`", strip_start=True, strip_end=True
    )
...
def get_upstream_table_names(table):
    ...
    return [
        convert_long_table_name(table_name) for table_name in table.parents()
    ]
...
def get_downstream_table_names(table):
    ...
    return [
        convert_long_table_name(table_name) for table_name in table.children()
    ]
...
def is_part_table_name(long_table_name):
    ...
    if (
        x[:2] == "__"
    ):  # remove first dunder, which I believe indicates imported table
        x = x[2:]
...
def get_parent_table_names(table_names):
    ...
    return [
        get_parent_table_name_from_part_table_name(x)
        for x in get_part_table_names(table_names)
    ]
...
def insert_manual_table_test_entry(table):
    ...
    if len(potential_keys) == 0:
        print(
            f"No potential entries from which to insert test entry for table "
...
def table_name_from_table_type(table_name, table_type):
    ...
    if len(split_table_name) not in [1, 2]:
        raise Exception(
            f"table name empty or has {table_type} appearing more than once. table name: {table_name}"
        )
...
def get_main_table_name(table_name):
    ...
    for fn in [
        table_name_from_selection_table_name,
        table_name_from_param_name_table_name,
...
def get_meta_param_name(table, verbose=False):
    ...
    try:
        ...
        passed_check = check_membership(
            [param_name],
            table.primary_key,
            "list with expected param name",
        ...
        # Case 2
        if not passed_check:
            param_name = unpack_single_element(
                [x for x in table.primary_key if "param_name" in x],
        ...
        if verbose and non_preferred_param_name:
            print(
                f"warning! non preferred param name for {get_table_name(table)}: {param_name}"
            )
...
def package_secondary_key(
    table, secondary_key_values, secondary_key_names_subset=None
):
    secondary_key_names = get_table_secondary_key_names(
        table, secondary_key_names_subset
    )
...
def get_entry_secondary_key(table_subset):
    ...
    secondary_key_values = [
        table_subset.fetch1(k)
        for k in get_table_secondary_key_names(table_subset)
    ]
...
def get_entry_primary_key(table_subset):
    ...
    return {
        k: v for k, v in table_subset.fetch1() if k in table_subset.primary_key
    }
...
def check_single_table_entry(table, key=None):
    ...
    if num_table_entries != 1:
        raise Exception(
            f"Exactly one entry should have been found in {get_table_name(table)} for key, but "
            f"{num_table_entries} were. Key: {key}"
        )


def unique_table_column_sets(
    table, column_names=None, key_filter=None, as_dict=False
):
    ...
    if as_dict:
        return [
            {k: v for k, v in zip(column_names, unique_column_set)}
            for unique_column_set in unique_column_sets
        ]
...
def get_cohort_test_entry(table, col_vary, num_entries, verbose=True):
    ...
    if verbose and target_vals is None:
        print(
            f"Could not find cohort table test entry based on {get_table_name(table)} "
...
def narrow_candidate_keys(table, candidate_keys):
    ...
    return np.asarray(candidate_keys)[
        valid_candidate_keys_bool(table, candidate_keys)
    ]
...
def valid_candidate_keys_bool(table, candidate_keys):
    ...
    return [
        idx
        for idx, candidate_key in enumerate(candidate_keys)
        if len(table & {k: candidate_key[k] for k in table.primary_key}) == 0
    ]
...
def get_schema_table_names(schema=None, schema_name=None):
    ...
    return np.unique(
        [
            snake_to_camel_case(
                get_string_prior_to_dunder(remove_leading_dunder(x))
            )
...
def get_schema_table_names_from_file(schema_name, schema_path=None):
    ...
    file_contents = np.asarray(
        get_file_contents(
            f"{schema_name.replace('.', '/')}.py", schema_path
        ).split("\n")
    )  # split lines
    ...
    table_idxs = [
        idx + 1
        for idx, x in enumerate(file_contents)
        if "@schema" in x and x[0] != "#"
    ]
...
def get_module_subdirs():
    ...
    return [
        ...
        if not x.startswith("_") and os.path.isdir(os.path.join(module_path, x))
    ]
...
def get_schema_names():
    ...
    schema_names = [
        f"{module_dir.replace('/', '.')}.{child_dir}.{x}".replace("..", ".")
...
def get_jguidera_schema_dir(schema_name):
    ...
    return unpack_single_element(
        [
            os.path.join(module_path, child_dir)
            for child_dir in subdirs
            for x in os.listdir(f"{module_path}/{child_dir}")
            if x == f"{schema_name}.py"
        ]
    )
    ...
    for schema_name in schema_names:
        table_names_str = ", ".join(
            get_schema_table_names_from_file(
                schema_name, schema_path=get_project_dir()
            )
        )
...
def add_param_defaults(
    key, target_keys=None, add_nonexistent_keys=False, replace_none=True
):
    ...
    if target_keys is not None:
        param_defaults = {
            k: v for k, v in param_defaults.items() if k in target_keys
        }
...
def get_curation_name(sort_interval_name, curation_id):
    return make_param_name(
        [sort_interval_name, curation_id], "_", tolerate_non_unique=True
    )
...
def populate_insert(table, **kwargs):
    ...
    if hasattr(table, "insert_defaults"):
        ...
        table().insert_defaults(
            key_filter=copy.deepcopy(key_filter)
        )  # copy to avoid changing outside function
...
def extract_from_path_name(path_name, extract_name=False, verbose=False):
    ...
    if len(path_name_tuple) != 2:
        raise Exception(
            f"path_name should have exactly one '_to_' but has {len(path_name_tuple)}"
        )
    ...
    check_membership(
        [end_well_name],
        (
            RewardWell & {"universal_track_graph_name": "fork_maze_universal"}
        ).fetch1("well_names"),
    )
...
def make_table_param_names(table, secondary_key_splits):
    ...
    check_membership(
        np.ndarray.flatten(np.asarray(secondary_key_splits)),
        secondary_key,
        "secondary key split members",
        f"{get_table_name(table)} secondary key",
    )

    # Return param name for each split
    return [
        table._make_insert_param_name({k: secondary_key[k] for k in x})
        for x in secondary_key_splits
    ]
...
def get_key(table_subset, key):
    ...
    if key is None:
        if len(table_subset) != 1:
            raise Exception(
                f"Trying to get cohort params for members of a cohort in ResTimeBinsPoolCohortParams. "
...
def add_upstream_res_set_params(kwargs):
    ...
    if key is not None:
        ...
        if (
            "res_time_bins_pool_cohort_param_name" in key
            and "res_time_bins_pool_param_name" not in key
        ):
            table_subset = ResTimeBinsPoolCohortParams & key
            if len(table_subset) == 1:  # one cohort represented by key
                res_time_bins_pool_param_names = table_subset.fetch1(
        ...
        if (
            "res_time_bins_pool_param_name" in key
            and "res_set_param_name" not in key
        ):
            try:
                ...
                res_set_param_name = (
                    ResTimeBinsPoolSel & key
                ).get_res_set_param_name()
...
def fetch1_dataframes_across_epochs(table, key, axis=0):
    ...
    dfs_concat = {
        object_id_name: pd.concat(
            [getattr(dfs_obj, object_id_name) for dfs_obj in dfs_objs],
            axis=axis,
        )
            ...
            "epoch": np.concatenate(
                [
                    ...
                    * check_return_single_element(
                        [
                            len(getattr(dfs_obj, object_id_name))
                            for object_id_name in object_id_names
                        ]
                    ).single_element
...
def restore_df_row_idx(
    df, column_name, idx_column_name=None, drop_idx_column=True
):
    ...
    df[column_name] = [
        pd.Series(x, index=y)
        for x, y in zip_df_columns(df, [column_name, idx_column_name])
    ]
...
def scrappy_clean(sel_table, main_table, safemode=True):
    ...
    for key in keys:
        if len(main_table & key) == 0:
            bad_keys.append(key)
    print(
        f"Found {len(bad_keys)} keys of {len(keys)} in {get_table_name(sel_table)} but not main table. Deleting..."
    )
    for key in bad_keys:
...
def get_table_curation_names_for_key(table, key):
    ...
    if "curation_name" in key:
        ...
        if "sort_interval_name" in key and "curation_id" in key:
            return [
                get_curation_name(key["sort_interval_name"], key["curation_id"])
            ]
        if "sort_interval_name" in key:
            return [
                curation_name
                for curation_name in set(table.fetch("curation_name"))
                if split_curation_name(curation_name)[0]
        ...
        if "curation_id" in key:
            return [
                curation_name
                for curation_name in set(table.fetch("curation_name"))
                if split_curation_name(curation_name)[1] == key["curation_id"]
            ]
        else:
            return np.unique(
                (EpochSpikeTimesRelabel & key).fetch("curation_name")
            )
...
def get_boot_params(boot_set_name, bonferroni_num_tests=None):
    ...
    if boot_set_name in [
        "default",
        "default_rat_cohort",
        "collapse_maze_segments",
        "collapse_maze_segments_rat_cohort",
        "brain_region_diff",
        ...
        "same_different_outbound_path_correct_diff",
...
class UpstreamEntries:
    ...
    def update_upstream_entries(self, table, key):
        ...
        table_name = get_table_name(table)
        upstream_table_key = return_table_key(
            table, key, tolerate_missing_columns=True
        )
    ...
    def merge_tracker(self, table):
        ...
        self.upstream_entries = merge_dicts_lists(
            [self.upstream_entries, table.upstream_obj.upstream_entries]
        )

    def remove_repeat_entries(self):
        return {
            k: remove_repeat_dicts(v) for k, v in self.upstream_entries.items()
        }
