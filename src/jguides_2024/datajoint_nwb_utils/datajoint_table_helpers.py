...
def populate_flexible_key(
    table, key=None, tolerate_error=False, error_message=None, verbose=False
):
...
def get_schema_table_names_from_file(schema_name, schema_path=None):
    ...
    file_contents = np.asarray(
        get_file_contents(f"{schema_name.replace('.', '/')}.py").split("\n")
    )  # split lines
...
def split_curation_name(curation_name):
    ...
    if len(split_curation_name) > 2:
        raise Exception("curation_name should have a single underscore")
