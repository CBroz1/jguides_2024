...
def get_epoch_timestamps_nwbf(
    ...
    expected_sampling_rates=[20000, 30000],
    error_tolerance_expected_vs_estimated_fs=0.5,
    tolerance_distance_to_epoch_bounds=0.002,
):
    """
    ...
    def _get_ptp_times(nwbf, start_idx, end_idx):
        return nwbf.fields["processing"]["sample_count"].data_interfaces[
            "sample_count"
        ].fields["timestamps"][start_idx : end_idx + 1] / (
            10**9
    ...
    num_samples_estimation = np.min(
        [
            ...
            len(nwbf.fields["acquisition"]["e-series"].fields["timestamps"]),
        ]
    )
    estimated_fs = 1 / np.mean(
        np.diff(
            nwbf.fields["acquisition"]["e-series"].fields["timestamps"][
                -num_samples_estimation:
            ]
        )
    )
    ...
    if not any(
        [
            expected_fs - estimated_fs
            < error_tolerance_expected_vs_estimated_fs
            for expected_fs in expected_sampling_rates
        ]
    ):
        raise Exception(
    ...
    nth_sample = int(
        np.round(estimated_fs)
    )  # to sparsify samples, take every nth_sample
    sparse_timestamps = nwbf.fields["processing"][
        "sample_count"
    ].data_interfaces["sample_count"].fields["timestamps"][::nth_sample] / (
        10**9
    )  # note that do not index into parent array using above idxs because slow

    # Define indices in full sample count data corresponding to sparse timestamps
    idxs_parent_array = np.arange(
        0,
        len(
            nwbf.fields["processing"]["sample_count"]
    ...
    if final_timestamp not in sparse_timestamps:
        sparse_timestamps = np.append(
            sparse_timestamps, final_timestamp
        )  # udpate sparse timestamps
        idxs_parent_array = np.append(
            idxs_parent_array, len(sparse_timestamps) - 1
        )  # update array with corresponding idxs in full data
    ...
    if len(after_epoch_idxs) > 0:
        ...
        end_idx = -1
    flank_epoch_idxs = [
        idxs_parent_array[start_idx],
        idxs_parent_array[end_idx],
    ]

    # Now narrow to timestamps within epoch
    ptp_times_around_and_in_epoch = _get_ptp_times(
        nwbf, start_idx=flank_epoch_idxs[0], end_idx=flank_epoch_idxs[1]
    )  # times in and around epoch
    valid_idxs = np.where(
        np.logical_and(
            ptp_times_around_and_in_epoch >= epoch_interval[0],
            ptp_times_around_and_in_epoch <= epoch_interval[1],
        )
    )[
        0
    ]  # time idxs in epoch
    ...
    if any(
        [
            abs(t1 - t2) > tolerance_distance_to_epoch_bounds
    ...
    return {
        ...
        "trodes": np.round(
            (
                nwbf.fields["processing"]["sample_count"]
                .data_interfaces["sample_count"]
                .fields["data"][flank_epoch_idxs[0] : flank_epoch_idxs[1]]
            )
            * (1000 / estimated_fs)
        )[valid_idxs],
        "trodes_sample_count": nwbf.fields["processing"]["sample_count"]
        .data_interfaces["sample_count"]
        .fields["data"][flank_epoch_idxs[0] : flank_epoch_idxs[1]][valid_idxs],
    }  # convert from samples to ms and round to be able to
...
def get_epoch_dios_nwbf(nwbf, epoch, dio_name):
    ...
    dio_events_ptp = np.asarray(
        nwbf.fields["processing"]["behavior"]["behavioral_events"]
        .fields["time_series"][dio_name]
        .fields["data"]
    )
    dio_times_ptp = np.asarray(
        nwbf.fields["processing"]["behavior"]["behavioral_events"]
        .fields["time_series"][dio_name]
        .fields["timestamps"]
    )
    ...
    epoch_interval = get_epoch_interval_nwbf(
        nwbf=nwbf, epoch=epoch
    )  # get epoch interval
    epoch_filter = np.logical_and(
        dio_times_ptp >= epoch_interval[0], dio_times_ptp <= epoch_interval[1]
    )  # create filter for epoch
    dio_value_filter = (
        dio_events_ptp == 1
    )  # create filter for DIOs with value one
    epoch_dio_events_ptp = dio_events_ptp[
        np.logical_and(epoch_filter, dio_value_filter)
    ]
    epoch_dio_times_ptp = dio_times_ptp[
        np.logical_and(epoch_filter, dio_value_filter)
    ]
