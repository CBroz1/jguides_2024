...
def _convert_curation_spreadsheet_dtype(row, column_name):
    ...
    # ...Convert to boolean datatype
        ...
        ]  # strip_string whitespace and convert to bool variable

    # Return dataframe row with new datatypes
    return row


def get_curation_spreadsheet_notes(
    subject_id, date, sort_description, curation_version, tolerate_no_notes=True
):
    """
    ...
    merge_idx_start = (
        unpack_single_element(np.where(table[:, 0] == "merge group labels")[0])
        + 2
    )
    # ...Get index of row where merge group labels end: at "additional labels"
    merge_idx_end = unpack_single_element(
        np.where(table[:, 0] == "additional labels")[0]
    )
    ...
    return [
        pd.DataFrame.from_dict(
            {
                column_name: _convert_curation_spreadsheet_dtype(
                    row, column_name
                )
                for column_name, row in zip(
                    table[idx_start - 1, :], table[idx_start:idx_end, :].T
                )
            }
        )
        for (idx_start, idx_end) in [
            (merge_idx_start, merge_idx_end),
            (label_idx_start, label_idx_end),
        ]
    ]
...
def make_curation_data(
    ...
    for sort_group_id in sort_group_ids:
        ...
        if len(table_entry) == 0 and tolerate_nonexisting:
            print(
                f"No entry in CuratedSpikeSorting for key {k} and tolerate_nonexisting is True. Continuing..."
            )
            continue
        ...
        additional_params = {
            k: v
            for k, v in sort_group_key.items()
            if k not in ["curation_id", "waveform_params_name"]
        }

        waveform_params_name_map = {
            "whitened": get_default_waveform_params_name(),
        ...
        for waveforms_key in waveforms_key_map.values():
            if not (Waveforms & waveforms_key):
                if verbose:
                    print(
                        f"Populating Waveforms table with key {waveforms_key}..."
                    )
        ...
        if "units" in css_entry:
            ...
            for waveforms_type, waveforms_key in waveforms_key_map.items():
                ...
                data_subset["sampling_frequency"] = (
                    we.sorting.get_sampling_frequency()
                )
                ...
                data_subset["waveform_window"] = np.arange(
                    -we.nbefore, we.nafter
                )
                # IMPORTANT NOTE: WAVEFORMS AND SPIKE TIMES ARE SUBSAMPLED (SEEMS MAX IS AT 20000). This
                # happens in line below.
                waveform_data = {
                    unit_id: we.get_waveforms(
                        unit_id, with_index=True, force_dense=True
                    )
                ...
                data_subset["spike_times"] = {
                    unit_id: timestamps_raw[
                        samples[data_subset["waveform_indices"][unit_id]]
                    ]
                    for unit_id, samples in spike_samples.items()
                }

                # Get average waveforms
                data_subset["average_waveforms"] = get_average_waveforms(
                    data_subset["waveforms"]
                )

                # Get peak channels
                data_subset["peak_channels"] = get_peak_channels(
                    data_subset["average_waveforms"]
                )

                # Get waveform amplitudes
                data_subset["amplitudes"] = get_waveform_amplitudes(
                    data_subset["waveforms"]
                )

                # Get amplitude size comparison
                data_subset["amplitude_size_comparisons"] = (
                    get_amplitude_size_comparisons(data_subset)
                )
                ...
                data_subset["correlogram_isi_violation_ratios"] = (
                ...
                data_subset["correlogram_asymmetries"] = (
                    get_correlogram_asymmetries(data_subset["correlograms"])
                )
                data_subset["correlogram_asymmetry_directions"] = (
                    get_correlogram_asymmetry_directions(
                        data_subset["correlograms"]
                    )
                )
                data_subset["correlogram_counts"] = get_correlogram_counts(
                    data_subset["correlograms"]
                )
                ...
                data_subset["burst_pair_amplitude_timing_bools"] = (
                    get_burst_pair_amplitude_timing_bools(data_subset)
                )

                # Get ISI violation percent for merged unit pairs
                data_subset["unit_pair_percent_isi_violations"] = (
                    get_unit_pair_percent_isi_violations(data_subset)
                )
                ...
                if include_amplitude_decrement_quantities:
                    ...
                    for max_dt in [0.015, 0.4]:
                        ...
                        ] = get_unit_merge_amplitude_decrements(
                            data_subset, max_dt
                        )
                        data_subset[f"amplitude_decrement_changes_{max_dt}"] = (
                            get_amplitude_decrement_changes(data_subset, max_dt)
                        )
                    ...
                    ] = get_unit_merge_valid_lower_amplitude_fractions(
                        data_subset
                    )
...
def label_units(
    ...
    preproc_params_name = return_spikesorting_params()["preproc_params_name"][
        targeted_location
    ]
    ...
    curation_merge_notes, unit_notes = get_curation_spreadsheet_notes(
        subject_id, date, sort_description, curation_version
    )
    ...
    sort_group_unit_ids = np.concatenate(
        [
            [
                ...
                for unit_id in vals["whitened"]["waveforms"].keys()
            ]
            for sort_group_id, vals in cluster_data["sort_groups"].items()
            if "whitened" in vals
        ]
    )
    sort_group_ids, unit_ids = list(
        map(np.asarray, list(zip(*sort_group_unit_ids)))
    )
    ...
    unique_sort_group_ids += [
        sort_group_id
        for sort_group_id, vals in cluster_data["sort_groups"].items()
        if "whitened" not in vals
    ]
    # ...set labels to accept
    labels = {
        sort_group_id: {
            unit: ["accept"]
            for unit in unit_ids[sort_group_ids == sort_group_id]
        }
        for sort_group_id in unique_sort_group_ids
    }

    # Initialize dictionary with merge groups
    merge_groups = {
        sort_group_id: [] for sort_group_id in unique_sort_group_ids
    }

    # Fill in changes
    if len(curation_merge_notes) > 0:
        merge_bool = curation_merge_notes[label_col_name] == "merge"
        merge_unit_id_pairs = list(
            zip_df_columns(
                curation_merge_notes[merge_bool], ["unit_id_1", "unit_id_2"]
            )
        )
        merge_sort_group_ids = curation_merge_notes[merge_bool]["sort_group_id"]
        # ...merge groups
        for merge_sort_group_id, merge_unit_id_pair in zip(
            merge_sort_group_ids, merge_unit_id_pairs
        ):
            if (
                merge_sort_group_id not in unique_sort_group_ids
            ):  # restrict to current brain region
                continue
    ...
    if len(curation_merge_notes) > 0:
        mua_bool = (
            np.asarray(
                [x.strip() for x in curation_merge_notes[label_col_name]]
            )
            == "both mua"
        )
        for sort_group_id, unit_id_1, unit_id_2 in zip_df_columns(
            curation_merge_notes[mua_bool],
            ["sort_group_id", "unit_id_1", "unit_id_2"],
        ):
            if (
                sort_group_id not in unique_sort_group_ids
            ):  # restrict to current brain region
                continue
    ...
    if len(unit_notes) > 0:
        for label in ["mua", "reject"]:
            invalid_bool = (
                np.asarray([x.strip() for x in unit_notes[label_col_name]])
                == label
            )
            for sort_group_id, unit_id in zip_df_columns(
                unit_notes[invalid_bool], ["sort_group_id", "unit_id"]
            ):
                if (
                    sort_group_id not in unique_sort_group_ids
                ):  # restrict to current brain region
...
def _compute_max_shift_cosine_similarity(wv_avg_1, wv_avg_2):
    ...
    for i in np.arange(-shift, shift + 1):
        ...
        wv_avg_nrm_1, wv_avg_nrm_2 = (
            wv_avg / np.linalg.norm(wv_avg, axis=0)
            for wv_avg in (wv_avg_1_, wv_avg_2_)
        )
...
def get_correlogram_default_params():
    return {"max_dt": 0.4, "min_dt": 0}


def _compute_correlogram(
    spk_times_1, spk_times_2, unit_1, unit_2, max_dt=None, min_dt=None
):
    # Get inputs if not passed
    if max_dt is None:
...
def get_max_shift_cosine_similarities(average_waveforms):
    return _compute_pairwise_cluster_data(
        _compute_max_shift_cosine_similarity, average_waveforms
    )
    ...
    for cluster_1, data_1 in spike_times.items():
        for cluster_2, data_2 in spike_times.items():
            pool.apply_async(
                ...
                args=(data_1, data_2, cluster_1, cluster_2, max_dt, min_dt),
...
def get_merge_candidates(
    cluster_data, threshold_sets, sort_group_ids=None, waveform_type="whitened"
):
    # Get inputs if not passed
    if sort_group_ids is None:
...
def merge_plot(
    ...
    fig, axes = plt.subplots(
        num_rows,
        num_columns,
        figsize=(num_columns * subplot_width, num_rows * subplot_height),
    ...
    fig.suptitle(
        f"{sort_group_id}_{unit_1} vs. {sort_group_id}_{unit_2}{threshold_name_text}",
        fontsize=20,
    )
    fig.tight_layout()
...
def plot_amplitude(
    ...
    if ax is None:
        _, ax = plt.subplots()
    if ch is None:
        ch = cluster_data["sort_groups"][sort_group_id][waveforms_type][
            "peak_channels"
        ][unit_id]
    ...
    ax.scatter(
        data_subset["spike_times"][unit_id],
        data_subset["amplitudes"][unit_id][ch, :],
        s=1,
        color=color,
    )
...
def plot_amplitudes(
    nwb_file_name,
    cluster_data,
    target_sort_group_unit_ids,
    waveforms_type="unwhitened",
    ...
    fig, axes = get_fig_axes(
        num_rows=len(target_sort_group_unit_ids),
        num_columns=len(ylims),
        subplot_width=10,
        subplot_height=2,
    )
    for sort_group_id, unit_id in target_sort_group_unit_ids:
        ...
        for ylim_idx, ylim in enumerate(ylims):
            ...
            plot_amplitude(
                ...
                unit_id,
                peak_ch,
                waveforms_type=waveforms_type,
                color=sort_group_colors[sort_group_id],
                ax=ax,
            )

            if (
                ylim is not None
            ):  # for some reason, seems if pass None, restriction still applied in some weird way
                ax.set_ylim(ylim)
            ...
            for table_name, color in zip(
                epoch_type_table_names, epoch_type_colors
            ):
...
def get_above_threshold_matrix_indices(
    ...
    return {
        threshold_name: _apply_metric_matrix_thresholds(
            cluster_data, sort_group_id, threshold_set.thresholds, waveform_type
        )
...
def _apply_metric_matrix_thresholds(
    cluster_data, sort_group_id, threshold_objs, waveform_type
):
    return np.prod(
        [
...
def plot_average_waveforms_wrapper(
    cluster_data, sort_group_unit_ids=None, subplot_height=3
):
    # Get inputs if not passed
    if sort_group_unit_ids is None:
        sort_group_unit_ids = np.concatenate(
            [
                [
                    (sort_group_id, unit_id)
                    for unit_id in cluster_data["sort_groups"][sort_group_id][
                        "whitened"
                    ]["average_waveforms"].keys()
                ]
    ...
    for sort_group_id, unit_id in sort_group_unit_ids:
        ...
        if (
            "merged_units"
            in cluster_data["sort_groups"][sort_group_id]["whitened"]
        ):
            if (
                unit_id
                in cluster_data["sort_groups"][sort_group_id]["whitened"][
                ...
                color = "red"
        plot_average_waveforms(
            cluster_data, sort_group_id, unit_id, color=color, ax=ax
        )
