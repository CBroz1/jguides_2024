...
class SpikeSortingRecordingCohortParams(SecKeyParamsBase):
    ...
    def insert_entry(
        ...
        available_sort_group_ids = (SpikeSortingRecording & key).fetch(
            "sort_group_id"
        )
        check_all_unique(
            available_sort_group_ids
        )  # if this fails, must make key more restrictive
        if not all(np.isin(sort_group_ids, available_sort_group_ids)):
            print(
                f"Not all sort groups available for key {nwb_file_name}, {sort_interval_name}. "
        ...
        for ssr_entry in (SpikeSortingRecording & key).proj():
            ...
            secondary_key_subset_map = {
                ...
                "nwb_file_name": nwb_file_name,
                "sort_interval_name": sort_interval_name,
                "preproc_params_name": ssr_entry["preproc_params_name"],
                "team_name": ssr_entry["team_name"],
            }
            spike_sorting_recording_cohort_param_name = self.make_param_name(
                secondary_key_subset_map
            )
            params_entry = {
                **{
                ...
                **{
                    column_name: ssr_entry[column_name]
...
class SpikeSortingRecordingCohort(ComputedBase):
    ...
    def make(self, key):
        ...
        available_sort_group_ids = (
            SpikeSortingRecording
            & {
                "nwb_file_name": ssr_cohort_params["nwb_file_name"],
                "sort_interval_name": ssr_cohort_params["sort_interval_name"],
            }
        ).fetch("sort_group_id")
        check_all_unique(available_sort_group_ids)
        if not all(
            np.isin(
                ssr_cohort_params["sort_group_ids"], available_sort_group_ids
            )
        ):
            print(
                f"Not all sort groups available for key {key}. Not populating SpikeSortingRecordingCohort."
            )
            return
        # Insert into main table
        self.insert1(key)
        print(
            "Populated SpikeSortingRecordingCohort for cohort "
        ...
        for sort_group_id in ssr_cohort_params["sort_group_ids"]:
            table_entry = (
                SpikeSortingRecording
                & {**ssr_cohort_params, **{"sort_group_id": sort_group_id}}
            ).fetch1()

            entry_key = {**table_entry, **key}
            SpikeSortingRecordingCohort.CohortEntries.insert1(
                entry_key, skip_duplicates=True
            )
            print(
                "Added an entry to SpikeSortingRecording.CohortEntries for cohort "
                "{spike_sorting_recording_cohort_param_name}".format(
                    **entry_key
                )
            )
...
def get_metric_params_name(max_spikes_for_nn):
    ...
    version_flag = "_v2"
    metric_params_name = (
        f"peak_offset_num_spikes_{max_spikes_for_nn}spikes{version_flag}"
    )
    starting_params = (
        MetricParameters
        & {"metric_params_name": f"peak_offset_num_spikes{version_flag}"}
    ).fetch1()
    starting_params.update({"metric_params_name": metric_params_name})
    starting_params["metric_params"]["nn_isolation"][
        "max_spikes"
    ] = max_spikes_for_nn
    starting_params["metric_params"]["nn_noise_overlap"][
        "max_spikes"
    ] = max_spikes_for_nn
...
def get_waveform_params_name(max_spikes_per_unit, n_jobs):
    ...
    waveform_params_name = (
        f"default_whitened_{max_spikes_per_unit}spikes_{n_jobs}jobs"
    )
    WaveformParameters().insert_default()
    starting_params = (
        WaveformParameters & {"waveform_params_name": "default_whitened"}
    ).fetch1()
    starting_params["waveform_params_name"] = waveform_params_name
    starting_params["waveform_params"].update(
        {"max_spikes_per_unit": max_spikes_per_unit, "n_jobs": n_jobs}
    )
...
def get_default_automatic_curation_params_name():
    ...
    isi_violation_threshold = 0.0025
    peak_offset_threshold = 2
    return get_automatic_curation_params_name(
        noise_threshold, isi_violation_threshold, peak_offset_threshold
    )


def get_automatic_curation_params_name(
    noise_threshold, isi_violation_threshold, peak_offset_threshold
):
...
def return_spikesorting_params():
    ...
    for region in ["CA1"]:
        parameter_set_dict["sorter_params_name"][
            region
    ...
    for region in ["mPFC", "OFC", "Cortex"]:
        parameter_set_dict["sorter_params_name"][
            region
        ] = "franklab_probe_ctx_30KHz_115rad_30clip"
        parameter_set_dict["preproc_params_name"][region] = "default_min_seg"
        from jguides_2024.spike_sorting_curation.jguidera_artifact import (
            return_global_artifact_detection_params,
        )

        parameter_set_dict["artifact"][region] = list(
            return_global_artifact_detection_params().keys()
        )[0]

    # Same param for all regions
    for region in ["CA1", "mPFC", "OFC", "Cortex"]:
        parameter_set_dict["waveform_params_name"][
            region
...
def print_sort_groups_CuratedSpikeSorting(
    nwb_file_names=None,
    curation_id=1,
    ...
    if nwb_file_names is None:
        ...
        nwb_file_names = get_jguidera_nwbf_names(
            high_priority=True, highest_priority=False
        )
    for nwb_file_name in nwb_file_names:
        sort_group_ids = np.sort(
            (
                CuratedSpikeSorting
                & {
                    "nwb_file_name": nwb_file_name,
        ...
        all_sort_group_ids = (
            SortGroupTargetedLocation & {"nwb_file_name": nwb_file_name}
        ).fetch("sort_group_id")
        missing_sort_group_ids = [
            x for x in all_sort_group_ids if x not in sort_group_ids
        ]
        print(
            f"Sort group ids in CuratedSpikeSorting for {nwb_file_name}, sort interval {sort_interval_name}, "
            f"curation_id {curation_id}: {sort_group_ids}"
        )
        print(
            f"Number of unique sort group ids: {len(np.unique(sort_group_ids))}"
        )
        print(
            f"Sort group ids in SortGroupTargetedLocation not yet in CuratedSpikeSorting: {missing_sort_group_ids}\n"
        )


def targeted_region_filter_parameter_set_name_map():
    return {
        "OFC": "franklab_default_cortex",
...
class DefineSortInterval:
    ...
    @classmethod
    def get_epochs_for_sort_interval_name(
        cls, nwb_file_name, sort_interval_name
    ):
        # Get epochs encompassed by a given sort interval

        # Get interval list name from sort interval name
        interval_list_name = cls.convert_sort_interval_name_interval_list_name(
            sort_interval_name
        )

        # Get starting interval list names from interval list name
        return NewIntervalList.get_epochs_for_new_interval_list_name(
    ...
    def get_epochs_for_curation_name(cls, nwb_file_name, curation_name):
        ...
        return cls.get_epochs_for_sort_interval_name(
            nwb_file_name, sort_interval_name
        )

    @classmethod
    def define_sort_interval_as_interval_list(
        cls, interval_list_name, interval_list, nwb_file_name
    ):
        sort_interval_name = cls.convert_sort_interval_name_interval_list_name(
            interval_list_name
        )
    ...
    def get_sort_interval_obj(self):
        ...
        IntervalList.insert1(
            {
                "nwb_file_name": self.nwb_file_name,
                "interval_list_name": obj.new_interval_list_name,
                "valid_times": obj.new_interval_list,
            },
            skip_duplicates=True,
        )
        ...
        sort_interval_name, sort_interval = (
            self.define_sort_interval_as_interval_list(
                obj.new_interval_list_name,
                obj.new_interval_list,
                self.nwb_file_name,
            )
        )
        return namedtuple(
            "SortIntervalQuantities", "interval_list_name sort_interval_name"
        )(obj.new_interval_list_name, sort_interval_name)


# Define valid times for sort
def define_sort_intervals(
    targeted_location, nwb_file_name, curation_set_name="runs_analysis_v1"
):
    """
    ...
    if curation_set_name in [
        "runs_analysis_v1",
        "runs_analysis_v2",
        "across_runs_analysis_v1",
        "across_runs_analysis_v2",
    ]:

        if curation_set_name in ["runs_analysis_v1", "across_runs_analysis_v1"]:
            version_num = None
        elif curation_set_name in [
            "runs_analysis_v2",
            "across_runs_analysis_v2",
        ]:
            if targeted_location in ["mPFC", "OFC"]:
                ...
                raise Exception(
                    f"version_num definition not accounted for, for targeted_location {targeted_location}"
                    f" and curation_set_name {curation_set_name}"
                )
        else:
            raise Exception(
                f"version_num definition not account for, for curation_set_name {curation_set_name}"
            )

        # Case 1: concatenated run and sleep (default)
        starting_interval_list_names_list = [
            ["raw data valid times"]
        ]  # use these interval lists and make changes as indicated by flags
        NO_PREMAZE = (
            True  # True to exclude periods when rat being carried to maze
        )
        ...
        unstable_hpc_nwb_file_names = [
            "peanut20201107_.nwb",
            "peanut20201108_.nwb",
            "peanut20201109_.nwb",
        ]
        if (
            nwb_file_name in unstable_hpc_nwb_file_names
            and targeted_location == "CA1"
        ):
            starting_interval_list_names_list = [
                [
        ...
        raise Exception(
            f"No code written for case where curation_set_name {curation_set_name}"
        )
