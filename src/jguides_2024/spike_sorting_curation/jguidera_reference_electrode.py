...
class ReferenceElectrode(dj.Manual):
    ...
    def insert_defaults(self, **kwargs):
        ...
        june_group_3 = {
            ...
            "hpc_right_nt": 10,
            "hpc_right_ch": 1,
        }  # use for day when nt23 went out and had not yet moved up nt22: june20220418
        june_group_4 = {
            "hpc_left_nt": 22,
            "hpc_left_ch": 1,
            "hpc_right_nt": 22,
            "hpc_right_ch": 1,
        }  # use for days when nt10 has axons
        june_group_5 = {
            "hpc_left_nt": 22,
            "hpc_left_ch": 1,
            "hpc_right_nt": 10,
        ...
        hpc_ref_dict = {
            ...
            **{file_name: june_group_2 for file_name in ["june20220415_.nwb"]},
            **{file_name: june_group_3 for file_name in ["june20220418_.nwb"]},
            **{file_name: june_group_4 for file_name in ["june20220419_.nwb"]},
            **{
        ...
        for (
            nwb_file_name,
            ref_dict,
        ) in hpc_ref_dict.items():  # for each nwb file
    ...
    def make_hpc_refs(hpc_ref_electrode, hpc_ref_ch):
        return [
            (e - 1) * 4 - 1 + c for e, c in zip(hpc_ref_electrode, hpc_ref_ch)
        ]
    ...
    def _insert_references(self, key, ref_dict):
        ...
        for hemisphere in ["right", "left"]:
            key[f"hpc_reference_electrode_{hemisphere}_hemisphere"] = (
                self.make_hpc_refs(
                    hpc_ref_electrode=[ref_dict[f"hpc_{hemisphere}_nt"]],
                    hpc_ref_ch=[ref_dict[f"hpc_{hemisphere}_ch"]],
                )[0]
            )
...
def make_refs_dict(nwb_file_name):
    ...
    ).fetch("electrode_group_name", "region_id", "target_hemisphere")
    for electrode_group_name, region_id, target_hemisphere in zip(
        electrode_group_names, region_ids, target_hemispheres
    ):
        region_name = (BrainRegion & {"region_id": region_id}).fetch1(
            "region_name"
        )
        if region_name in ["Hippocampus", "hippocampus", "corpus callosum"]:
            ...
            raise Exception(
                f"{region_name} not accounted for in code. Must specify map between this region and ReferenceElectrode columns"
            )
