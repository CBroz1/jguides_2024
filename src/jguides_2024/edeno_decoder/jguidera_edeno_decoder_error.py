...
class EdenoDecodeErrSummSel(SelBase):
    ...
    def _get_potential_keys(
        ...
        brain_region_units_cohort_types = np.unique(
            EdenoDecodeErrSummParams.fetch("brain_region_units_cohort_type")
        )
        # ...apply passed filter
...
class EdenoDecodeErrSumm(PathWellPopSummBase):
    ...
    def _get_val_text(self):
        ...
        boot_set_name = (EdenoDecodeErrSummParams & self.fetch1("KEY")).fetch1(
            "boot_set_name"
        )
    ...
    def _get_multiplot_params(**kwargs):
        ...
        kwargs.update(
            {
                "edeno_decode_err_summ_param_name": EdenoDecodeErrSummParams().lookup_param_name(
                    {
                        k: kwargs[k]
                        for k in [
                            "boot_set_name",
