...
class EdenoDecodeErrParams(SecKeyParamsBase):
    ...
    def _default_params(self):
        return [["absolute_original_scale", 0.01]]
...
class EdenoDecodeErr(ComputedBase):
    ...
    def get_aligned_map_test_pos(self, key=None):
        ...
        max_tolerated_time_diff = (EdenoDecodeErrParams & key).fetch1(
            "max_tolerated_time_diff"
        )

        return (EdenoDecodeMAP & key).get_aligned_map_test_pos(
            max_tolerated_time_diff
        )
    ...
    def make(self, key):
        ...
        if error_type == "absolute_original_scale":
            scale_factor = (
                EdenoDecode & key
            ).get_stacked_edge_track_graph_param("scale_factor", "test")

            error_df = pd.DataFrame(
                abs(
                    pos_obj.test_position.decode_position
                    - pos_obj.map_pos.map_posterior
                )
                / scale_factor,
                columns=["error"],
            )
        ...
        # ...raise exception if not accounted for in code
            ...
            raise Exception(
                f"error_type {error_type} not accounted for in code"
            )
    ...
    def plot_error(self):
        ...
        ax.plot(
            pos_obj.map_pos.map_posterior,
            color="red",
            label="MAP position_and_maze",
        )
        ax.plot(
            pos_obj.test_position.decode_position,
            color="gray",
            label="test position_and_maze",
        )
...
class EdenoDecodeErrSummBinParams(dj.Manual):
    ...
    def insert_defaults(self, **kwargs):
        ...
        for param_name, params in self._get_default_params_map().items():
            ...
            self.insert1(
                {
                    "edeno_decode_err_summ_bin_param_name": param_name,
                    "edeno_decode_err_summ_bin_params": params,
                },
...
class EdenoDecodeErrSummSecKeyParams(PopulationAnalysisSecKeyParamsBase):
    ...
    def _default_params(self):
        ...
        edeno_decode_err_summ_bin_param_names = (
            EdenoDecodeErrSummBinParams().get_default_param_names()
        )
        return [
            [x, y, z]
            for x in boot_set_names
            for y in edeno_decode_err_summ_bin_param_names
...
class EdenoDecodeErrSummSel(SelBase):
    ...
    def insert1(self, key, **kwargs):
        ...
        for upstream_key in upstream_keys:
            self.EdenoDecodeErr.insert1(
                {**key, **upstream_key}, skip_duplicates=True
            )
    ...
    def _get_potential_keys(
        ...
        brain_region_units_cohort_types = np.unique(
            EdenoDecodeErrSummSecKeyParams.fetch(
        ...
        ) in TrainTestEpochSet().get_train_test_epoch_set_names(
        ...
        if "train_test_epoch_set_name" in key_filter:
            train_test_epoch_set_boot_set_name_map = {
                k: v
                for k, v in train_test_epoch_set_boot_set_name_map.items()
                if k == key_filter["train_test_epoch_set_name"]
            }
        ...
        for brain_region_units_cohort_type in brain_region_units_cohort_types:
            ...
            for (
                train_test_epoch_set_name,
                boot_set_names,
            ) in train_test_epoch_set_boot_set_name_map.items():
                ...
                ).fetch1("nwb_file_names", "train_test_epoch_names")
                ...
                for param_set in param_sets:
                    ...
                    if populated:
                        ...
                        for boot_set_name in boot_set_names:
                            for edeno_decode_err_summ_param_name in (
                                EdenoDecodeErrSummSecKeyParams
                                & {
                                ...
                                key.update(
                                    {
                                        "edeno_decode_err_summ_param_name": edeno_decode_err_summ_param_name
                                    }
                                )
                                # Add key to keys
...
class EdenoDecodeErrSumm(PathWellPopSummBase):
    ...
    def make(self, key):
        ...
        boot_set_name, edeno_decode_err_summ_bin_param_name = (
            EdenoDecodeErrSummSecKeyParams & key
        ).fetch1("boot_set_name", "edeno_decode_err_summ_bin_param_name")

        # Get parameters for binning error
        bin_key = {
            "edeno_decode_err_summ_bin_param_name": edeno_decode_err_summ_bin_param_name
        }
        bin_params = (EdenoDecodeErrSummBinParams & bin_key).fetch1(
            "edeno_decode_err_summ_bin_params"
        )

        # Get position_and_maze bin edges
        bins_obj = EdenoDecode().get_pos_bins(
            bin_params["num_pos_bins"], "test", upstream_keys
        )
        ...
        if (
            ...
            pos_bin_nums = np.arange(
                1, len(bins_obj.bin_edges)
            )  # one indexed for use with np.digitize output
            ...
            for idx, upstream_key in enumerate(upstream_keys):
                ...
                pos_obj = (
                    EdenoDecodeErr & upstream_key
                ).get_aligned_map_test_pos()
                pos_digitized = np.digitize(
                    pos_obj.test_position.decode_position,
                    bins=bins_obj.bin_edges,
                )

                # Get mean error in position_and_maze bins
                for bin_num, x_val in zip(
                    pos_bin_nums, bins_obj.bin_centers_df.x_vals
                ):
                    ...
                    nwb_file_name_train_test_epoch_name = f"{upstream_key['nwb_file_name']}_{upstream_key['train_test_epoch_name']}"
    ...
    def _get_val_text(self):
        ...
        boot_set_name = (
            EdenoDecodeErrSummSecKeyParams & self.fetch1("KEY")
        ).fetch1("boot_set_name")
    ...
    def _get_val_lims(self, **kwargs):
        ...
        decode_variable_param_name = (EdenoDecodeParams & key).fetch1(
            "decode_variable_param_name"
        )
        boot_set_name = (EdenoDecodeErrSummSecKeyParams & key).fetch1(
            "boot_set_name"
        )
        ...
        if decode_variable_param_name == "ppt_default":
            ...
            raise Exception(
                f"decode_variable_param_name {decode_variable_param_name} not accounted for in code"
            )
    ...
    def _get_xticks(self):
        decode_variable_param_name = (
            EdenoDecodeParams() & self.fetch1("KEY")
        ).fetch1("decode_variable_param_name")
        if decode_variable_param_name == "ppt_default":
            return [0, 0.5, 1]
    ...
    def _get_multiplot_params(**kwargs):
        ...
        kwargs.update(
            {
                "edeno_decode_err_summ_param_name": EdenoDecodeErrSummSecKeyParams().lookup_param_name(
                    {
                        k: kwargs[k]
                        for k in [
                            "boot_set_name",
                            "edeno_decode_err_summ_bin_param_name",
                            "brain_region_units_cohort_type",
                        ]
                    },
                    args_as_dict=True,
        ...
        for edeno_decode_param_name in kwargs["edeno_decode_param_names"]:
            ...
            for train_test_epoch_set_name in kwargs[
                "train_test_epoch_set_names"
            ]:
                kwargs.update(
                    {"train_test_epoch_set_name": train_test_epoch_set_name}
    ...
    def _get_multiplot_ax_key(**kwargs):
        ...
        return (
            kwargs["train_test_epoch_set_name"],
            kwargs["edeno_decode_param_name"],
            0,
            column_iterable,
        )
