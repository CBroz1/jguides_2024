...
alpha_path_context = 0.09
s_path_context = subplot_width * 2
PlotParams = namedtuple(
    "PlotParams", "plot_fn_names kwargs_list time_bin_shorthands"
)
...
plot_params_map = {
    "task_period": PlotParams(
        [
            "plot_by_ppt",
            "plot_by_relative_time_in_delay",
            "plot_by_relative_time_at_well_post_delay",
        ],
        [
            {"s": s_task_progression},
            {"s": s_task_progression, "exclusion_params": wa_exclusion_params},
            {"s": s_task_progression},
        ],
...
row_iterables = np.arange(
    0, np.max([len(x.plot_fn_names) for x in plot_params_map.values()])
)  # correspond to coloring schemes
...
fr_vec_emb_param_name = FRVecEmbParams().lookup_param_name(
...
for _, subject_df_subset in subject_df.iterrows():
    ...
    if len(epochs) == 1:
        brain_region_units_param_name = (
            BrainRegionUnitsParams().lookup_single_epoch_param_name(
                nwb_file_name,
                int(unpack_single_element(epochs)),
    ...
    # Otherwise, use all units active across run epochs in the day
        ...
        brain_region_units_param_name = (
            BrainRegionUnitsParams().lookup_param_name(
                [
                    EpsUnitsParams().lookup_param_name(
                        [min_epoch_mean_firing_rate]
                    ),
                    "runs",
                    "all",
    ...
    brain_regions_ = [
        x
        for x in brain_regions_
        if BrainRegionUnits().get_num_units(
            {
                **key,
                **{
                    "brain_region": x,
    ...
    if populate_tables:
        ...
        time_bin_shorthands = np.unique(
            np.concatenate(
                [x.time_bin_shorthands for x in plot_params_map.values()]
            )
        )
        for time_bin_shorthand in time_bin_shorthands:
            ...
            ).fetch1("res_time_bins_pool_param_names")
    ...
    with plt.style.context("dark_background"):
        ...
        gs_map, ax_map, fig = get_gridspec_ax_maps(
            ...
            wspace=wspace,
            hspace=hspace,
            add_subplot_args=add_subplot_args,
        )
                    ...
                    for brain_region in brain_regions_:
                        ...
                        epochs_description = (
                            EpochsDescription().get_single_run_description(
                                nwb_file_name, target_epoch
                            )
                        )
                        curation_name = df_pop(
                            (CurationSet & key).fetch1_dataframe(),
                            {
                                "brain_region": brain_region,
                                "epochs_description": epochs_description,
                            },
                        ...
                        # Plot embedding
                        getattr(FRVecEmb & key, plot_fn_name)(
                            target_epoch,
                            ax=ax,
        ...
        # Save figure
        file_name = (
            f"embedding_{format_nwb_file_name(key['nwb_file_name'])}_eps{key['epochs_id']}_sm"
            + f"{key['res_epoch_spikes_sm_param_name']}_{brain_region_units_param_name}_{fr_vec_emb_param_name}"
        )
        save_figure(
            fig, file_name, save_fig=save_fig, dpi=800, figure_type=".png"
        )
