...
def decoding_plot(
    ...
    train_test_epoch_set_names = [
        TrainTestEpochSet().get_Haight_single_contingency_rotation_set_name(
            [subject_id]
        )
    ...
    if rat_cohort:
        train_test_epoch_set_names = [
            TrainTestEpochSet().lookup_rat_cohort_set_name()
        ]
    ...
    params.update(
        {
            "edeno_decode_param_names": edeno_decode_param_names,
            "train_test_epoch_set_names": train_test_epoch_set_names,
...
params.update({"mega_row_gap_factor": 0.18})
