...
def _plot_legend(
    brain_region_vals, relationship_vals, save_fig=False, file_name=None
):

    # Get map from brain region value to color
    colors_df = PathWellPopSummBase()._get_colors_df(
        "brain_region_val", "relationship_val"
    )

    # Initialize figure
    width_scale_factor = 0.53
    height_scale_factor = 0.2
    fig, ax = plt.subplots(
        figsize=(
            len(brain_region_vals) * width_scale_factor,
            len(relationship_vals) * height_scale_factor,
        )
    )

    # Plot
    for brain_region_val_idx, brain_region_val in enumerate(brain_region_vals):
        for relationship_val_idx, relationship_val in enumerate(
            relationship_vals[::-1]
        ):
            color = df_pop(
                colors_df,
                {
                    ...
                    "relationship_val": relationship_val,
                },
                "color",
            )
            ax.add_patch(
                matplotlib.patches.Rectangle(
                    [brain_region_val_idx, relationship_val_idx],
                    1,
                    1,
                    facecolor=color,
                )
...
def nn_plot(
    ...
    if plot_type == "default":
        fr_diff_vec_cos_sim_wa_nn_ave_param_name = (
            "1^0.25^mask_duration_10^potentially_rewarded_trial_stay_trial"
        )
    ...
    FRDiffVecCosSimCovNnAveSummBase().multiplot(
        recording_set_names,
        table_names,
        brain_regions_separate,
        brain_region_vals,
...
if make_plot:
    ...
    fig = _plot_legend(
        ...
        relationship_vals,
        True,
        "brain_region_relationship_colors_legend_path",
    )
    ...
    fig = _plot_legend(
        ...
        relationship_vals,
        True,
        "brain_region_relationship_colors_legend_well",
    )
    ...
    _plot_legend(
        ...
        relationship_vals,
        True,
        "brain_region_relationship_colors_legend_path_div",
    )
    ...
    _plot_legend(
        ...
        relationship_vals,
        True,
        "brain_region_relationship_colors_legend_well_div",
    )
