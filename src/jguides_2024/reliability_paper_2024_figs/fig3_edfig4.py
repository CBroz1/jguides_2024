...
def _get_param_names(
    condition_name,
    time_bins_shorthand,
    cross_validation_table_name,
    covariates_shorthand,
):
    ...
    condition_trials_param_name = ConditionTrialsParams().lookup_param_name(
        [condition_name]
    )
    trials_pool_param_name = (
        ResTimeBinsPoolSel().lookup_trials_pool_param_name_from_shorthand(
            time_bins_shorthand
        )
    )
    source_table_key = {
    ...
    train_test_split_pool_param_name = (
        TrainTestSplitPoolSel().lookup_param_name(
            source_table_name=cross_validation_table_name,
...
def get_unit_names_map(nwb_file_name, epochs_id, test_mode=False):
    ...
    if nwb_file_name == "J1620210607_.nwb" and epochs_id == "10":
        unit_names_map = {
            ("CA1", 0): ["8_11", "0_6", "12_25"],
            ("CA1", 1): ["15_18", "0_1"],
            ("mPFC", 0): ["28_34", "29_24", "27_27"],
            ("mPFC", 1): ["29_59", "29_7"],
            ("OFC", 0): ["22_99", "23_4", "23_22"],
            ("OFC", 1): ["23_83", "25_43"],
        }

    elif nwb_file_name == "J1620210606_.nwb" and epochs_id == "8":
        unit_names_map = {
            ("CA1", 0): ["12_17", "15_14", "9_2"],
            ("CA1", 1): ["10_15", "0_6"],
            ("mPFC", 0): ["26_11", "29_32", "27_49"],
            ("mPFC", 1): ["27_37", "29_5"],
            ("OFC", 0): ["24_50", "24_110", "23_32"],
            ("OFC", 1): ["25_32", "24_68"],
        }
    ...
    if unit_names_map is None:
        raise Exception(
            f"unit_names_map not defined for {nwb_file_name}, epochs_id {epochs_id}"
        )
...
param_names_obj = _get_param_names(
    condition_name,
    time_bins_shorthand,
    cross_validation_table_name,
    covariates_shorthand,
)
prediction_similarity_restrictions = {
    "covariate_restrictions": {"ppt": [0, 1]},
    "restrict_to_non_overlapping_space": False,
}
data_list.append(
    (
        name,
        param_names_obj.res_time_bins_pool_cohort_param_name,
        param_names_obj.train_test_split_pool_param_name,
        param_names_obj.x_interp_pool_cohort_param_name,
        condition_name,
        prediction_similarity_restrictions,
    )
)
...
min_num_spikes = 1
res_time_bins_pool_cohort_param_names = [
    ResTimeBinsPoolCohortParams().lookup_param_name_from_shorthand("path_20ms")
]
...
populated_keys = ElNet().print_populated(
    return_keys=True, populate_tables=populate_tables
)
...
if group_data_quick_load:
    ...
    if restrict_single_contingency:
        populated_keys = [
            x
            for x in populated_keys
            if "J1620210606_.nwb" == x["nwb_file_name"]
            and x["epochs_id"] == "8"
        ]
    ...
    for key in keys:
        ...
        file_name_save = (
            "glm_"
            + format_nwb_file_name(key["nwb_file_name"])
            + f"_ep{epoch}"
            + min_spikes_text
            + min_fr_text
        )
        ...
        # Store data
        data_list.append(
            (key["nwb_file_name"], key["epochs_id"], glm_container)
        )
    glm_container_df = df_from_data_list(
        data_list, ["nwb_file_name", "epochs_id", "glm_container"]
    )
    ...
    for nwb_file_name in np.unique(glm_container_df.nwb_file_name):
        epochs = np.unique(
            [
                (
                    EpochCohort
                    & {"nwb_file_name": nwb_file_name, "epochs_id": x}
                ).get_epoch()
                for x in df_filter_columns(
                    glm_container_df, {"nwb_file_name": nwb_file_name}
                ).epochs_id
            ]
        )
...
if not group_data_quick_load:
    ...
    brain_regions = [
        x
        for x in ["mPFC", "OFC", "CA1"]
        if x in [x[0] for x in unit_names_map.keys()]
    ]
    ...
    height_ratios = np.asarray(
        [
            np.max(
                [
                    len(v)
                    for k, v in unit_names_map.items()
                    if k[1] == glm_restriction_idx
                ]
            )
    ...
    height_ratios = height_ratios + np.asarray(
        [0, 0.1]
    )  # for some reason, figure looks slightly imbalanced without this correction

    # Define global figure that spans all subfigures
    # define horizontal spacing between plots
    glm_restriction_hspace = 0.05
    units_hspace = 0
    ...
    subfigs_glm_restriction_idxs = global_fig.subfigures(
        nrows=num_glm_restriction_idxs,
        ncols=1,
        hspace=glm_restriction_hspace,
    ...
    for glm_restriction_idx_idx, glm_restriction_idx in enumerate(
        ...
        num_conditions = len(
            glm_container.conditions_df.loc[
                glm_container._get_glm_restriction_val(
                    glm_restriction_idx, "condition"
                )
            ].condition_vals
        )
        wspace = 0.3 / num_conditions
        glm_restriction_idx_fig = subfigs_glm_restriction_idxs[
            glm_restriction_idx_idx
        ]
        subfigs_brain_regions = glm_restriction_idx_fig.subfigures(
            nrows=1, ncols=num_brain_regions, wspace=wspace
        )
        ...
        for brain_region_idx, brain_region in enumerate(brain_regions):
            ...
            subfigs_units = units_fig.subfigures(
                nrows=len(unit_names), ncols=1, hspace=units_hspace
            )
            for unit_name, fig in zip(unit_names, subfigs_units):
                glm_container.nnls.plot_fr_pred_sim_glm_coeff(
